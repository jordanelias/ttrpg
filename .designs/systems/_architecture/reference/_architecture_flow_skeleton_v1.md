...
|---|---|---|
| `TypeRegistry.load` | `engine/substrate/keys.py:213 load` | `engine/cross_scale/echo_transport.py:84` (inside `_registry`); also the cook-side caller `tools/export_key_types.py:63` (inside `build`) |
...
| `KeyLog.content_hash` | `engine/substrate/keys.py:459 content_hash` | `engine/mc_v18.py:332 key_log_hash` |
| `KeyLog.__len__` | `engine/substrate/keys.py:358 __len__` | `engine/mc_v18.py:333 keys_emitted` |
...
| `TickScheduler.emit` | `engine/substrate/keys.py:510 emit` | `engine/cross_scale/echo_transport.py:345`, `:438`; `systems/factions/sim/faction_action.py:424`; `systems/factions/sim/parliamentary_transfer.py:240` |
...
| `TickScheduler.accounting_boundary` | `engine/substrate/keys.py:581 accounting_boundary` | `engine/mc_v18.py:179` |
| `TickScheduler.next_tick` | `engine/substrate/keys.py:593 next_tick` | `engine/mc_v18.py:180` |
...
- S3 `[loop]` per season, `run_season` invokes `_faction_actions_callback` — `engine/mc_v18.py:286`, `:116`
  ...
  - S3.4 `[write][gate]` `accounting_boundary()`: runs every queued `_pending_apply` callable in emission order — this is where the deferred Faction/Settlement writes from S3.2.2.4.3/.4.7 actually land; resets phase — `engine/mc_v18.py:177-179`, `engine/substrate/keys.py:581-591`
  - S3.5 `next_tick()`: raises `TerminationBreach` if the emission queue is non-empty, else resets the per-tick emission counter — `engine/mc_v18.py:180`, `engine/substrate/keys.py:593-601`

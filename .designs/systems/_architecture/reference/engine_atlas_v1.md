...
> Original path: `systems/_architecture/reference/engine_atlas_v1.md`
...
commit `6545067`, format owned by `systems/_architecture/reference/subsystem_flow_skeletons_v1.md`, guarded by
...
`systems/_architecture/reference/subsystem_flow_skeletons_v1.md` §5, not repeated here.
...
`systems/_architecture/reference/subsystem_flow_skeletons_v1.md:145`.

---

## 1. The campaign spine

One ordered loop; `run_campaign` is the engine's single driver — `engine/mc_v18.py:239 run_campaign`.
...
from the geography YAML — `engine/autoload/game_state.py:304 create_world`, called at
`engine/mc_v18.py:251 run_campaign`; settlements down-seam `engine/autoload/game_state.py:304-305 create_world`.
**S2** The victory streak tracker and the scene slate are module-level, not `world` state, so they are
explicitly cleared — `engine/mc_v18.py:252-253 run_campaign`. **S3** Two flags resolve once:
`DISPATCH_COMBAT_BRIDGE` (default OFF) is stashed on `world` — `engine/mc_v18.py:264 run_campaign` — and
`ECHO_TRANSPORT` (default ON) decides whether the Key substrate attaches at all —
`engine/mc_v18.py:268 run_campaign`. **S4** When it is on, `make_scheduler` builds the type registry →
`KeyLog` → `TickScheduler` onto `world.echo_scheduler` — `engine/mc_v18.py:270 run_campaign`. **S5** 13
trigger callbacks subscribe to that scheduler — `engine/cross_scale/articulation.py:152 subscribe_all`,
called at `engine/mc_v18.py:285 run_campaign`; none of the 13 can fire in a default campaign (§3c).

**The season loop.** `for _ in range(max_s)`, breaking as soon as a winner exists —
`engine/mc_v18.py:287-288 run_campaign`. Each iteration calls `run_season`, whose composition is exactly
three steps — `systems/overview/sim/season.py:48 run_season`, invoked at `engine/mc_v18.py:294 run_campaign`.
...
- **S7 — Step 2, the action callback** supplied by the driver — `engine/mc_v18.py:143 _faction_actions_callback`.
  - **S7.1 Faction actions.** For each faction parliamentary *and* holding territory: `faction_take_action`,
    wrapped so one faction's exception cannot abort the season — `engine/mc_v18.py:151-163 _faction_actions_callback`;
  ...
  - **S7.2 The scene phase.** `run_scene_phase` evaluates triggers, queues scenes, then drains the slate
    ...
    `engine/mc_v18.py:168 _faction_actions_callback`. Exactly **one** trigger is field-evaluable today
    (Stability Crisis) and it queues a `contest` scene — `engine/cross_scale/scene_dispatch.py:77 evaluate_triggers`.
    Each slot zooms in, dispatches by `scene_type`, checks handoff validity, emits its Domain Echo, zooms
    out — `engine/cross_scale/scene_dispatch.py:403 dispatch_scenes`; the contest branch derives two parties
    from one faction's own aggregates and runs the Agôn kernel — `engine/cross_scale/scene_dispatch.py:292-305 _resolve_slot`.
...
- **S9 — Victory check.** Every faction is scored; the first qualifying result sets `world.winner` —
  `engine/autoload/victory.py:103 check_all_factions`, called at `engine/mc_v18.py:297 run_campaign`. The
  loop exits at the *next* iteration's guard, not here — `engine/mc_v18.py:298-301 run_campaign`.
...
`engine/mc_v18.py:303-313 run_campaign`. **S11** The world is dict-serialized into
`CampaignResult.final_state` alongside the telemetry counters — `engine/autoload/game_state.py:355 serialize_world`,
called at `engine/mc_v18.py:334 run_campaign`.
...
— `engine/cross_scale/scene_dispatch.py:77 evaluate_triggers`. **No `thread` scene type exists** in the
dispatcher and the driver imports no threadwork module — `engine/cross_scale/scene_dispatch.py:218-226 _resolve_slot`.
...
declares the field that gates it — `engine/cross_scale/echo_transport.py:169-174 classify_scene_outcome`.
...
authority still heads at the demoted doc — `CURRENT.md:56 npc_behavior_v30` (§3e).
...
undocumented by the contract — `engine/mc_v18.py:303-313 run_campaign` (§3b, §3e).
...
here include the fieldwork loss-consequence asymmetry (`systems/fieldwork/reference/fieldwork_flow_skeleton_v1.md:153`),
...
| f | The driver's victory-threshold param is dead: the live gate is the victory module's own constant, a different owner with no wiring between them — pinned by a falsifier test that sweeps the dead param and moves no outcome | overview, victory | `engine/mc_v18.py:69-81 DEFAULT_PARAMS` |
...
| Head rows missing from the currency authority for four subsystem folders (`characters`, `ui`, `victory`, `world`), one of which holds a canonical head doc on disk | IN | `systems/_architecture/reference/subsystem_flow_skeletons_v1.md:145` |

...
> Original path: `systems/_architecture/reference/holonic_container_doctrine_v1.md`
...
| **Emergence contract** (name the local systems that produce each intended emergent behaviour; prohibit scripted shortcuts) | P-01..P-15 (`canon/`), the no-GM principle, the emergent-arc machinery (`valoria-arc-generator`), the anti-fabrication gate | Exists as philosophy + tooling; not yet per-behaviour spec. The two named failure modes below are its enforcement edge |
...
1. **Scripting drift.** The cheapest path to a working behaviour is a top-down special case
   ...
   terms: nothing may write global state to force a P-01..P-15-shaped result; the

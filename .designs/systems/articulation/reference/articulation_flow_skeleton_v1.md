...
> Original path: `systems/articulation/reference/articulation_flow_skeleton_v1.md`
...
(`references/module_contracts.yaml:1104` `sim_module`), per standing rule 1.
...
| `evaluate_articulation_triggers(world)` | `engine/cross_scale/articulation.py:44 evaluate_articulation_triggers` | `engine/tests/test_pipeline_reach.py:790 evaluate_articulation_triggers` — a stub-conversion regression probe only; see §7 |
...
| `_on_key(key, scheduler)` (closure returned by `_make_trigger_callback`) | `engine/cross_scale/articulation.py:140 _on_key` | `engine/substrate/keys.py:629 callback` — `TickScheduler._emit_at_depth`'s subscriber-dispatch loop |
...
| `key` (`Key`, the emitted object passed to the callback) | key | `TickScheduler._emit_at_depth`'s subscription loop | `engine/substrate/keys.py:629 callback` |

## 3. Flow

- **S1** `[gate]` Campaign boot resolves the `ECHO_TRANSPORT` flag (params override, else env var,
  default `'1'`). Resolution logic: `engine/mc_v18.py:84-94 _echo_transport_on`; called at
  `engine/mc_v18.py:268 _echo_transport_on`
  - **S1.1** `[branch]` If ON: attach `world.echo_scheduler = echo_transport.make_scheduler(...)`.
    `engine/mc_v18.py:270 make_scheduler`
  - **S1.2** `[write]` `subscribe_all(world.echo_scheduler)` runs once at boot, inside the same
    `if _echo_transport_on(...)` branch. `engine/mc_v18.py:285 subscribe_all`
...
- **S2** `[gate]` For a callback to ever fire, some production code must call
  ...
  `engine/substrate/keys.py:562 emit`
  - **S2.1** `[branch]` The only production module that constructs and emits real `Key` objects
    ...
    `engine/cross_scale/echo_transport.py:109-112 KEY_TYPE_BY_SCENE`,
    `engine/cross_scale/echo_transport.py:321-323 Key`
...
- **S3** `[emit]` When a matching `Key` IS emitted, `TickScheduler._emit_at_depth` logs it, then
  ...
  `engine/substrate/keys.py:628-616 callback`
...
- **S6** `[emit]` At campaign end, `run_campaign` reads the pre/post delta of that counter into
  `CampaignResult.stub_hits`. `engine/mc_v18.py:327 stub_hits`
...
|---|---|---|---|
| `StubResult` (frozen, `stub=True`) | return value, discarded by the caller | `TickScheduler._emit_at_depth`'s `callback(key, self)` call does not capture the return value | `engine/cross_scale/articulation.py:141 stub_resolve`, `engine/substrate/keys.py:629 callback` |
| `stubwire.invocations` delta | telemetry counter | `CampaignResult.stub_hits` | `engine/mc_v18.py:327 stub_hits` |
| `scheduler.subscriptions[type_id]` list entries | registration side-effect | `engine.substrate.keys.TickScheduler` (owning instance, held on `world.echo_scheduler`) | `engine/cross_scale/articulation.py:169 subscribe_all`, `engine/substrate/keys.py:559 subscribe` |
...
| `TickScheduler.subscriptions` | W (append-only, via `.subscribe`) | `engine.substrate.keys` | `engine/substrate/keys.py:559 subscribe` |
...
| `out` | `engine.substrate.keys.TickScheduler` | `subscribe_all` registers callbacks via the substrate's `.subscribe`; callbacks are later invoked by the substrate's own emission path | `engine/cross_scale/articulation.py:169 subscribe_all`, `engine/substrate/keys.py:628-616 callback` |
...
| **Tier 2's own trigger-evaluation function is never invoked by production code either.** `evaluate_articulation_triggers`'s only caller in the tree is a stub-conversion regression probe that asserts it is *still* stub-wired, not a consumer of its output; `engine/mc_v18.py`'s season loop never calls it. | `engine/tests/test_pipeline_reach.py:777-790 _OI17_FULL_MODULE_ENTRYPOINTS` / `:767 test_oi17_full_module_conversions_are_stub_wired` |

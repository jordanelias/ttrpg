...
> Original path: `systems/characters/reference/characters_flow_skeleton_v1.md`
...
- S7.3 `[emit]` The snapshot becomes `CampaignResult.final_state`. `engine/mc_v18.py:326` (final_state=game_state.serialize_world(world))
...
| `CampaignResult.final_state['convictions'/'beliefs']` | return value | nothing reads these two keys downstream — see §7 gap 2 | `engine/mc_v18.py:334 final_state` |
...
| 3. Code↔contract divergence: `references/module_contracts.yaml`'s `piety_track` entry declares `conviction.py` as a Key-substrate consumer of 9 Key types and an emitter of `state.scar_acquired`, but `conviction.py`/`beliefs.py`/`companion.py` contain no Key subscription, `echo_scheduler`/`TickScheduler` reference, or `.emit(` call anywhere — the module is a plain function-call API. | `references/module_contracts.yaml:395`, `references/module_contracts.yaml:402-413` (`sim_module: .../conviction.py`, `consumes:` 9 entries, `emits: state.scar_acquired`); `grep -rn "scar_acquired\|echo_scheduler\|TickScheduler\|\.emit(" systems/characters/sim/*.py` → no matches. |
| 4. The read (deserialize) direction of the World save/restore round-trip is test-only in production. `serialize_world` (the write direction) is called at the end of every campaign (`engine/mc_v18.py:326`), but `restore_world` has zero production callers — only `engine/tests/test_world_population.py` exercises it. | `grep -rn "restore_world(\|serialize_world(" --include="*.py" .` → `serialize_world` called from `engine/mc_v18.py:326`; `restore_world` called only from `engine/tests/test_world_population.py`. |

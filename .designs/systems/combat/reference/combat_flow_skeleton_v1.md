...
| **`DISPATCH_COMBAT_BRIDGE` defaults OFF** (a default-off flag per §2 architecture rule 4). With it off, `scene_dispatch`'s combat branch calls the DEPRECATED `systems.combat.sim.combat.resolve_combat_round` (its own v30 pool/damage model, distinct from `combat_engine_v1`'s sigma-leverage model) rather than the canonical engine — moot in practice per the gap above, but the flag default is itself a traced absence. | `engine/mc_v18.py:97-108 _dispatch_combat_bridge_on` (`os.environ.get('DISPATCH_COMBAT_BRIDGE', '0') == '1'`) |

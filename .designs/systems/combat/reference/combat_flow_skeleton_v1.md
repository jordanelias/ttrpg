...
> Original path: `systems/combat/reference/combat_flow_skeleton_v1.md`
...
| `systems.combat.sim.combat.resolve_combat_round(participants, scene=None, rng=None)` (DEPRECATED engine) | `systems/combat/sim/combat.py:268 resolve_combat_round` | RETIRED as a consumer 2026-09-25 (ED-900/904, ED-1029): the flag-off branch no longer calls this function or any resolver — see §5 S5.1.3 and §7's gap note. The function itself is untouched, unconsumed by the campaign seam |
...
S5. **Campaign-seam dispatch** (outside `systems/combat/`, traced for the IN-side seam): `scene_dispatch._resolve_slot` on `st == "combat"`. `engine/cross_scale/scene_dispatch.py:226` (st == "combat").
  - S5.1 `[gate][branch]` `if getattr(world, "dispatch_combat_bridge", False):` — flag decided once per campaign by `mc_v18.run_campaign` (default OFF). `engine/cross_scale/scene_dispatch.py:234`, `engine/mc_v18.py:256`.
    - S5.1.1 **ON branch**: `combat_bridge.derive_parties(ctx, world)` → `[gate]` `None` on missing/unresolved `ctx['factions']` → deferred; else `combat_bridge.resolve(parts[0], parts[1], rng)` → **S1 `wrapper.fight`**. `engine/cross_scale/scene_dispatch.py:236-240`.
    - S5.1.2 `[write]` Sets `ctx["echo"]` (`actor_faction`/`target_faction`/`most_relevant_stat="Mil"`/`degree`) keyed off the bridge's `result` int. `engine/cross_scale/scene_dispatch.py:262-269`.
    - S5.1.3 **OFF branch (default) — RETIRED 2026-09-25 (ED-900/904, ED-1029).** Was: `[gate]` no `ctx['participants']` → deferred; else `systems.combat.sim.combat.resolve_combat_round(parts, ...)`, the deprecated engine. Now: a named gap unconditionally — `out["reason"]` states flag-off personal combat has no resolver, since `combat_engine_v1` via the bridge is the only one. `ctx['participants']` never had a producer (measured: `rg` over `engine/`, `tests/`, `tools/`, `systems/` found only the old read). `engine/cross_scale/scene_dispatch.py:269-277`.
  - S5.2a `[branch][gate]` If `world.echo_scheduler` is attached (`ECHO_TRANSPORT` on, default ON): `echo_transport.emit_scene_echo("combat", ...)` is called — reached from **both** S5.1 sub-branches (bridge ON and deprecated-engine OFF/default alike), not gated on which one ran. `engine/cross_scale/scene_dispatch.py:392-394`.
    - S5.2b `[gate]` Inside `emit_scene_echo`: early-returns `{}` if `ctx["echo"]` (only set by S5.1.2, ON-branch only) or its derived `key_type` is absent. `engine/cross_scale/echo_transport.py:384`.
    - S5.2c `[branch][gate][emit]` Past that gate, a third, separate gate on the computed domain echo (`er.fires` plus a resolved faction/stat and a non-zero delta) decides whether a `scene.combat_resolved` **Key** is actually constructed and `sched.emit`ted into the substrate (deferred faction-stat apply at the accounting boundary). `engine/cross_scale/echo_transport.py:412-440`.
  - S5.3 **Never reached in the live campaign today** — see §7: no call site ever queues a `combat`-type scene, so `_resolve_slot`'s `st == "combat"` branch (now a single bridge-or-named-gap branch, not two resolver branches — see S5.1.3) is dead code under the current season-loop trigger set regardless of the flag.
...
| ~~`resolve_combat_round()` return `RoundResult` (deprecated engine)~~ RETIRED 2026-09-25 — the OFF branch now returns a named-gap dict, not this function's result | return value | `scene_dispatch._resolve_slot`'s `out["result"]` (OFF branch) | `engine/cross_scale/scene_dispatch.py:269-277` |
...
| up | `engine/cross_scale/scene_dispatch.py` (IN lane) | dispatches queued `combat`-type scenes to the bridge (flag ON) or, RETIRED 2026-09-25, a named gap (flag OFF, default — was the deprecated `systems.combat.sim.combat`) | `engine/cross_scale/scene_dispatch.py:224-277` |
...
| out | `engine/cross_scale/echo_transport.py` (IN lane) | on the bridge ON-branch + `ECHO_TRANSPORT` on, converts `ctx["echo"]` into a `scene.combat_resolved` Key and a deferred `Faction.adjust` write — **combat_engine_v1 itself never touches this**, the seam is entirely on the IN side | `engine/cross_scale/echo_transport.py:101`, `engine/cross_scale/echo_transport.py:418-440` |
| out | `references/module_contracts.yaml` `personal_combat` module | declares `emits: scene.combat_hit / scene.combat_felled / scene.combat_resolved` as though the engine itself emits Keys — see §7 divergence | `references/module_contracts.yaml:1156-1159` |
...
| Gap | Evidence |
|---|---|
| **`combat_engine_v1` neither emits nor consumes a substrate `Key`.** The `personal_combat` module contract declares `emits: scene.combat_hit / scene.combat_felled / scene.combat_resolved` and `consumes: scene.combat_strike / scene.combat_hit` as the engine's own I/O, but zero `.py` files under `combat_engine_v1/` reference `engine.substrate`, `Key(`, or `KeyLog` — the only grep hits are unrelated prose uses of the word "substrate" (initiative/tradition substrate). The one real `scene.combat_resolved` Key construction site lives entirely outside the subsystem, in `engine/cross_scale/echo_transport.py`. | `grep -rn "substrate\|KeyLog\|Key(" systems/combat/combat_engine_v1/*.py` → no substrate-API hits; `engine/cross_scale/echo_transport.py:418` (key = Key(); emits block at `references/module_contracts.yaml:1156-1159`; consumes block at `references/module_contracts.yaml:1153-1155` |
| **No live trigger ever queues a `combat`-type scene**, so the entire `st == "combat"` branch of `scene_dispatch._resolve_slot` — both the bridge (ON) and the named-gap (OFF, default; RETIRED 2026-09-25, was the deprecated engine) sub-branches — is unreachable from the season loop today, independent of `DISPATCH_COMBAT_BRIDGE`'s value. `evaluate_triggers` only ever fires a `"contest"` scene (Stability Crisis); the repo's only `scene_slate.queue_scene(...)` call site passes `ev["scene_type"]` from that fired-triggers list. | `engine/cross_scale/scene_dispatch.py:77-101 evaluate_triggers` (fires only `scene_type: "contest"`); `engine/cross_scale/scene_dispatch.py:107` (scene_slate.queue_scene(ev["scene_type"], ...)) (the only call site repo-wide — confirmed by `grep -rn "queue_scene(" .` returning just the definition + this one call); module docstrings at `engine/cross_scale/scene_dispatch.py:36-39` and `engine/cross_scale/combat_bridge.py:37-38` both independently assert the same absence |
| **`DISPATCH_COMBAT_BRIDGE` defaults OFF** (a default-off flag per §2 architecture rule 4). With it off, `scene_dispatch`'s combat branch is a named gap (RETIRED 2026-09-25: it used to call the DEPRECATED `systems.combat.sim.combat.resolve_combat_round`, its own v30 pool/damage model; that call was deleted, not repointed, since `ctx['participants']` never had a producer) rather than the canonical engine — moot in practice per the gap above, but the flag default is itself a traced absence. | `engine/mc_v18.py:97-108 _dispatch_combat_bridge_on` (`os.environ.get('DISPATCH_COMBAT_BRIDGE', '0') == '1'`); `engine/cross_scale/scene_dispatch.py:269-277` |

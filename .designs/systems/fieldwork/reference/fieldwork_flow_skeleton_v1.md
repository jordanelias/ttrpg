...
> Original path: `systems/fieldwork/reference/fieldwork_flow_skeleton_v1.md`
...
`module_contracts.yaml` entry — `references/module_contracts.yaml:511`)
...
| `form_knot(actor_a, actor_b, world, actor_a_obj, actor_b_obj, season, rng)` | `systems/fieldwork/sim/knots.py:172 form_knot` | `—` in production (`engine/mc_v18.py` records a `stubwire.stub_resolve` call under the literal name `'form_knot(world-gen|season-tick)'` at `engine/mc_v18.py:223-224` instead of calling this function — see §7). Called only from unit tests: `engine/tests/test_knots_ed912.py:55` |
| `sustain_knot(knot_id, strain_delta, source, world)` | `systems/fieldwork/sim/knots.py:250 sustain_knot` | `systems/threadwork/sim/opposing.py:245` (inside `resolve_opposing_operations`, `systems/threadwork/sim/opposing.py:103`) — but `resolve_opposing_operations` itself has no caller anywhere in the tree (§7); also called directly by unit tests |
...
| `get_knot(knot_id, world)` | `systems/fieldwork/sim/knots.py:382 get_knot` | `—` (no caller found) |
| `get_active_knots(world)` | `systems/fieldwork/sim/knots.py:386 get_active_knots` | `—` (no caller found) |
| `reset_knots(world)` | `systems/fieldwork/sim/knots.py:390 reset_knots` | test helper only: `engine/tests/test_knots_ed912.py:43`, `engine/tests/test_knots_ed912.py:54` |
...
| `world.rng` (fallback: fresh `random.Random()`) | `world-state` | `form_knot`'s rng resolution | `systems/fieldwork/sim/knots.py:218-221` |
...
- S2. `form_knot` — `[gate]` prerequisite chain, each a short-circuit return of `None`:
  ...
  - S2.3 `[gate]` disposition (via `disposition_with_<actor_b>` or fallback `.disposition`)
    `>= KNOT_DISPOSITION_MIN`. `systems/fieldwork/sim/knots.py:189-193`
  - S2.4 `[gate]` at least one actor's `.ts >= KNOT_TS_MIN_PARTY`. `systems/fieldwork/sim/knots.py:195-198`
  - S2.5 `[gate]` `_count_knots_for_actor(actor_a, world) < floor(bonds_a/2)+1`.
    `systems/fieldwork/sim/knots.py:200-203`, count helper `systems/fieldwork/sim/knots.py:167-170`
  - S2.6 `[gate]` no existing active Knot already links `actor_a`/`actor_b`.
    `systems/fieldwork/sim/knots.py:205-211`
  - S2.7 `[loop]`/`[gate]` formation roll via `roll_pool(pool, tn=KNOT_FORMATION_TN, rng=rng)`; net
    ...
    `systems/fieldwork/sim/knots.py:213-234`
  - S2.8 `[write]` on success, construct `Knot(...)` with an incrementing id (`world.knot_id_counter`
    ...
    `systems/fieldwork/sim/knots.py:236-248`
- S3. `sustain_knot` — `[gate]` knot-id/active checks, then `[write]` strain update clamped to the
  ...
  `systems/fieldwork/sim/knots.py:251-273`
- S4. `check_knot_rupture` — `[gate]` knot-id/active/known-trigger checks; `[branch]` Close-tier
  ...
  `[write]` deactivates the knot and logs a rupture. `systems/fieldwork/sim/knots.py:276-313`
- S5. `apply_knot_loss` — `[branch]` on `mode`:
  - S5.1 `mode == 'break'`: `[write]` sets `composure_damage`/`disposition_set_to` on the returned
    ...
    `try/except (ImportError, AttributeError): pass`). `systems/fieldwork/sim/knots.py:339-365`
  - S5.2 `mode == 'rupture'`: `[write]` sets `disposition_set_to`/`coherence_delta`; late-imports
    ...
    try/except guard). `systems/fieldwork/sim/knots.py:367-378`
- S6. `resolve_opposing_operations` (peer module, not fieldwork's own entry point) — `[branch]` on
  ...
  `systems/threadwork/sim/opposing.py:243-256`. This step is unreachable in practice — see §7.
...
| `apply_conviction_scar(...)` call | `lateral` | `systems.characters.sim.conviction` | `systems/fieldwork/sim/knots.py:349-363` |
| `apply_coherence_delta(...)` call | `lateral` | `systems.threadwork.sim.coherence` | `systems/fieldwork/sim/knots.py:374-375` |
...
| `up` | `engine.mc_v18` | season loop records a `stub_resolve` marker literally named `form_knot(world-gen|season-tick)` in place of calling `form_knot` | `engine/mc_v18.py:223-228` |
...
| `lateral` | `systems.threadwork.sim.coherence` | `apply_knot_loss` late-imports `apply_coherence_delta` on rupture | `systems/fieldwork/sim/knots.py:374-376` |
| `lateral` | `systems.characters.sim.conviction` | `apply_knot_loss` late-imports `apply_conviction_scar` on a high-strain Close break | `systems/fieldwork/sim/knots.py:347-363` |

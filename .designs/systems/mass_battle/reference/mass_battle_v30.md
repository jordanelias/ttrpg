...
> Original path: `systems/mass_battle/reference/mass_battle_v30.md`
...
**Morale is continuous in play (ED-1024, Jordan ruling 2026-06-17).** The §B.2 / §A.4 starting Morale values are integers, but Morale is a continuous quantity once play begins: the exhaustion and Size-fraction triggers erode it by fractional amounts and it is compared as a real number (a sub-unit at Morale 0.4 has not yet routed). Live Morale is never rounded to an integer.

**Per-sub-unit Discipline governs formation as well as combat (ED-1026).** A sub-unit's *own* Discipline — not the unit's — governs how well it holds formation while advancing (the movement formation-hold speed gate) and whether it keeps a demanding shape or drifts to Line (the formation-drift check). A low-Discipline sub-unit advances more slowly and loses its formation independently of a disciplined sibling in the same unit.

**Destruction and within-battle recovery (ED-1027).** When a unit is destroyed (Size 0) its rout propagates to every sub-unit, so per-sub-unit consumers see the rout consistently with the derived unit rout. Within-battle Morale recovery (the between-turn recovery, currently a no-op at a 0 rate) applies per sub-unit when enabled, each own-Morale sub-unit recovering toward its own start. (The inter-unit cascade's per-sub-unit-aware Discipline resistance is in §A.12.)
...
**Morale reset between battles (PP-711, MB-03):** Morale resets to its starting formula (general's Command + unit quality modifier, cap 7) at the start of each new battle. Morale degradation does not persist across battle boundaries. Rationale: persistent Morale snowballs toward deterministic rout in long campaigns regardless of player decisions; campaign fatigue is a separate (future) mechanic at faction layer, not unit-level Morale. **Per-sub-unit reset (ED-1025):** the reset applies to each sub-unit as well as the unit — every own-Morale sub-unit returns to its own starting Morale, and because sub-unit and unit rout are *derived* from Morale reaching 0 (a flag only ever set, never spontaneously cleared), the reset also clears the routed/broken flags on the unit and its sub-units, so a unit that broke in one battle reforms for the next. Discipline is **not** reset here (PP-712 persists). A single-sub-unit unit reduces exactly to the unit-level reset.

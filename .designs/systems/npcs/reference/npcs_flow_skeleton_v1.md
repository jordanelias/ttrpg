...
> Original path: `systems/npcs/reference/npcs_flow_skeleton_v1.md`
...
`systems/_architecture/reference/subsystem_flow_skeletons_v1.md` for the format contract. `systems/npcs/`
...
|---|---|---|
| `generate_npc(faction, role, world, territory_id, rng)` | `systems/world/sim/npe.py:237 generate_npc` | `—` (no production call site; see §7 gap 1) |
| `simulate_npc_actions(world)` | `systems/world/sim/npe.py:364 simulate_npc_actions` | `systems/overview/sim/accounting.py:138 simulate_npc_actions` |
| `get_npcs_in_territory(territory_id, world)` | `systems/world/sim/npe.py:414 get_npcs_in_territory` | `—` (no caller found; see §7 gap 4) |
| `reset_npcs(world)` | `systems/world/sim/npe.py:419 reset_npcs` | `—` (test-helper docstring; no caller found) |
...
|---|---|---|---|
| `world` (`World`) | arg | caller-supplied `GameState`/`World` | `systems/world/sim/npe.py:237 generate_npc` |
| `world.territories[tid].accord` / `.prosperity` / `.owner` | world-state | `Territory` fields read for ecology weighting | `systems/world/sim/npe.py:197-219 _ecology_weights` |
| `world.rng` | world-state | campaign RNG, falls back to a fresh `random.Random()` when absent | `systems/world/sim/npe.py:253-256 rng` |
| `faction`, `territory_id` | arg | optional caller overrides on `generate_npc` | `systems/world/sim/npe.py:237-239 generate_npc` |
| `role` | arg | accepted parameter on `generate_npc`, never read in the function body — docstring marks it reserved for future use | `systems/world/sim/npe.py:248-249 generate_npc` |
| `world.npcs` (per-territory NPC lists) | world-state | populated store `simulate_npc_actions` iterates | `systems/world/sim/npe.py:377 store` |
| `world.season` | world-state | stamped onto emitted `NPCAction.season` | `systems/world/sim/npe.py:408 NPCAction` |
...
- **S1** `[branch]` `generate_npc` resolves an `rng` — caller-supplied, else `world.rng`, else a
  fresh `random.Random()`. `systems/world/sim/npe.py:253-256 rng`
- **S2** `[branch]` If `territory_id` is `None`, one is drawn via `rng.choice(world.territories)`.
  `systems/world/sim/npe.py:258-259 territory_id`
- **S3** `_ecology_weights(world, territory_id)` computes prosperity/accord-derived weight
  ...
  `canonical_accord` first. `systems/world/sim/npe.py:197-234 _ecology_weights`
...
- **S4** `[branch]` Tier-1 archetype seed: faction affiliation is the caller override, else a
  weighted roll toward the controlling faction. `systems/world/sim/npe.py:265-268 npc_faction`
- **S5** `[loop]` Per active issue, a base Stance value is nudged by ecology weights.
  `systems/world/sim/npe.py:272-281 ACTIVE_ISSUES`
- **S6** Worldview conviction(s), compromise category, volatility and loyalty are derived from
  `rng` draws and ecology weights. `systems/world/sim/npe.py:284-298 CONVICTIONS`
- **S7** `[branch]` Tier-2 deviation roll (`rng.randint(1, DEVIATION_DIE_MAX)`); above threshold,
  ...
  applies — see §7 gap. `systems/world/sim/npe.py:301-344 dev_roll`
- **S8** `[write]` An `NPC` is constructed and appended to the territory's list in the resolved
  ...
  `systems/world/sim/npe.py:346-360 _next_npc_id_val`
- **S9** `[emit]` `generate_npc` returns the constructed `NPC`. `systems/world/sim/npe.py:361 npc`
- **T1** `simulate_npc_actions` resolves `world.rng` (or a fresh `random.Random()`) and reads the
  NPC store. `systems/world/sim/npe.py:372-377 rng`
- **T2** `[loop]` For every same-territory NPC pair: `[gate]` skip unless worldviews share a
  conviction. `systems/world/sim/npe.py:380-384 npcs`
- **T3** `[gate]` `[loop]` Collect issues where the pair's Stance values are exactly adjacent
  (`abs(diff) == 1`); skip the pair if none. `systems/world/sim/npe.py:386-393 adj_pairs`
- **T4** `[gate]` Roll `d6` against the pair's average Volatility. `systems/world/sim/npe.py:395-396 avg_vol`
- **T5** `[write]` On pass, shift both NPCs' Stance on the first adjacent-issue pair toward each
  other by one step. `systems/world/sim/npe.py:397-404 issue`
- **T6** `[emit]` Append an `NPCAction(action_type='stance_drift', ...)` descriptor.
  `systems/world/sim/npe.py:405-410 NPCAction`
- **T7** `[emit]` Return the accumulated action list (discarded by its accounting caller — see §4).
  `systems/world/sim/npe.py:411 actions` / `systems/overview/sim/accounting.py:138 simulate_npc_actions`
...
|---|---|---|---|
| `NPC` (return value) | arg | caller of `generate_npc` — no in-tree production caller (§7 gap 1) | `systems/world/sim/npe.py:361 npc` |
| `world.npcs[territory_id]` append | write | `world` (persisted via `game_state.serialize_world`/`restore_world`) | `systems/world/sim/npe.py:359-360 store` |
| `world.npc_counter` increment | write | `world` state; read back as `CampaignResult.npcs_generated` telemetry | `systems/world/sim/npe.py:129-131 _next_npc_id` / `engine/mc_v18.py:326 npcs_generated` |
...
| Mutated `NPC.stance` values (in-place) | write | `world.npcs` store (same objects `get_npcs_in_territory` would read) | `systems/world/sim/npe.py:399-404 issue` |
...
| `Territory.accord` / `.prosperity` / `.owner` | R | `engine/autoload/game_state.py` (`Territory`) | `systems/world/sim/npe.py:197-219 _ecology_weights` |
| `World.rng` | R | `engine/autoload/game_state.py` | `systems/world/sim/npe.py:253 rng` / `systems/world/sim/npe.py:372 rng` |
| `World.season` | R | `engine/autoload/game_state.py` | `systems/world/sim/npe.py:408 NPCAction` |
| `_npcs_by_territory` / `_npc_counter` (module fallback) | RW | `systems/world/sim/npe.py` | `systems/world/sim/npe.py:117-118` |
...
|---|---|---|---|
| up | `engine/autoload/game_state` | `World.npcs`/`.npc_counter` registry fields owned by game_state, populated by npe | `engine/autoload/game_state.py:277-278 npcs` |
...
| lateral | `engine/mc_v18` | honest-deferral `stubwire.stub_resolve('generate_npc(world-gen|season-tick)', ...)` fired once per season instead of calling `generate_npc` | `engine/mc_v18.py:213-221 stub_resolve` |
| down | `engine/substrate/canon_buckets` | `_ecology_weights` imports `canonical_accord` as a no-deps leaf | `systems/world/sim/npe.py:45 canonical_accord` |
...
| declared, unimplemented | `systems/factions` (npc_behavior contract's `consumes`/`emits` Key edges: `scene.gossip`, `scene.witness`, `state.opinion_revised`, etc.) | contract declares edges against a `sim_module: none` — see §7 gap 3 | `references/module_contracts.yaml:270 npc_behavior` / `references/module_contracts.yaml:306 npc_behavior` |

## 7. Traced gaps

| Gap | Evidence anchor |
|---|---|
| `generate_npc` has no world-gen or season-tick call site — `engine/mc_v18.py` fires a `stubwire.stub_resolve('generate_npc(world-gen|season-tick)', ...)` in its place every season, and a dedicated test pins `npcs_generated == 0` on a seeded campaign. Only `simulate_npc_actions` (the drift half) is wired. | `engine/mc_v18.py:213-221 stub_resolve` · `engine/tests/test_world_population.py:142-154 test_generate_npc_has_no_automatic_call_site_this_wave` · `engine/mc_v18.py:326 npcs_generated` |

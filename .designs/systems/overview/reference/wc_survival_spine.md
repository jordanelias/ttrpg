...
> Original path: `systems/overview/reference/wc_survival_spine.md`
...
**WC decay clause** (§victory_v30 §6 + npc_behavior §3.4 cross-reference, calibrated 2026-04-24 stress-test): WC decreases when the player performs Continuity-disruptive operations within a season. Trigger: ≥ 2 disruptive ops in the same season → WC −1 (cap −1/season from this source). Single disruptive ops produce Conviction Scar per §3.4 (NPC Scar cap = 1/season per NPC) but do NOT advance WC decay alone — the player is paying the relationship cost via Scar accumulation, not double-paying via WC decay. Disruptive operation types: Dissolution of living being; Lock on being without consent; POP that disrupts substrate-stable configurations; unmitigated Gap creation persisting >2 seasons; Substrate Saturation events involving player practitioner.

**Arc D firing condition** (calibration clarification): WC reaching 0 from this decay path fires Arc D Confrontation **if and only if** the player held WC ≥ 1 in any of the prior 4 seasons (i.e. the player had previously engaged with Wardens). Initial-state WC 0 means Arc A no-engagement, NOT Arc D — Arc D requires a relationship to break. Once Arc D fires from this path: Warden Outpost Outreach ceases; Mending Community recovery suspended for 2 seasons.

**Calibration rationale (ED-759 stress-test):** Prior decay rate (−1 WC per disruptive op, cap −2/season) made Arc E Wager pathway unreachable — a player gaining the requisite Scars ≥ 2 (one per season under §3.4 cap) simultaneously dropped WC by 2, falling below the Wager prerequisite (WC ≥ 2 + Scars ≥ 2). Revised rate (≥2 ops/season → −1 WC, cap −1/season) preserves the trap-state penalty for aggressive Path-4 play while making Path-3 Wager actually reachable from foreseeable Path-2 entry. Wager pathway is thus a strategic option, not a theoretical one.

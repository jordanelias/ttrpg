...
> Original path: `systems/settlements/reference/settlements_flow_skeleton_v1.md`
...
|---|---|---|
| `populate_from_geography(world, path=None)` | `systems/settlements/sim/registry.py:215 populate_from_geography` | `engine/autoload/game_state.py:304 create_world` (by the `world_gen_settlements` role since plan S5a, not imported) |
...
|---|---|---|---|
| `World.settlements` | `RW` | `engine/autoload/game_state.py` (declared), written by `registry.py` | `engine/autoload/game_state.py:290 settlements` |
| `World.territory_infrastructure` | `RW` | `engine/autoload/game_state.py` (declared), written by `infrastructure.py` | `engine/autoload/game_state.py:290 territory_infrastructure` |
| `World.npc_drift_state` | `RW` (declared only — no live writer other than the module fallback; see §7) | `engine/autoload/game_state.py` (declared) / `temperaments.py` (`_drift_store`) | `engine/autoload/game_state.py:291 npc_drift_state` |
...
| `lateral` | `systems/factions/sim/faction_action.py` | Reads `ADJACENCY` for conquest-target and threat-signal derivation | `systems/factions/sim/faction_action.py:42`, `systems/factions/sim/faction_action.py:149`, `systems/factions/sim/faction_action.py:198` |
...
| `restore_world`'s `settlements` branch has no production caller — only exercised by `engine/tests/test_world_population.py`'s explicit serialize/restore round-trip | `engine/autoload/game_state.py:425-427 restore_world`; `engine/tests/test_world_population.py:1-18` |
...
| **The `settlement_layer` contract's `executes:false` flag is correct for the file it names, but that file is the wrong one.** Its declared `sim_module` is `settlement.py`, the orphan this skeleton independently proves has zero importers; the live registry code (`registry.py`) that actually runs at boot and every `loop.s3` has no contract entry of its own. The contract pointing at the dead file while the live file goes uncontracted is the real defect. | `references/module_contracts.yaml:931-934 settlement_layer`; `systems/settlements/sim/settlement.py:95 compute_settlement_state`; `systems/settlements/sim/registry.py:215 populate_from_geography` |

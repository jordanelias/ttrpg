...
> Original path: `systems/threadwork/reference/threadwork_v25_historical.md`
...
Thread Sensitivity score is revealed to the player. Practitioner designation granted. Truth −1 (permanent: the old framework can never feel complete again).
...
## 2.6 Opposing Operations — Contested Intentionality (PP-632)

**Scope:** Two practitioners in Thread contact simultaneously targeting the same configuration with opposing intentionalities. See `references/params_threadwork.md §Opposing Operations` for full tables.

**Opposing operation pairs:** Weave/Pull · Lock/Dissolution · Lock/Pull · Weave/Dissolution.
**Not opposing (compound):** Weave/Lock · Pull/Dissolution.
**Never opposing:** Any operation vs Mending (categorically different targets — Mending targets substrate absence; all other ops target presence). Operations at different scales (different configurations).

### Opposing Engagement Modifier
Each practitioner adds +floor(opponent TPS ÷ 2), minimum +1, to their Ob.
...
### Resolution
Both practitioners roll simultaneously. Outcomes by degree combination:

| Practitioner A | Practitioner B | Outcome |
|---|---|---|
| ≥Ob (Success/Overwhelming) | ≥Ob | Shifting Object — both partially resolve; RS worst +1; both pay Coherence + knot strain |
| ≥Ob | Partial | A resolves at degree; B pays knot strain without degree-table consequences |
| ≥Ob | Failure | A resolves at full degree; B standard Failure |
| Partial | Partial | d6: 1–2 Shifting Object (−1 scale); 3–6 nothing resolves. Both pay Coherence + knot strain |
| Partial | Failure | A Partial resolves; B standard Failure |
| Failure | Failure | Nothing resolves. RS −1 at target. Both pay 1 Composure |

**FR vs FR both-Fail:** RS −(scale value); both +2 Ob next op, 4 Composure. Scale: Personal −2, Relational −3, Territorial −4, Structural −5.

### Consequences

**Knot strain — loser:** +1 Ob next Thread op this scene, 2 Composure.
**Knot strain — winner:** 1 Composure.
**FR loser:** +2 Ob, 4 Composure. If winner Dissolved: +1 Wound (knot tear).
**Tie (both ≥Ob):** Both +1 Ob, 2 Comp. FR tie: +2 Ob, 4 Comp.
**Both fail:** 1 Composure each.
Composure restores at scene change. Ob penalty: next Thread op in scene or scene end.

**Co-movement:** Coherence costs are per-practitioner (§3.2). Epistemic and actual co-movement fire ONCE for the compound event (one disturbance on one thread, not two). P-01: inseparability is a property of the thread being disturbed, not of the practitioners disturbing it.

**Mending immunity:** Mending cannot be directly opposed. Indirect interference: +1 Ob co-movement at site; P-17; territory effects. Ob ceiling 8.

### Sustained Opposition
Each sequential failure to overcome opposition: +1 Ob. If Shifting Object is already present, second opposition advances deterioration: seasons → sessions → scene collapse.

### N-Way Opposition (3+ practitioners)
Automatic lattice collapse. All operations fail. Gap opens at target scale. RS −(2 × n practitioners). All pay +2 Ob, 4 Composure.

### Cross-Mode
**Mass battle:** Highest net wins at degree; loser resolves Partial. Tie: Shifting Object + both Partial. RS ×3 (PP-192). Temporal co-movement not ×3 (PP-226).
**Board Game:** Both succeed → Shifting Object + Co-Movement Card. One succeeds → resolves + Co-Movement Card. Both fail → nothing. No personal consequences at BG scale.
**Hybrid cross-phase:** Personal Phase roll stands. BG rolls at Cascade. TTRPG practitioner uses BG NPC's TPS (or faction Intel ÷ 2, min 1). BG uses TTRPG practitioner's TPS.
...
| 2 | Fractured | −2D to all social rolls. −2D to Recall-based rolls. (PP-234) All Knots at accelerated wrongness (+1 strain per session). Once per scene with Thread operation: Spirit TN 7 Ob 1 or lose 1 round to a dissociative episode. Truth maximum reduced by 1 per Coherence level below 3. **Belief Co-Authorship begins:** Game Master presents the practitioner's shifting perceptual framework as the character's internal voice. Player must rewrite each Belief to reflect the framework in which the categories that structure consciousness are loosening. Roll Fractured Fallout on entering this band. |

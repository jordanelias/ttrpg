...
> Original path: `systems/world/reference/ms_trajectory_v1.md`
...
**Critical-band warning toward MS 0 (PST-1 extension, 2026-05-30).** The MS=0 Post-Calamity transition (peninsular_strain_v30 §6.4: acquisition suspended 3 seasons, Mending doubled, recovery MS→20 within 10 seasons) is the substrate's deepest tip and previously had **no** leading signal — the band-crossing warnings above only cover the 20/40/60 edges. By the same Scheffer/Holling precedent that mandated those signals (the deepest bifurcation is where critical-slowing warning matters most), the **warning window of 12 MS extends to the approach toward MS 0**: while MS ≤ 12 (within 12 of the substrate floor) the early-warning trend is active and intensifies as MS → 0, giving the peninsula a diegetic tearing-signal before Post-Calamity fires. This is a falling-direction signal only; it adds no new threshold and does not alter the MS=0 trigger or its recovery path (no R/loop change — Smooth/legibility consistency with §5.1).

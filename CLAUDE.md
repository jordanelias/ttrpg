...
| `designs/` | System design docs, **being rehomed subsystem-by-subsystem to `systems/`** (ED-IN-0071 P4, §2a). Still here: `scene/` (combat engine, social contest), `provincial/` (mass battle, factions), `territory/`, `personal/`, `factions/`, and `audit/` (audits stay — they go to the by-category `audits/` lifecycle in P5, not `systems/`). |

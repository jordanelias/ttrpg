...
| `designs/` | System design docs, **being rehomed subsystem-by-subsystem to `systems/`** (ED-IN-0071 P4, §2a). Still here: `architecture/` (Key substrate → `systems/_architecture/`), `scene/` (combat engine, social contest), `provincial/` (mass battle, factions), `territory/`, `threadwork/`, `personal/`, `factions/`, `world/`, and `audit/` (audits stay — they go to the by-category `audits/` lifecycle in P5, not `systems/`). |
| `systems/` | Design docs by **subsystem** (ED-IN-0071 P4, RULED §2a: one subsystem = one folder = one ID lane = one CURRENT.md row = one `HANDOFF_<LANE>.md` = one Godot module tree). Each subsystem co-locates its design `.md` at the root + a `sim/` subfolder for its oracle scripts. **P4 slice 1 EXECUTED (2026-07-17):** the three doc-only clean subsystems `npcs/`, `articulation/`, `ui/` (no sim, RULED 1:1) moved from `designs/`; the whole toolchain was taught the new primary. Old `designs/{npcs,articulation,ui}/…` paths alias via `references/restructure_ledger.md`. Remaining subsystems (combat, social_contest, factions, fieldwork, mass_battle, settlements, character/*, threadwork, world, _architecture) + the per-subsystem `sim/` distribution land in later P4 slices. |

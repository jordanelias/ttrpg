...
| `audit/` | The audit-report corpus, organized by write-ownership lane (`references/lane_assignments.yaml`): `lane-a/` (design-content), `lane-b/` (infrastructure), `lane-c/` (simulation/Godot), `other/` (cross-cutting/unowned). Consolidated 2026-07-18 from `designs/audit/`, `tests/audit/`, and scattered locations — but only *standalone* documents; audits embedded in a numbered session-chapter sequence or sitting beside the code they critique were initially left in place, then relocated here 2026-07-19 (ED-IN-0071 P4/P5 continuation, PR #191) when `designs/` was retired — the dated-session sequences now live at `audit/<dated-session>/`. Old `designs/audit/…` paths alias via `references/restructure_ledger.md` (a `designs/audit/ → audit/` dir-prefix row). |

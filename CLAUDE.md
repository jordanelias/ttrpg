...
| `engine/season/` | **LAYER 2 — the game code. RATIFIED 2026-09-05 (ED-IN-0202).** The executable season loop: 11 modules, the registries they read **at runtime** — `rosters`, `verb_table` and `write_matrix`, which `shape.py` opens (`:449`, `:1407`, `:291`) and which are therefore mechanism under §0.05, which is why they live with the code and not with the prose. ⚠ **This said FOUR and named `hole_register.yaml` among them; that was false and is corrected 2026-09-05.** `shape.py` never opens it — its readers are `register.py` and `run_cases.py`, the corpus GRADER. So the register is mechanism for the grader and **reference for the game**, which is why its stale `:NNN` citations are a hand-fix and mint no guard (§0.1 pt 5), `cases/`, `runs/`, and a 184-test suite that CI now runs. ⚠ **IT RUNS AND IT IS NOT YET A GAME.** `requirements.yaml` scores Jordan's nine at 6 `not_met` / 3 `partial` — no scene container, **no roll anywhere in the loop**, 54 of 143 cases unrepresentable, 6 of 32 verbs executing. `register.py --requirements` is the gate; read it before citing this tree as done. ⚠ It carries the SECOND declared `sys.path` seam into `systems/` (`combat_seam.py`, shrink-only, declared in `PATH_SEAM_ALLOWED`). |

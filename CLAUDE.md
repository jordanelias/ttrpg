...
| `designs/` | System design docs, **being rehomed subsystem-by-subsystem to `systems/`** (ED-IN-0071 P4, §2a). Still here: `architecture/` (Key substrate → `systems/_architecture/`), `scene/` (combat engine, social contest), `provincial/` (mass battle, factions), `territory/`, `personal/`, `factions/`, `world/`, and `audit/` (audits stay — they go to the by-category `audits/` lifecycle in P5, not `systems/`). |
| `systems/` | Design docs by **subsystem** (ED-IN-0071 P4, RULED §2a: one subsystem = one folder = one ID lane = one CURRENT.md row = one `HANDOFF_<LANE>.md` = one Godot module tree). Each subsystem co-locates its design `.md` at the root + a `sim/` subfolder for its oracle scripts. **P4 slices EXECUTED (2026-07-17):** slice 1 — the three doc-only clean subsystems `npcs/`, `articulation/`, `ui/` (no sim, RULED 1:1) moved from `designs/`, and the whole toolchain was taught the new primary (`systems/` is now a Python **package**). Slice 2 — **`threadwork/`** (the doc+sim template): `designs/threadwork/` + `sim/thread/` → `systems/threadwork/` + `systems/threadwork/sim/`, imported as `systems.threadwork.sim.*` (was `sim.thread.*`); `ci_co_file_checker` gained a **pure-rename exemption** so relocating a params-bearing `_v30` doc doesn't demand a spurious params co-change. Old `designs/{npcs,articulation,ui,threadwork}/…` + `sim/thread/…` paths alias via `references/restructure_ledger.md`. Remaining subsystems (combat, social_contest, factions, fieldwork, mass_battle, settlements, character/*, world, _architecture) + the per-subsystem `sim/` distribution land in later P4 slices. |
...
| `sim/` | Monte-Carlo / simulation code — the **1:1 Python reference the GDScript port is built from**. The **per-subsystem** sims live here (`personal/`, `provincial/`, `territory/`, `world/`, `peninsular/` — `thread/` moved to `systems/threadwork/sim/` in P4 slice 2, imported as `systems.threadwork.sim.*`, and the rest distribute to `systems/<subsystem>/sim/` in later P4 slices); the engine **core** (`substrate`/`autoload`/`cross_scale`/`mc_v18`) moved to `engine/` (P3 Phase A, 2026-07-16) and is imported as `engine.*`. See `sim/README.md` + `sim/CONVENTIONS.md`, but note those docs understate progress (§7). `sim/README.md` also disambiguates against the confusingly-named `tests/sim/` and `tests/sim_framework/` (neither is this package). |

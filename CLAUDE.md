...
| `engine/` | **The executable model.** A Python package: `substrate/` (Key substrate), `autoload/` (singleton hub), `cross_scale/`, `mc_v18.py` (campaign driver), `engine_params/` (the typed Class-C exports the Godot port ingests), `tests/` (seeded regression + parity, CI job `sim-regression`). Per-subsystem sims depend UPWARD on this core. ⚠️ **CORRECTED 2026-08-20 — the words that stood here, "acyclic, autoload is a leaf", were FALSE and had been for some time.** Measured: **six** non-test modules under `engine/` imported `systems.*` at module level (`mc_v18.py`, `cross_scale/echo_transport.py`, `cross_scale/parliamentary_bridge.py`), and `engine/autoload/game_state.py` imports **seven** subsystems' state classes inside functions — a deferred import hides a cycle from the interpreter, it does not remove it. The concrete cycle: `systems/factions/sim/faction_action.py` imports `engine.autoload.game_state`, which imports `systems.factions.sim.treaty`. The direction is being inverted one seam at a time (plan Act C3; seam 1 landed 2026-08-20, 6 → 5) and **`tests/valoria/test_engine_does_not_import_systems.py` is the ratchet** — an allow-list that can only shrink, which also pins the cycle so this claim cannot be restored while the code contradicts it. Do not re-assert acyclicity until that file is empty. |

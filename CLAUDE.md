...
| `arcs/` | Generated **narrative content** (emergent arcs/scenarios, throughlines, `gm_ref/`), promoted out of `designs/arcs/` to a top-level primary (ED-IN-0071, 2026-07-16) — neither system-mechanics nor world-canon. Its arc **registers** (`arc_register*` tracking clocks/events/factions/territory/threads, formerly `references/arc_register*` + `references/arcs/`) live in `arcs/registers/` — distinct from the process ledgers in top-level `registers/`. Old paths alias via `references/restructure_ledger.md`. |

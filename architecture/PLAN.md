...
**Do.**
1. Create `engine/season/hole_register.yaml`, one row per hole in
   ...
   carry-overs**.
2. Write `engine/season/register.py` with a blocking `--check` that
...
>
> `hole_register.yaml` carries **113 rows** as of 2026-09-05 (`python engine/season/register.py --counts` is the authority) — the rows include the 32 transcribed mechanically from `V2` §VII, the
...
| **N5** | **A register `site:` is never checked to exist.** `H-32` is `assumption` with `site: sitting.judging_set`; no `sitting` symbol exists and `world_q.judging_set` raises (step-7 correction, ED-IN-0203: this row named `Query.judging_set`, which no longer exists — `judging_set` is a `queries/world_q.py` module function, unchanged in behaviour). `R2` tests non-emptiness only. | `grep -n sitting shape.py` → comments only | **`W19`** |

...
## §C.12 · The POSITIVE Godot mapping — what each shape becomes, and the four idioms we reject on purpose

**Added 2026-09-05 at adoption (ED-IN-0202), on Jordan's instruction that Layer 1 "work with Godot
principles while still being bespoke to our project".** §C.8–§C.10 above names what Godot
...
| shape | Godot construct | why this one |
|---|---|---|
| `World` | `RefCounted`, threaded by parameter | already ruled in §C.8. See rejection 1 |
| `Person` · `Site` · `Record` · `Seat` · `Rung` | `RefCounted` data in `core/`, **never `Node`** | see rejection 2 |
| `Tenure` | `RefCounted`, stored on its **subject** (D-10) | one home for one relation; a node-parented edge would give it two |
| `Act` · `Scene` · `Query` | plain functions and frozen records | a Query is a function, never a stored aggregate (D-8) |
| `Event` + the log | an append-only `Array` owned by the driver | see rejection 3 |
| `rosters` · `verb_table` · `write_matrix` · `requirements` | `Resource` (`.tres`) or JSON, **one loader**, twelve invariants at boot | Godot's `_ready` is the natural home for a boot-time loader; a failed invariant must abort the boot, not warn |
| the write gate | one `func` every mutation crosses; typed `Result` | no exceptions in GDScript, so a refusal is a value — which this architecture already required |
...
1. **`World` is NOT an autoload**, though an autoload singleton is the idiomatic Godot answer for
   ...
   (*a decision reads world truth*) and D-46 (*world state behind a global name*) both depend on it
   being **unreachable by name**. Godot cannot make that structural — the grade is MECHANICAL, by
   the two licensed guards and a path scan. Make `World` an autoload and the decision layer's
   isolation is gone with no error anywhere.

2. **Core carriers are NOT `Node`s**, though nodes-in-a-tree is *the* Godot idiom for game objects.
   D-21 forbids a container with a clock: carriers have no behaviour and only the driver calls a
   step. A `Node` arrives with `_process` and `_physics_process` attached, so making a `Site` a Node
   hands every carrier a clock — **the fourth clock D-17 and loader invariant 5 exist to prevent** —
   and it is engaged by default rather than by a call. The scene tree is also a second parent
   hierarchy competing with `contain`, which is D-10's two-homes defect at project scale.

3. **The log is NOT signals**, though signals are how Godot propagates change. `append` asserts
   non-empty `causes`, every cause resolvable in `log ∪ acts ∪ {ROOT}`, and non-decreasing season.
   A Godot signal carries no cause, no ordering guarantee and no receipt, so a signal-carried event
   stream cannot satisfy any of the three, and `causes[]` — the thing that makes the season
   auditable — has nowhere to live. **Signals remain correct at the `port/` boundary**: emit one
   per appended Event *after* the append, for the view layer to consume. The log is the truth; a
   signal is a notification about it, and never the other way round.

4. **The RNG is threaded, never global.** `randi()` and `randf()` read a project-global generator,
   which makes a run unreproducible and would destroy the seeded content hash that is currently the
   only execution artifact this architecture has (§0.2). When R-09's producer is built — *chains of
   events within a scene are probabilistic, not deterministic*, `engine/season/requirements.yaml` —
...
The nine modules (§A.2), the write matrix, the `requires` grammar, the degree ladder read from
`engine/autoload/dice_engine.py`, and the Key substrate the port already ingests
(`engine/engine_params/`) are **project shapes**. Godot neither helps nor hinders them; they port as
plain typed code. Do not let a Godot idiom reshape one — the direction of fit is that the port
serves the model, which is §6's standing port↔oracle rule (ED-1050): *never let a port "correct" its
oracle in-place.*

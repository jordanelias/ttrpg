...
| **top-down** | aggregate → substrate delivery | the GAP: no `scale_transitions §3` rule delivers a provincial/peninsula `da.*`/`state.*`/`env.*` Key down to a personal/scene/settlement consumer | **OPEN (J-1)** — 8 cross-band seams / 15 type-edges (19 raw assessor count) |
| **diagonal** | cross-scale AND cross-family | `da.*` (provincial DA family) → personal `npc_behavior` is both; same A6 gap | **absent — OPEN (J-1)** |

**The single structural finding:** the graph traces **up and across cleanly; down is unwritten.** Bottom-up is the deliberately narrow, capped, queued Domain-Echo channel; top-down has only *presentation* (the Scene Slate reads world state) and *modifier* (board degree → scene Ob) channels — no mechanical Key-delivery. This is the campaign's largest flow hole and is Jordan's to rule (J-1: engine-mediated-exempt vs explicit §3 down-rule). The 8 genuine cross-band seams (15 type-edges; the assessor's raw count is 19, incl. 1 near-lateral) are **contingent on that ruling** — if the substrate stream is declared scale-agnostic, they evaporate and the assessor gains a carve-out (verdict §7.2). The engine already *delivers and applies* down (`key_substrate §4.1` step 4 + scale-agnostic `§4.2`); the true gap is that strategic Keys carry no personal targets + no §3 rule (atlas §2.4 / §7-bis P3).
...
- **3-B · Vertical / scale edges** (`scale_transitions §3`, the eight handoff rules): `Personal→Thread · Personal→Faction (Domain Echo) · Personal→Scene (Contest) · Scene→Faction (Domain Echo) · Thread→Faction · Thread→Mass · Mass→Personal (general duel) · Scene→Mass · Fieldwork↔All`. **All run upward or lateral except `Mass→Personal` (§3.7, the lone down-*handoff*, a narrow duel); §3.9 carries a down-*modifier*. None is top-down Key-*delivery* (the J-1 gap).**
...
|---|---|---|---|---|---|---|
| **1** | **Top-down delivery gap** — no rule delivers a provincial/peninsula Key down to a personal/scene/settlement consumer; the eight named `scale_transitions §3` handoffs carry no strategic→personal Key-delivery (§3.7 runs down as a narrow duel; §3.9 carries a down-modifier). The single largest flow hole; blocks Wave-S/4 module specs. | wiring 3-B; flat-spec E2.4 / F1; verdict §4 + A6×19 + §2-J2; game_flow §9-F1; module_map §1 (`!A6` dashed) | extend `scale_transitions §3` with a down-rule **OR** declare engine-mediated delivery A6-exempt (assessor gains scale-agnostic-stream carve-out + tests) | **P2/P3** (contingent) | ED-1009 (adjudication verdict; `ED-1006` is the extraction finding, not this gap) | **J-1** |

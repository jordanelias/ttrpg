...
<!-- POST-SESSION-B/C NOTE (2026-04-19): Arc 31 and Arc 34 reference systems dissolved after this file was generated. Löwenritter Löwenritter Autonomy (§8.9) replaced by Löwenritter graduated autonomy (4-stage Loyal/Restless/Autonomous/Split) — see designs/architecture/conflict_architecture_proposal.md. intelligence broker covert Network (§8.7) dissolved — Shadow Network phenomena now render at settlement layer (designs/territory/settlement_layer_v30.md §4.7-4.9); Dalla Virke became independent intelligence broker. Arc 31 "Quaestio of Baralta" and Arc 34 "Ehrenwall's Count" retained as pre-dissolution illustrations of emergent structure only — do not treat their mechanical seeds as current canon. -->

---

## Arc 31: "The Quaestio of Baralta"
**Primary mechanics:** Church Influence threshold (§7.2), Grand Debate (§9 / debate_system_redesign_v1), Löwenritter Löwenritter Autonomy (§8.9), intelligence broker covert Network (§8.7), Personal Combat (§8), Mass Battle (mass_battle_v3), Thread → Mass handoff (§11.3)
...
flowchart TD
    ...
    B["Ehrenwall Löwenritter Autonomy = 2\n(CI ≥40 + no Crown action → Counter +1)"]
...
**Why this arc is emergent:** Church Influence rising from Church Mandate (not player action) activates Baralta's unique mechanic; Baralta's move activates Niflhel's opportunism (not directed by any player); the Löwenritter counter increments from Church Influence level (passive threshold, not player decision). Three independent systems produced the crisis before any player acted on it.

**Arc shape:** 3–4 seasons. Season 1: Church Influence threshold + Baralta declaration. Season 2: Grand Debate proceeding + assassination. Season 3: Debate resolution + Templar repositioning. Season 4 (conditional on H2): Martial Law + Mass Battle.
...
| intelligence broker exposed | intelligence broker Intel −1; Olafsson Grievance Marker | §8.7 Assassination mode failure |
...
| Tribunal conviction | Maret removed from play; TK5 effect fires (Church Influence +3) | §8.5 TK track |
...
**Primary mechanics:** Southernmost expedition (§6.3), Forgetting (§6.1), Coherence track (§4.5), Personal combat (§8), Thread operations in Southernmost, Faction consequence on return (§11.3 Domain Echo), Church Influence spike
...
flowchart TD
    ...
    F["CI spike: practitioners with Ceiral knowledge exist\n(§7.2: 'Practitioner-healed configuration at territorial scale' event)\n→ CI +3 (one-time event on return)"]
    ...
    K["Battle outcome\n→ Win: Practitioners survive; CI +1 (confrontation cost)\n→ Lose: Practitioners captured; Tribunal; CI +3"]
...
| Return with Ceiral knowledge | Church Influence +3 | §7.2 one-time event |
...
**Primary mechanics:** Löwenritter Löwenritter Autonomy (§8.9), Martial Law (§8.9), Institutional Pressure track (§7.3), Mass Battle (mass_battle_v3), Seasonal Accounting (§8.12), Grand Debate to remove Martial Law (§9)
...
flowchart TD
    ...
    B["Almud yields Torben\n→ Loyalty Clock: 8 → adapting tier\n→ Ehrenwall Löwenritter Autonomy +1 (Loyalty 5–4)"]
    C["CI ≥40 without Crown action\n→ Ehrenwall Löwenritter Autonomy +1\n(§8.9: passive CI advance + no Crown response)"]
    D["Altonian border incursion\n→ Crown loses 2 territories without Military Domain Action\n→ Ehrenwall Löwenritter Autonomy +1 → total = 3"]
...
**Why this arc is emergent:** Counter increment conditions are independent (Torben departure = Institutional Pressure-driven; Church Influence advance = Church-driven; military loss = Altonian-driven). None requires the other. All three fire from passive threshold mechanics in the same campaign window without player orchestration.

**Arc shape:** 5–6 seasons to fully develop. Counter conditions accumulate across seasons 1–4; Split fires season 5; Martial Law and debate season 5–6; mass battle conditional on season 6+.
...
| Baralta wins Debate to remove Martial Law | Church Influence −1, Löwenritter Stability −1 | §8.4 Unique Action consequence chain |
...
**Primary mechanics:** Klapp Combat Endurance track (§13.2), Thread Sensitivity development (threadwork_v25 §2.3 Discovery), Church Stability (§8.3), Church Influence generation pause (§7.2), Debate (Church Tribunal), Faction play cascade
...
The mechanical cascade is specific: Church Stability drops below 3 at the next accounting. Per §7.2, the Church Influence generation pause activates — Cardinals competing internally suppresses institutional momentum. Church Influence does not rise from Mandate that season. This is the single most effective Church Influence brake in the game other than a successful Grand Debate, and it fires from an Non-Player Character's spiritual experience, not from player action. Olafsson wants Klapp investigated. Himlensendt is caught between doctrine and the reality that his most capable administrator has become something his theology cannot categorise. Two Trajectories: Klapp doubles down (suppression) or fractures (Conversion). If fracture, he becomes the Church's first practitioner in 45 years. If suppression, he drives the Inquisitors with new fervour and Church Influence resumes at +2/season.
...
flowchart TD
    ...
    E["Church Stability drops to ≤3\n→ CI generation pause: §7.2 Church Stability brake\n→ Mandate-based CI not generated this season\n→ MS-driven CI increase continues regardless"]
...
| Church Stability ≤3 | Church Influence generation pause | §7.2 Church Stability brake |
...
|---|---|---|---|
| Arc 31 (Quaestio of Baralta) | Arc 34 (Ehrenwall's Count) | Church Influence level + Crown Mandate erosion | If Baralta loses Grand Debate → Church Influence spike activates Löwenritter Autonomy condition 2 faster |
...
| Arc 34 (Ehrenwall's Count) | Arc 31 | Martial Law + faction ops | Martial Law removes Baralta's ability to invoke Sovereign Authority Doctrine until removed → Church Influence advance unchecked during Martial Law period |
| Arc 35 (Klapp Threshold) | Arc 32 | Church Stability brake + archive access | Church Influence pause from Klapp crisis gives Vaynard's TK advancement window; Klapp covert sharing directly advances TK +1 |
| All arcs | Mending Stability track | Thread ops accumulation | Every successful Territorial+ Thread operation across all arcs: Thread Tension +1–2. Across 5 arcs at 3 operations each: expected Thread Tension rise ~15–20. Mending Stability structural pressure. |
...
*End of Arc batch 31–35. Commit to `designs/gm_ref_cp14/arcs/simulated/arcs_31_35_hybrid_systems.md`. Simulation runs for Arcs 31 and 33 follow in simulation report.*

...
designs/gm_ref_cp14/arcs/simulated/arcs_16_19_faction_domain_echoes.md: ✓ fetched (194 lines)
designs/gm_ref_cp14/arcs/simulated/arcs_20_23_branching.md: ✓ fetched (294 lines)
designs/gm_ref_cp14/arcs/simulated/arcs_24_27_branching.md: ✓ fetched (293 lines)
designs/gm_ref_cp14/arcs/simulated/arcs_28_30_coherence_zero.md: ✓ fetched (229 lines)
designs/gm_ref_cp14/arcs/simulated/arcs_31_35_hybrid_systems.md: ✓ fetched (412 lines)
...
The interdependence arises from three shared mechanical substrates: the Mending Stability (MS) track, the Church Influence (CI), and the Guilds-Niflhel supply chain. Every arc touches at least two of these. The arcs are numbered 36–40 to continue the existing sequence.

---

## Arc 36: "The Tithe That Fed the Enemy"

**Primary mechanics:** Church Wealth recovery +1/season (Prudence Cardinal OPTIMISER — npc_roster §13), Conviction (CV) erosion in over-tithed territories (+1/Year-End — npc_roster §13 BG interface), Restoration Movement (RM) Presence marker placement (params_factions §RM), CI passive advance +1/season (params_factions §CI Passive Advance PP-402)

**Primary Non-Player Characters (NPCs):** Aldric Tormann (Prudence Cardinal), Yrsa Vossen, Sæmund Haelgrund
...
flowchart TD
    ...
    L -->|Yes| M["Parish Revolt fires<br/>(BG Event Card:<br/>Church Wealth −2,<br/>CV −1 all affected,<br/>CI −1)"]
    L -->|No| N["Loop continues.<br/>Prudence Cardinal<br/>cannot stop optimising."]
    M --> O["→ FEEDS Arc 38<br/>(CI −1 weakens<br/>Church institutional<br/>momentum)"]
...
**Primary mechanics:** Invasion Pressure (IP) generation rate −1/season (Laskaris PROTECTIVE — npc_roster §11), Schoenland arms supply −1 unit/season (Solberg STABILITY-SEEKING — npc_roster §12), CI passive advance +1/season (params_factions §CI PP-402), IP starting value 5 (clock_registry), Laskaris Flip at Elske Loyalty ≤ 2 (npc_roster §11 BG Event Card), Solberg recall risk
...
flowchart TD
    ...
    F["Arc 36: Parish Revolt<br/>fires → CI −1"] --> G["Church pressure eases<br/>temporarily"]
    G --> E
...
flowchart TD
    ...
    D -->|Failure| F["Truth −1.<br/>New Belief offered.<br/>Vaynard destabilised<br/>but unaware why."]
...
Strand is the fulcrum. His OVERPERFORMER flaw means Crown's administrative capacity depends on him personally. His flattery vulnerability means he is the easiest Crown figure for outside factions to influence. When the Supply Chain Exposure fires (Arc 37) and the Church opens a Heresy Investigation against the Guilds, Strand's reaction determines whether Crown protects the Guilds (Crown needs their Wealth) or abandons them (Crown needs Church cooperation on CI suppression). Strand's decision is not ideological. It is personal. Whoever acknowledges his competence gets his loyalty. The Church has never acknowledged his competence. Feldhaus has — she has been treating him as a peer for years because the Guilds' trade network depends on Crown administrative cooperation. Strand will protect the Guilds because Feldhaus makes him feel like he belongs.

But Strand's protection of the Guilds costs Crown its Church relationship. CI suppression depends on Crown–Church cooperation (or Baralta's Hafenmark-based suppression). If Crown shields a faction under Heresy Investigation, the Church reads this as institutional defection. CI passive advance continues (+1/season — params_factions §CI PP-402). Baralta's Church Influence (CI) suppression (−1/season while her Mandate ≥ 4 — params_factions §Hafenmark Sovereign Authority Doctrine) becomes the only remaining CI brake. Baralta's political position strengthens because the Crown's administrative champion made a personal decision that weakened Crown's institutional position.
...
flowchart TD
    ...
    L --> M["Church reads this as<br/>institutional defection.<br/>CI cooperation<br/>collapses."]
    M --> N["CI passive advance<br/>+1/season unchecked<br/>(only Baralta's<br/>suppression remains)"]
...
| 3 of 4 | Managed crisis | One stabilising mechanism survives. Almud can act through the surviving channel. CI does not cross Phase Transition unless Arc 36 is among the active 3. |
...
|---|---|---|---|
| Parish Revolt fires (Church Wealth −2, CI −1) | 36 | 38, 40 | CI −1 eases Church pressure temporarily (38). Church seeks Guilds loans (40). |
...
| Guilds seek alliance partner | 40 | 36, 38 | Crown protection of Guilds collapses CI cooperation (36). Guilds funding flows reshape factional military balance (38). |

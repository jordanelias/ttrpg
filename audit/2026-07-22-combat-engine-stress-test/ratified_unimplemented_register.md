...
|---|---|---|---|---|---|
| PC-1 | **Emergent half-sword via `grippable`/ricasso** | audit **G3** + plan **P3** + **JD-3** (2026-07-04) | `elements[].grippable=True` on ricasso zones; `affords_halfsword(w)=(∃ grippable fwd element)∧can_halfsword_thrust(cv,pc)`; retire `HALFSWORD_FORM`/`HALFSWORD_BASE` as behaviour gates; land **byte-identical at {longsword, estoc}** | **✅ BUILT — ED-PC-0014, PR #210** | landed byte-identical at parity ({longsword,estoc}); `geo['halfsword']` de-vestigialised; `test_combat_halfsword_affordance.py` green |
| PC-2 | **Half-sword roster expansion** (which attested ricassos grant a form) | **JD-3** (default: parity only) | after PC-1 lands, flip `grippable=True` for flamberge/greatsword (attested ricassos) → new anti-plate capability | **◑ CAPABILITY SHIPPED, ACTIVATION HELD — ED-PC-0016, PR (this)** | `grippable=True` on both ricassos; `affords_halfsword` now {longsword,estoc,greatsword,flamberge}. **Switch HELD (needs_jordan):** adversarial pass found the naive switch is a net **liability** (base out-fights the half-sword when closed: greatsword @medium 93→50%), and a cheap single-strike conditional is **unsound** (strength-fragile, contradicts the duel). Needs a duel-aware decision. Authored forms + analysis: `pc2_halfsword_expansion.md` |
...
| PC-4 | **ED-PC-0012 — secondary-point `THRUST_AUTH_REF` fix** | ED-PC-0012 (open, "known, documented, load-bearing residual") | scale `DELIVERY['point']` by the candidate's own thrust magnitude (as `CUT_AUTH_REF` does for 'cut'); expected to remove the sabre/scimitar/falchion spurious plate-switch | **✅ BUILT — ED-PC-0017 (executes ED-PC-0012), PR (this)** | `THRUST_AUTH_REF=0.52` (anchored below the weakest native pointer bear_spear 0.53 — corrects the ED's stale 0.37 cite). sabre/scimitar/falchion/podao spurious switch removed (cutters at every tier); two-handed cutters route to earned point / Mordhau; native pointers byte-identical. Roster ripple re-verified; voulge edge-gate residual pinned. ED-PC-0012 → resolved |
| PC-5 | **PHASE-C — reach-class / mass-model / poleaxe-spike recalibration** | tracked accepted-red; deferred pending Jordan target bands | armour-conditional approach stop-hit (spear/yari), poleaxe spike-vs-plate, PoB_frac normalization (heft-ordering) | **◑ PARTIALLY BUILT — ED-PC-0015, PR #210** | **DONE:** the half-sword-liability→win flip + the armour-conditional reach-class decay (spear/yari), via `core.thrust_authority(head_len)` scaling the gap-press (both-heavy LS 38→73%; spear heavy 85→28%; poleaxe heavy still 97%; dagger heavy 72→93%). Three balance invariants updated to grounded shape. **STILL DEFERRED (accepted-red, Phase-C):** poleaxe spike-vs-plate *selection* (`test_gap_game_poleaxe_spikes_plate` — a mass/percussion-authority calibration, untouched by thrust_auth) and PoB_frac/heft-ordering (`test_falsifiable_heft_ordering`) |
...
was a *new bug*, not ratified-unimplemented — but it meant PC-1's byte-identical expansion would propagate a broken
mechanic, so PC-1 and PC-5 had to land together, not PC-1 alone. **RESOLVED (ED-PC-0015, PR #210):** `thrust_authority`
flips the liability to a win via the point-to-hand lever primitive — a short-lever pommel-pressed thrust (dagger/
half-sword) defeats a harness at the gaps; a long reach-thrust (spear) cannot. With PC-5's fix landed, the **PC-2**
roster expansion (flamberge/greatsword ricassos) is now safe to propagate a *winning* capability, not a liability.
...
| FA-2 | **ED-FA-0002 domain_actions home** (M1 workplan next-action) | ledger "RATIFIED 2026-07-05" (PR #81); rescoped by ED-FA-0006 | flip `domain_actions` off `doc:null`; add per-verb `da.*` tag column; rule 3 bucket forks | **NOT DONE** | `references/module_contracts.yaml:505` no tag column; `HANDOFF_FA.md:95` "ED-FA-0002 (open)" |

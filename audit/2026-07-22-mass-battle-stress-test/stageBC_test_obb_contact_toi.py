"""Spatial-model v2 Stage B+C acceptance: the CIRCLE->BOX contact + TOI-halt shape swap.

Stage B (core.contact._find_contacts_standoff) fires contact on obb_front_reach_overlap -- the boxed
engagement surface -- instead of the isotropic circle math.hypot(Δ) <= standoff_from_reach. Stage C
(hierarchy.units.resolve_toi_and_commit) halts each cell's Euclidean advance at that SAME OBB touch
surface, not the circular standoff. Both apply on the FIELD_MOVEMENT path only; the grid oracle
(FIELD_MOVEMENT=0) must not see the new code at all.

This file is the R1 probe the plan (spatial_model_v2_plan.md §1 R1, Stage C) demands:
  (a) head-on contact fires at OBB touch, not before;
  (b) after every committed tick, opposing cells never interpenetrate (bodies disjoint; the reach
      engagement surface overlaps by <= eps -- touch, not drive-through);
...
from mass_battle.geometry import cellbox_from, obb_overlap, obb_front_reach_overlap  # noqa: E402
from mass_battle.engine import build_unit  # noqa: E402
from mass_battle import validators as _val  # noqa: E402

EPS = 1e-6  # touch-not-overlap tolerance on the reach engagement surface (see _reach_pen_shallow)
...
def _reach_pen_shallow(box_a, box_b, eps):
    """True unless the two reach-extended engagement surfaces overlap DEEPLY. If they don't overlap,
    trivially fine. If they do, require the overlap to be shallow: pulling box_b away from box_a by
    `eps` along the centre-to-centre axis must separate them. Uses only the trusted primitive
    (obb_front_reach_overlap) -- no bespoke depth math to get wrong."""
    if not obb_front_reach_overlap(box_a, box_b):
        return True
    dr = box_b.cr - box_a.cr
    dc = box_b.cc - box_a.cc
    mag = math.hypot(dr, dc)
    if mag < 1e-12:
        return False  # coincident centres == deep interpenetration
    ur, uc = dr / mag, dc / mag
    pulled = cellbox_from(box_b.cr + eps * ur, box_b.cc + eps * uc, box_b.heading,
                          reach_front=box_b.reach_front)
    return not obb_front_reach_overlap(box_a, pulled)


class _CommitProbe:
    """Wraps resolve_toi_and_commit: after each REAL commit, records (i) whether any opposing cell
    BODIES interpenetrate (they must never), (ii) the deepest reach-surface overlap seen (must stay
    <= eps -- touch, not drive-through)."""
    def __init__(self):
        self.body_interpenetration = False
        self.deep_reach_overlap = False
        self.commits = 0
    ...
    def __enter__(self):
        def wrapped(a_subs, b_subs):
            ...
            for ba, bb in _cross_side_boxes(ua, ub):
                ...
                if not _reach_pen_shallow(ba, bb, EPS):
                    self.deep_reach_overlap = True
...
def test_head_on_no_interpenetration_no_stall(field_path):
    ...
    tick do opposing bodies interpenetrate nor the reach surface drive through (touch, not overlap)."""
    import random
    random.seed(4242)
    a = build_unit('Line', 3, 'A', 'A', 9)
    b = build_unit('Line', 3, 'B', 'B', 9)
    ...
    assert not probe.deep_reach_overlap, "reach engagement surface drove through (overlap > eps)"
...
def test_cavalry_charge_reaches_contact(field_path):
    ...
    resolve. Cavalry = 'Fast'; the box halt must not stop it short of the enemy body's engagement
    surface."""
    import random
    random.seed(77)
    ...
    assert not probe.deep_reach_overlap, "charge drove the reach surface through"

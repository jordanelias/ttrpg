...
> **✅ RESOLVED 2026-07-29 (ED-MB-0047, plan-v2 I4).** Root cause found and fixed at the primitive.
> There were literally **two token objects for one subsystem**: the curated-core seed entry (keyed
> `'Mass Combat'`, scale `province`, six patterns) and a second token `'Mass Battle'` minted by
> `derive_tokens` from `module_contracts`' `mass_battle` module at scale `mechanic`, whose pattern
> set (`{'Mass\ Battle', 'mass_battle'}`) is a strict **subset** of the seed's. Hits split between
> them; that split *is* the Μ-degree divergence. `add()` dedupes on the exact normalized NAME, so
> it structurally cannot resolve an **alias-spanning** token — re-keying alone just moved the
> duplicate (canonical_sources' `mass_combat` system key then minted `'Mass Combat'` at scale
> `system`; measured). Fix: seed entries may declare `alias_names`, which are pre-seeded into the
> dedupe set, and the entry is re-keyed to the repo's canonical name `Mass Battle`.
> **Measured delta, clean processes, whole token universe: 274 → 273 tokens; removed exactly
> `Mass Combat`; added nothing; exactly one scale change (`Mass Battle` mechanic → province).**
> `province` is the classification that agrees with `registers/mechanics_index.yaml`
> (`mass_battle: scale: provincial`); `mechanic` is not a member of that ontology — it is the flat
> default stamped on every module_contracts module, so it never carried MB-specific information.
> Guards (both mutation-verified, 3/3 mutants killed):
> `tests/valoria/test_vector_audit.py::test_seed_alias_names_are_not_minted_as_separate_tokens`
> (parameterized over the seed table, so any future alias-spanning seed inherits it) and
> `::test_mass_battle_token_has_one_owner_at_the_provincial_scale`.
...
> **⚠ PARTLY CORRECTED 2026-07-29 (ED-MB-0047, plan-v2 I4) — the measurement is right, the
> inference is half wrong.** The `pp = 0` reading is `patch_register_active.yaml`-scoped, and that
> file holds **6 patches total**. Scanning the archives, mass-battle-affecting patches **do**
> exist and did go through the register: **10 of them** — `PP-683, PP-690, PP-703, PP-711, PP-712,
> PP-713` (`patch_register_archive_2026_05_02_a.yaml`, 17 patches) and `PP-195, PP-233, PP-251,
> PP-508` (`patch_register_archive_stage4_promotions_2026_04_25.yaml`, 33 patches). So "MB bypasses
> the patch register **entirely**" is false as history: MB patches were filed and then **archived**
> (see the active register's own header, batches 2026-05-02 and 2026-05-10). What survives is the
> narrower and still-true claim: **current** MB work allocates `ED-MB-NNNN` and no `PP-NNN`, so the
> live register has no MB edges. Whether engine-mechanics changes *should* allocate PPs is a
> **convention ruling, not a defect** — filing retroactive PP entries to make a graph metric
> non-zero would be fabrication. Routed to the Jordan docket rather than executed.
> **Out-of-scope observation, logged not chased (Jordan's 2026-07-29 duplication directive):
> 12 of 17 `deprecated/archives/patches/*.yaml` files do not parse as YAML at all** (duplicate
> keys, missing document starts, unquoted colons). Any tool that reads patch history is silently
> reading ~2 of 17 archives. IN-lane; not filed as an ED here.
...
> **⚠ CORRECTED + RESOLVED 2026-07-29 (ED-MB-0047, plan-v2 E4): it was 1 doc, not 3.** Two of the
> three named files are `*_index.md` — **auto-generated navigational views**
> (`<!-- INDEX — auto-generated by doc_index_gen.py -->`), not design docs. Measured corpus-wide:
> of **79** `*_index.md` files under `systems/ audit/ canon/ godot/ arcs/`, **78 carry no Status
> line**, and the single exception is a hand-written audit index, not a generator artifact. The
> generator emits no Status field at all, so a hand-added one would be clobbered on the next
> regeneration *and* would make a derived view a second currency owner for its parent — the §8
> violation this audit exists to find. That `gen_audit` "independently" agreed is **G7, not
> triangulation**: both instruments count generated views as design docs, which is one blind spot,
> not two witnesses.
> **The genuine miss is `military_layer_v30.md` alone** — its status was real but sat inside an
> HTML comment (`<!-- Status: CANONICAL — approved 2026-04-17 -->`), which `obs_core.STATUS_RE`
> cannot match (it anchors on 0–3 leading `#`). Promoted verbatim to a real `## Status:` line; the
> comment is left as provenance. Nothing was ratified — the 2026-04-17 approval already existed.
> Both MB indexes were **regenerated** through `doc_index_gen.py` (never hand-edited), which also
> cleared two pre-existing staleness bugs in them: retired `designs/provincial/…` canonical paths,
> and a `TC Revision` heading the doc had renamed to `CI Revision` (ED-782).
...
> **✅ RESOLVED 2026-07-29 (ED-MB-0047, plan-v2 E4) — and it was not a status conflict, it was a
> parse artifact.** The head carried **two** `## Status:` lines: `WORKING DESIGN — no appendix
> sections. Read straight through.` (line 8) above `CANONICAL — approved 2026-04-17 (editorial
> batch acceptance)` (line 11). `obs_core.first_status()` — the single owner of status parsing —
> returns the **first** match, so every instrument read the doc as WORKING DESIGN while its own
...
> **No status was flipped and nothing was ratified** — the CANONICAL approval line is byte-unchanged
> and now simply parses. Same repair applied to the `_infill.md` twin. Verified: all four MB
> *design* docs now return `CANONICAL …` from `first_status()`.
...
| F6 | Mass Battle vs Mass Combat: divergent mu-degree and scale class | **RESOLVED 2026-07-29 (ED-MB-0047)** — two token objects for one subsystem; `alias_names` primitive + re-key. Measured 274→273 tokens, one scale change, nothing else moved. 2 guards, 3/3 mutants killed |
| F7 | `pp = 0` — MB work bypasses the patch register entirely | **PARTLY CORRECTED 2026-07-29 (ED-MB-0047)** — 10 MB-affecting PPs exist, archived (the reading is active-register-scoped). Residual (current work files EDs not PPs) is a convention ruling → Jordan docket, not a defect |
...
| F9/F10 | 3 docs without `## Status:`; head is `WORKING DESIGN` | **CORRECTED + RESOLVED 2026-07-29 (ED-MB-0047)** — F9 was 1 doc, not 3 (2 were generated indexes; 78/79 corpus-wide carry no Status — G7 blind spot, not agreement). F10 was a two-`## Status:`-lines parse artifact, not a status conflict; nothing ratified |

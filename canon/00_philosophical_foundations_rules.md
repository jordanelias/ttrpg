...
> **`canon/philosophy/`**, which does not carry a condensed rule set at all.
>
...
> than below.
>
> **This file specifically is recommended for retirement rather than migration.** It was a third
> hand-written restatement of a doctrine already stated twice, and two of the suite's worst drifts
> entered through it: a second unrelated referent for *Providence*, and the claim that what came
> through the tear were threadcut beings. It also restated the Locked Zones epistemically, contradicting
> the substrate-side account. A condensed rule set that changed the claims it condensed is worse than
> none. If a loadable summary is wanted, generate it from `10_constraints.md` plus section headers.
>
...
## A — Core Metaphysical Rules

### A1. Thread Nature (Dual Role)
Threads are simultaneously the constitutive ground of all being AND the condition of possibility for being to be rendered. Every operation on threads alters both facticity and how the world comes to be rendered. There is no intervention that touches one without the other.

### A2. Inseparability Principle
Thread operations move ALL three dimensions (temporal, epistemic, actualized) simultaneously. No operation isolates one dimension. Operations are distinguished by intended target, not by which dimension they "primarily" move. Co-movement of unintended dimensions is metaphysical fact, not design constraint.

### A3. Temporal Dimension
Temporality is the independent foundational axis making rendering possible. Manipulating a thread temporally alters objective factual history. Every operation produces Temporal Disjunction: a gap between the practitioner's retained experience and the altered factual past. Degree varies with magnitude of temporal co-movement.

### A4. Intelligibility Dimension
Intelligibility = epistemic accessibility, NOT ontological constitution. The unintelligible ground (Ein Sof) is not void — it is epistemically inaccessible to human rendering while being infinitely constituted. A thread can be fully constituted yet epistemically inaccessible.

### A5. Ein Sof
The unintelligible ground is infinite positive being that exceeds all attributes and categories. It is the source from which threads are continuously spooled. It cannot be described, categorized, or attributed — not because it is empty but because it exceeds all frames of reference.
...
### A14. Calamity
The Calamity was an over-drawing of the substrate — not a spell gone wrong but a fundamental structural event. What came through the resulting fracture were threadcut beings: entities that radically are without becoming. The locked zones are regions where the rendering cannot stably present what exists.

### A15. Knowledge Transmission
Ontological knowledge (thread-constitution, rendering process) cannot be transmitted as propositional content. The inner tradition preserves it as religious poetry, practiced technique, and relational apprenticeship. The outer world's ontical knowledge (facts, properties) is fundamentally different in kind.
...
| Wrapping | Looser entanglement with consequential connection |
...
## C — Universal Mechanical Principles

### C1. Inseparability Constraint (ALL formats)
Every thread operation MUST produce consequences across all three dimensions. Player specifies primary target; system determines secondary consequences. Secondary effects follow from current dimensional state, not random.

### C2. Temporal Disjunction Cost (ALL formats)
Track cumulative Temporal Disjunction. Low = narrative discomfort. High = mechanical consequences (unreliable memory, past-present mismatch). Natural cost curve prevents unlimited thread manipulation.

...
| Players | Mode | Notes |
|---|---|---|
| 2 | Competitive (asymmetric) | Each controls one major faction; remaining factions run on NPC AI |
| 3–4 | Competitive-cooperative | Each controls one faction; NPC AI runs remainder |
...
**Revolution is always NPC-controlled** regardless of player count. Its Rawlsian framework and movement structure make it more useful as a reactive force than as a direct player instrument.

**Schoenland is always NPC-controlled.** See B8 (NPC AI).
...
- 1× Leader card (NPC leader stats, conviction, deviation cost)
...
- 10× NPC AI cards per NPC faction (Crown, Church, Hafenmark, Varfell, Guilds, Niflhel, Revolution, Schoenland)
...
- Failure: TC +1. Heresy Investigation fires. Baralta Mandate −1.
...
*Intelligence mode:* Roll Intel vs target Intel. Success: learn one hidden faction stat or one NPC active Belief. Overwhelming: learn two.
...
*Against NPC factions:* Roll Influence vs Ob = NPC faction Stability ÷ 2 (round up). Success: NPC faction takes one action aligned with your interest at next accounting. Overwhelming: two actions.
...
- Ongoing: Each season the Investigation is open: target Mandate −1 (cumulative). TC +2.
- Closing: Grand Debate (Priority 4, 5 exchanges). Win: Investigation closed. Lose: Mandate −2, TC +2 additional.
...
| 3 | TC +1 (Vaynard's structural theory destabilises institutions). Succession leverage formalized: +1D Diplomacy with Crown. |
| 4 | Varfell will offer Collection access for Thread education. Declare to Revolution or any practitioner faction. |
| 5 | TC +3. Vaynard understands Galbados's structural nature. Seeks capability. Game-level event: TK 5 triggers [EDITORIAL: Varfell TK 5 consequence to be determined — capability-seeking resolution path]. |

---

## B8: NPC AI

NPC-controlled factions follow deterministic priority trees. At the start of each Planning Phase, the active player to the left of the NPC's assigned seat (or rotate if no assigned seat) draws the top NPC AI card for that faction and executes it.

Each NPC AI card specifies:
...
**Priority 1 — Almud constraint check:** If TC > 45, Crown executes one Diplomacy order attempting to secure Church non-aggression pact. Do not execute military orders this season.

**Priority 2 — Succession:** If IP > 29 and Torben has not been sent (Loyalty Clock not started), Crown executes Diplomacy vs Altonia (Ob 4). Failure: Torben departs next season.
...
**Priority 3 — Alliance response:** If all major factions are at Stability ≥ 5 simultaneously: Schoenland opens naval alliance discussions with Valoria (IP −2 if accepted by Crown + any other faction by Diplomacy).

**Default:** Trade order (Schoenland profits every season regardless of Valorian internal politics).

---

### Löwenritter Coup Trigger and NPC AI

The Löwenritter are a Crown sub-faction at game start. They have no independent orders. If the coup threshold fires, they become an independent NPC faction.

**Coup Tracker:** Private GM/designated-player counter. Starts at 0. Threshold: 5.
...
**IP Thresholds (I-01 through I-05):**
...
| S-03 | **Assassination Attempt.** Niflhel AI executes Assassination order against the current leading faction's leader (highest combined stats) regardless of normal AI priority. |
...
- A Grand Diplomatic Scene was completed this season (Diplomacy + Influence both deployed, all player factions agree): IP frozen, peace treaty possible, game continues.
- Schoenland naval alliance was secured before IP = 75 and remains active: Altonian naval capacity limited; IP −10 immediately; invasion becomes border conflict. Game continues.
...
| Named NPC alive and affiliated with your faction | +2 per NPC |
...
- Crown vs. Church: classic axis tension on TC (highest drama).
- Hafenmark vs. Church: procedural vs. divine authority (TC-focused with Sovereign Authority counterplay).
- Varfell vs. Niflhel: intelligence warfare (low clock drama, high information game).
- Crown vs. Varfell: succession + information asymmetry (balanced; IP becomes primary pressure).

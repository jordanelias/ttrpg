...
| Fieldwork → Any (Register Shift) | When hostile contact interrupts fieldwork: current fieldwork action resolves first (degree applied, Evidence/Exposure updated), then the new register (Combat, Contest, Mass Battle) begins. When fieldwork resolves to a Finding with faction-level scope: Domain Echo fires at scene end per §11.5. When a Strategic Phase order (Hybrid) creates a fieldwork opportunity: GM grants one bonus Personal Phase scene (replaces, does not extend, the seasonal cap). Fieldwork ↔ Combat: Exposure at Watched when interrupted → interrupting party gains +1D on first exchange Offence. Fieldwork ↔ Mass Battle: fieldwork suspends at battle start; Evidence Tracks freeze; resumes from frozen state after battle. Fieldwork ↔ Contest: current Disposition maps to Conviction Track offset (±1 per 2 Disposition, cap ±2); Evidence cited in Contest = +2D Recall bonus; Evidence not consumed. See fieldwork_design_v1.md §2.3 for full six-direction handoff rules. (PP-628) |

...
> This section formalizes existing ruled canon and doubles as engine_clock's home doc, retiring its doc:null/[ASSUMPTION] grade. It ratifies nothing new.
...
**Named behavior — the level-triggered perpetual-scene generator (NOT a hidden bug).** A level-triggered Sufficient Scope condition combined with a persistent/indelible Key can drive a scene EVERY tick indefinitely. Concretely (verified in code): Stability Crisis fires whenever `Sta <= 2`, re-evaluated FRESH every tick (level-triggered, not edge-triggered); a persistent Key that holds `Sta <= 2` therefore regenerates a scene every single tick. This is per-tick-terminating (each tick's scene resolves and halts) but never cross-tick-convergent. **This is flagged as a named, understood behavior, not a bug the spec hides.** It requires a ledger-tracked Jordan RULING: is a perpetual level-triggered scene INTENDED pressure-mechanic (a crisis that genuinely persists until the underlying stat recovers), or does it need an edge-trigger / hysteresis / cooldown fix? This section does NOT invent that ruling (OF-PERPETUAL).
...
4. **OF-PERPETUAL** — is the level-triggered perpetual-scene behavior (Stability Crisis-class conditions) an intended pressure mechanic or does it need hysteresis/edge-triggering?
5. **OF-CAP** — CASCADE_DEPTH_MAX / EMISSIONS_PER_TICK_MAX numeric values, from calibration.
6. **ORD-3 / ORD-4 (§1, O.4/O.5/O.6/O.7)** — compute_observers order-preservation and scene_slate._queue's module-global fix. Both are determinism preconditions this spec's own guarantee depends on but cannot itself rule (out of authority to edit key_substrate_v30.md / decide sim/ hygiene scope).
7. **OF-1, OF-2, OF-4, OF-5, OF-6 (§2)** and **the D.4/D.5 residuals (§3)** — smaller, non-blocking authoring/registry decisions.

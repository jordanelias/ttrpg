#!/usr/bin/env python3
"""
Phase B Stage 8b — Stage 10 sim battery WITH trigger 9 (cluster detection).
...
def matches_trigger(key):
    ...
    # Trigger 9 (PP-688 §3.1 candidate, integration plan D10 deferral resolution)
...
def run_simulation(seasons=30, verbose=False):
    ...
    for season in range(1, seasons + 1):
        ...
        # 8.5 Trigger 9 — cross-faction cascade clustering detection (PP-688 §3.1 9th trigger)
        # Condition: pairwise cosine sim of fidelity histories crosses ±0.7 sustained ≥ 4 seasons
        if season >= 4:
            fids_list = list(factions.keys())
            for ai, fa in enumerate(fids_list):
                for fb in fids_list[ai+1:]:
                    hist_a = cascade_fidelity_history[fa][-4:]
                    hist_b = cascade_fidelity_history[fb][-4:]
                    if len(hist_a) >= 4 and len(hist_b) >= 4:
                        sim = cosine_sim(hist_a, hist_b)
                        # Track sustained crossings
                        key_id = f"cluster:{fa}:{fb}"
                        if not hasattr(run_simulation, '_cluster_state'):
                            run_simulation._cluster_state = {}
                        st = run_simulation._cluster_state.get(key_id, {'last_sign': 0, 'streak': 0, 'fired': set()})
                        if abs(sim) > 0.7:
                            sign = 1 if sim > 0 else -1
                            if sign == st['last_sign']:
                                st['streak'] += 1
                            else:
                                st['streak'] = 1
                                st['last_sign'] = sign
                            # Fire once per cluster transition (sustained ≥ 4 seasons + not yet fired this regime)
                            regime_key = (fa, fb, sign)
                            if st['streak'] >= 4 and regime_key not in st['fired']:
                                emit_key('meta.cascade_cluster_event', 'system',
                                         [fa, fb],
                                         {'cluster_pair': [fa, fb],
                                          'similarity': sim,
                                          'cluster_type': 'aligned' if sign > 0 else 'anti_aligned',
                                          'sustained_seasons': st['streak']},
                                         season,
                                         scale='peninsular' if abs(sim) > 0.95 else 'territorial')
                                st['fired'].add(regime_key)
                        else:
                            st['streak'] = 0
                        run_simulation._cluster_state[key_id] = st
...
if __name__ == '__main__':
    print(f"Stage 8b Sim — PP-686 v2 + PP-687 + PP-688 + trigger 9 (cluster detection) over 30 seasons")

...
1. **Bare-stat faction dice are statistically degenerate** (ER-1 / diagnostic F1/F3). A Military-2 faction rolling 2D succeeds at a Domain Action with P ≈ 0.070 (Ob 3) or 0.010 (Ob 4). This is a **resolver-design** problem at one scale — fixable by aggregating the pool (as mass battle already does, §3.5/ER-9) or going deterministic+stochastic (as GD-2 already mandates and CK3/EU/KoDP exemplify). It does **not** implicate the engine elsewhere.
...
|---|---|---|
| F1 — bare-stat NPC Domain pool degenerate | ER-1 — bare 1–7D pool degenerate where stakes highest | **Same finding.** Corrected number: 0.070 @ Ob 3. Fix: aggregate (ER-9 model) or deterministic+stochastic (GD-2). |
...
|---|---|---|---|---|
| ER-1 / F1 | Bare-stat faction pool degenerate (0.070 @ Ob 3) | P2 | Aggregate (ER-9) or deterministic+stochastic (GD-2) — **resolver** | open; Jordan call §7 |
...
**Distribution after reconciliation: 0 P1 (ER-2 closed), 6 P2, 5 P3. No paradigm finding.** The verdict is *more* firmly repair-and-keep than v1, because its only P1 dissolved into a one-line fix.

---

## §7 — Jordan's Calls (sharpened)

1. **Faction resolver (the one real change).** Aggregate the pool (mass-battle ER-9 model: sum relevant unit/derived values so faction actions roll healthy pools) **or** go deterministic+stochastic (GD-2 / CK3-style capped-% + weighted events). *Input:* ER-1 degeneracy is now quantified (0.070 @ Ob 3); the project's own GD-2 and mass-battle layer already point to both fixes. This is a resolver swap at one scale, not an engine replacement.

...
**06-22 loose-ends sweep (post-v4 reconciliation; live-HEAD verified):** a past-month sweep against ledger@HEAD (707 entries, max **ED-1038**) + 3 active handoffs reconciled the 06-11→06-22 window into this register.
- *Resolved since v4 — docket rows below are stale-open, annotated:* **J-1** top-down delivery → **ED-1038** (`scale_transitions §12` all-directions down-rule, 06-19); **J-22** GD-1 → **ED-1031** (all-15, 06-19 → unblocks J-11/LC-7); **J-2** substantially → ED-935 (7 types)+ED-936 (consumers)+ED-937 (remainders → rows #29–32; residual = Jordan payload/magnitude confirms); voice-canon closeout (J-3) → **ED-1030** (PROVISIONAL→CANONICAL); combat docket D1–D9 → **ED-1029** (ratified adopt-all); per-subunit mass-battle batch → ED-1016/1017/1023–1028 (canonized into mass_battle_v30). *[J-1 & J-22 are decision-class — both filed `resolved`/`ratified` in the 06-19 reconciliation session; confirm Jordan ruling if not already explicit.]*
- *Added as live residual (rows #34–#38, J-35):* mass-battle orphaning/ED-907/secondaries (#34→J-9); combat 06-19 residual + 6 blocking decisions (#35→J-33); player-progression A/B′ fork (#36→J-35); ci_political empty-fetch (#37→Lane B); resolution-diagnostic mu-shift variant (#38).
- *Verified still-open (no change):* ED-911, ED-970/971, ED-1032/1033/1036, ED-864/879, ED-907(prov), ED-869, ED-1009 (whole-graph R-FAIL). *Not added (out of register scope):* 'NERS audit' phrase routing + audit bare-word-rule extension (skill-governance); arcs 56–60 uncommitted (creative-tier); Claude Code/ultracode migration (workflow). `[as_of ledger@HEAD this session; SELF-AUTHORED sweep — additive tracking only, no authored values overwritten, Jordan-vetoable]`
...
| 7 | ✅ **RESOLVED 06-19 → all-15 (ED-1031)** · GD-1 victory threshold — canon/02 + mechanics_index vs victory_v30 §0+§11 (byte-confirmed twice); a 12-territory hold wins under canon/02 and not under victory_v30. Gates victory finalization. | **P1** | flatten C1 (SURVIVES-STRENGTHENED) | **J-22** → gates J-11/LC-7 |
...
| 17 | **Personal combat residual** — engine CANONICAL (ED-900–904) but: 3 post-ED-904 unratified commits (incl. `deb405b944` wound −1D, −18pp first wound); **F1 coverage gap** (ranged/group/thread live only in superseded combat_v30 layer; ED-911 staged; **see #35 + loose_ends_register.md — D1-D9 ratified ED-1029 + 6 Jordan-blocking decisions**); F2 tradition imbalance + dead abilities; F3/F4 doc↔engine seams. | **P1** (F1) | combat analysis VERDICT/F1–F7 | **J-33** (D-α…δ parallel-owned — coordinate) |
| 18 | **Mass battle** — COMPLIANT-WITH-BACKLOG; debt = tri-strata doc drift (ED-899 FOLLOW-UP) + F1 (ED-800 implemented nowhere) + F2 (ED-910 control surface absent) + band/CAP/discipline set; ED-970/971 **now filed open**; **see #34 + massbattle_loose_ends_resolution.md — ED-1032 orphaning + ED-907 ratify + per-subunit secondaries**. | P2 | massbattle analysis §0/§7; ners verdict | **J-9** → LA-8 + LC-8 |
...
| 34 | **Mass-battle post-v4 residual (2026-06-20)** — **ED-1032** formation-drift cell orphaning: on drift, cells not re-keyed → ~42% of a drifted unit's troops spatially invisible+inert (no frontage, never bleed) while HP exact; re-key fix **STAGED+validated** (orphan→0, HP preserved) — alters drifting-unit dynamics → needs Jordan adoption + engine re-pin. Tri-strata propagation (live pool `Command×(1+cohesion)`, ED-899/1013, vs legacy `min(Size,Cmd)+Cmd`) **STAGED-ready**. **ED-907** FM three-level command **[provisional]** — 06-20 Jordan FM restatement = the fidelity sign-off it awaited → recommend flip prov→resolved; confirm unbuilt layers (Hold-shape/Adapt, allocation grid, intervention windows) deferred-UI not gaps. **Per-subunit SECONDARIES** (ED-1016/1017 primary shipped): per-subunit rout; fine troop taxonomy onto per-subunit stats — await go/veto. ED-970/971 **now filed open** (were 'staged' #18). Register: `massbattle_loose_ends_resolution.md`. | P2/P3 | massbattle_loose_ends 2026-06-20; ED-1032/907/970/971/1016/1017 | **J-9** (+Lane C orphaning re-pin) |
| 35 | **Personal-combat post-v4 residual (2026-06-19)** — combat decision docket D1–D9 **ratified adopt-all** (**ED-1029**); implementation pending (D1/D3/D4/D6/D7 wiring + D5 + D9/ED-911). 26-item consolidated register (`loose_ends_register.md`) splits the rest; **6 Jordan-blocking decisions**: D2 puncture magnitude · wound model **N+magnitude** (Health=WI×N) · ED-879 G8 · ED-864 scene-C4 direction · D8 new-traditions authorship · F7 crit-bar. PoB/F5 reshape: **D-A** damage-scale reshape/uncap (**BLOCKING** — needs lethality target; overlaps Stage 3) gates **D-B** (2b magnitude, wired+validated, blocked on D-A). | **P1** (D-A, ED-911) | loose_ends_register 2026-06-19; ED-1029/911/879/864; combat reconciliation handoff 2026-06-18 | **J-33** (D-α…δ + D-A/D-B — coordinate) |
| 36 | **Player-progression / growth fork** — **Branch A** (no stat-raising; growth = Standing + Renown + horizontal traditions; maximally canon-aligned, zero apparatus) **vs Branch B′** (each Standing rank grants one Duty-driven attribute-deepening pick; rank/generation-capped; GD-1 held by 1–7 ceiling + Echo caps). Surfaced by the 06-16 progression work, which flags it brushing **canon/00 A11** (canon deliberately has no stat-raising) → creative/structural tier, Jordan's call. v2 merge already resolved D1 currency-proliferation (adopted canonical Standing/Renown/Resources/Convictions/Scars; cut 2 redundant currencies). | **P1-class decision** | player_agency_v30 v2 (2026-06-16) | **J-35** |
| 37 | **`ci_political_v30.md` empty on tracked fetch** — raw file on main = 26286 bytes BUT tracked `read_files_graphql` returns 0 (index-routes → `_index.md`) → **read-routing defect**, content exists; CI is the **central pacing valve** (CI≥60→IP+2; Seizure 60; `((CI−60)/40)^3.3`). Surfaced 06-18/06-19 (arcs fetch fell back to clocks.md/stats/faction_layer §2.7). Ties row #6 (CI-clock overload) + #26 (CI threshold tri-scheme). | P2 (canon-access) | arcs 2026-06-18 flag; live read 2026-06-22 | **Lane B** (rebuild/repoint) → ties J-29 |
| 38 | **`valoria-resolution-diagnostic` mu-shift variant** — skill names continuous-engine μ/σ + probit remapping but never operationalizes (Phase 3a dice-only). 06-20 variant build found F1–F8; correction (restrict P(success)/Ob duality, promote degree-divergence to the useful lever, fix de-swing line, varying-N + Ob-floor caveats, numeric instance, μ_net disambiguation, date/anchor fixes) **recommended, NOT committed**. Skill-maintenance, not game canon. | P3 (tooling) | mu-shift-variant session 2026-06-20 (F1–F8) | **Lane B / skill-maintenance** |
...
|---|---|---|
| **J-2** | ◐ **SUBSTANTIALLY RESOLVED** (ED-935 7 types + ED-936 consumers + ED-937 remainders → rows #29–32; residual = Jordan payload/magnitude). Registry §10 type set: 7 F2 register-or-strike (incl. doc-12 §8 / substrate §8.4–8.6 side), F3 `scene.combat_resolved`, `battle_concluded` naming, `draft_da` strike, `belief_revised`/`scene.dialogue` attribution | LA-2 → every dice-engine port (LC-4) |
| **J-1** | ✅ **RESOLVED 06-19** (ED-1038 — `scale_transitions §12` all-directions down-rule; *verify ruling authority*). Original — Top-down delivery: **(a)** declare engine-mediated delivery canonical + A6-exempt (assessor carve-out + tests; **strengthened** — §4.1/§4.2 already deliver+apply; option adds authoring discipline: strategic emitters populate personal targets where intended, + §3 documentation) **or (b)** author an explicit §3 down-rule | 8 seams' status; assessor fix (LB); `scale_transitions` edit (LA); strategy flag (LC); Wave-S/4 specs |
| **J-22** | ✅ **RESOLVED 06-19 → all-15** (ED-1031; unblocks J-11/LC-7; *verify ruling authority*) | victory finalization (J-11 → LC-7); canon/02 ↔ victory_v30 ↔ mechanics_index one-commit propagation |
...
| **J-11** | **UNBLOCKED** (J-22 resolved 06-19) · Victory: treaty wiring go + territory fallback | LC-7 |

| **J-35** | **Player-progression fork**: Branch A (no stat-raising; rank/authority/access growth) vs B′ (Standing-rank-gated Duty-driven attribute-deepening pick, generation-capped). Brushes canon/00 A11 (fixed-metaphysics: no stat-raising). | any progression port; player_agency growth model |

...
- **LA-23** (new) — **ledger-status reconciliation for the recent rulings** (the §0 LEDGER-FLIP/LEDGER-GAP notes): (a) flip ED-841/842/912/914 `open` → resolved/ratified now that the 2026-06-28 prose ruling is canon; (b) file the missing ED-938/ED-939 entries (or repoint the #13 references) so the J-31 citations resolve. Lane A ONLY (structural ledger ops, reserved-range discipline). Each fix also subtracts from the report-only `validate_ed_citations` count (LB-23). | gated: none (editorial-tier; Jordan-vetoable). |

...
| Stamina | 5–35, `(3×End)+(2×Spirit)` | Combat action-economy counter | Concentration (3×Focus+2×Spirit) — same linear-combination shape | None — the two resources deliberately share a shape (attention/energy) with different attribute weights; not redundant, they gate different things (physical actions vs. reading/composure). |
...
under Mode A/gap register, not duplicated here. **No action** — number systems within personal_combat
are internally coherent; the bounded-attribute vs. continuous-physics split is by design.

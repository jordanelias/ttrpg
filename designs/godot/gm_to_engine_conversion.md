...
> resolve resolvers/schemas via `references/module_contracts.yaml`. Tracked by ED-1054.
...
|--------|-------------|--------------------------|
| Contest §2 Step 7 | "GM records on hidden ledger" | Engine tracks all contest state internally. Piety Track, Composure, Concentration, and Resonant Style targeting are engine-managed. Player sees: current track position, their own Composure, opponent's visible state. Hidden: opponent's exact Composure, their targeting declaration before reveal. |
...
| Social Contest §2 Step 4 | "GM narrates partial outcome proportional to final position" | **Outcome table indexed by final Piety Track position:** Track 4 = slight compromise toward A (A gets 60% of their demand, B gets 40%). Track 5 = true middle (50/50 split). Track 6 = slight compromise toward B. Track 3 or 7 = Decisive. Implementation: each contest defines an outcome table with position-indexed results. Generic fallback: Disposition ±1 for slight, ±2 for Decisive. |

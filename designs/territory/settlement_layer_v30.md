...
- Only currently-controlled territories count; gaining/losing territory recomputes the mean (lose a high-L territory → Mandate falls). The N=1 case (a faction holding one territory, e.g. Church) computes normally: Mandate = round(0.5·L + 0.5·PS) of that territory. A faction controlling **zero** territories has Mandate 0 (the empty-set mean is defined as 0) — consistent with emergent or embedded factions (Restoration; Löwenritter pre-coup) that hold no Mandate until they control territory (`faction_state_authoring §8` seeds them at L/PS 0). **Aggregation note:** the mean is a *plain* mean. The sibling territory→faction rollup — Public Temperament (`faction_canon §3.4`) — is described as population-weighted, but per-territory population is not a numeric tracked stat (it is categorical at settlement level), so **both** temperament and Mandate use a plain mean in practice; population-weighting is a deferred refinement for both once per-territory population is modeled (LPS-3).

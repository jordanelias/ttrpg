...
- Only currently-controlled territories count; gaining/losing territory recomputes the mean (lose a high-L territory → Mandate falls). The N=1 case (a faction holding one territory, e.g. Church) computes normally: Mandate = round(0.5·L + 0.5·PS) of that territory. A faction controlling **zero** territories has Mandate 0 (the empty-set mean is defined as 0) — two cases differ: an **embedded** faction (Löwenritter pre-coup, acting through its host) has Mandate N/A until it controls territory; **Restoration** is territoryless but operates at the **community** level via Presence markers, so its L/PS live in its Presence localities (per `faction_state_authoring §6` / PP-460 — "L+PS track community-level acceptance rather than peninsula-wide") and its aggregate is taken over those localities, not controlled territories (its Community Weaving PS-prerequisite reads community-level). **Aggregation note:** the mean is a *plain* mean. The sibling territory→faction rollup — Public Temperament (`faction_canon §3.4`) — is described as population-weighted, but per-territory population is not a numeric tracked stat (it is categorical at settlement level), so **both** temperament and Mandate use a plain mean in practice; population-weighting is a deferred refinement for both once per-territory population is modeled (LPS-3).

...
- P-01 (inseparability): Mustering affects Prosperity, which affects Thread Tension contribution calculations at high Thread Tension.
...
| Legendary (stat at 5+) | 4 | Campaign-defining figure; Game Master introduces via narrative |
| Practitioner officer | 3 + Thread Sensitivity ≥ 30 requirement | Extremely rare; may not exist at low Thread Tension |
...
**Unit composition (optional layer — Game Master may use simplified stat blocks for minor engagements):**
...
| Practitioner attached | +1D to Cohesion near Thread events; can perform Thread ops during battle | Thread Sensitivity 30+ officer assigned |
...
- Brutal disposition: requires Cohesion 4+ (troops must be disciplined enough to commit atrocities on command without breaking). Brutal against a civilian population: automatic +1 Theocracy Counter if Church observes.
...
- Commanded by the Cardinal of Fortitude (Non-Player Character) or a Player Character Templar knight if one exists.

**Deployment authorization:**
- Templars deploy only on Church orders (Cardinal of Fortitude Domain Action).
- Deployment without ducal/Crown authorization: Theocracy Counter +2 per deployment.
- Deployment WITH authorization: Theocracy Counter +0, but Crown Mandate −1 (appearing to submit to Church military direction).
...
- Templar officers have the Devout Constraint (cannot develop Thread Sensitivity). If a Templar officer somehow loses the Devout Constraint, they are expelled from the order (automatic defection to whichever faction the Game Master determines is appropriate).
...
- Templars ordered to destroy an Einhir site: automatic success (they are trained for this). Thread Tension +3. Theocracy Counter +1 (zealotry visible to population).

**Heresy enforcement:**
- Templars may be deployed to enforce a Heresy conviction: arrest a target. Roll Templar unit Martial vs target's faction Military ÷ 2. Success: target arrested. Failure: target escapes; Theocracy Counter +1 (public spectacle of Church failure).
...
Each Player Character may declare faction affiliation (or none) at character creation. Membership provides:
...
| Cost | Mechanic |
|------|----------|
| Obligation | Game Master may call one faction-obligation scene per season (deliver a message, attend a meeting, vote a certain way). Refusing: Reputation −1 with faction. |
...
2. At least 3 named NPCs with Loyalty 5+ to the founding Player Character (the nucleus of institutional capacity).
...
**Founding procedure:**
1. Player Character declares faction publicly (or covertly — covert factions have Influence but no Mandate until revealed).
2. Set starting attributes:

| Attribute | Starting Value |
|-----------|---------------|
| Mandate | 1 (or 0 if covert) |
| Influence | = founding Player Character's Circles ÷ 2 (round up; min 1) |
| Wealth | = founding Player Character's Resources ÷ 2 (round up; min 1) |
| Military | 0 (must be mustered) |
| Intelligence | = founding Player Character's highest relevant History ÷ 2 (round up; min 1) |
...
## G-043: Non-Player Character Faction Elevation to Player Character Status

### Problem
When a significant Non-Player Character or Non-Player Character-led group becomes important enough to warrant Player Character-level control, there's no procedure.

### Design

**Triggers for elevation:**
- An Non-Player Character faction reaches Influence 3+ and Stability 3+ (institutional viability).
- A player's character dies and the player wants to take over an existing Non-Player Character faction leader.
- The Game Master determines an Non-Player Character faction has become a significant campaign actor.

**Procedure:**
1. Game Master and player agree on elevation.
2. Non-Player Character faction's attributes transfer unchanged.
3. The Non-Player Character faction leader becomes a full Player Character with: existing stats (if established), or stats generated using standard character creation if only narrative stats existed.
4. The Non-Player Character faction leader's Beliefs are rewritten by the new controlling player (reflecting the shift from Game Master-driven to player-driven motivation).
5. Existing Loyalty scores of officers toward the Non-Player Character-leader transfer.
6. The faction's ethical framework tendency remains (institutional inertia) but the new Player Character leader may deviate from it (leader ≠ institution, per editorial decision).

**Constraint:** Elevation is a one-way door. Once a faction has a Player Character leader, it cannot revert to Non-Player Character status without the Player Character dying, retiring, or being deposed.
...
**Alliance formation:**
1. Both faction leaders (Player Character or Non-Player Character) agree to terms in a scene (social roll not required for voluntary agreement; required if one party is reluctant).
...
- Non-Player Character factions: Game Master rolls Loyalty-equivalent check each season to determine if they honor the treaty. Ob = number of seasons remaining. At Ob 4+: Non-Player Character factions become unreliable.
...
| Religious mandate | Church authorizes action against heretics | Attacker: Mandate +1 if Church Mandate > 5. Theocracy Counter +1. |
...
2. Game Master assesses casus belli category.
...
- Previous controller's Circles contacts in territory: burned (Network Damage per G-054 for any Player Character with contacts there).
...
**Altonian presence by Institutional Pressure range:**

| Institutional Pressure Range | Presence | Mechanical Expression |
|----------|----------|----------------------|
| 0–29 (Dormant) | Trade delegations, cultural exchange | Schoenland trade active (+1 Wealth to coastal territories). Altonian spies present but inactive (Game Master layer only). |
| 30–44 (Aggressive) | Economic pressure, diplomatic demands | Schoenland trade conditional (requires Influence Ob 2 to maintain). Altonian ambassador makes demands: 1 per season, escalating. Refusal: Institutional Pressure +1. Compliance: Crown Mandate −1 (appearing weak). |
...
| 60–74 (Warlike) | Invasion preparation visible | Altonian army assembles at Border Pass (visible to any faction with Intelligence 4+). Diplomatic Resolution becomes possible if conditions met. All Valorian factions receive Institutional Pressure notification at seasonal accounting. |
...
- **Diplomacy:** At Institutional Pressure 30+, any faction may attempt to negotiate with Altonia. Circles Ob 3 to contact Altonian representative. Social scene resolves as Appeal or Debate vs Altonian representative's conviction (Order-leaning; Forensic arguments +1D).
- **Espionage:** Intelligence Domain Action vs Altonia: Ob 4 (they are a major power). Success reveals: current Institutional Pressure acceleration sources, which internal faction is dominant, and whether diplomatic resolution conditions are met.
- **Sabotage:** At Institutional Pressure 60+, a faction may attempt to delay invasion by sabotaging Altonian supply lines. Intelligence Ob 5. Success: Institutional Pressure −2. Failure: Institutional Pressure +1 (provocation).
- **Alliance offer:** A faction may offer vassalage to Altonia to redirect invasion elsewhere. Influence Ob 3 + Crown Mandate must be below 3 (Altonia only accepts broken states). Success: faction becomes Altonian client state — Institutional Pressure frozen but faction loses sovereignty (cannot make treaties, must contribute Military to Altonian interests). [EDITORIAL: Is this option too politically extreme to include, or is it a valid dark-path choice?]
...
**Prerequisites:**
- Thread Tension ≥ 40 (the Southernmost is dormant below this — nothing to find).
- At least one Player Character or officer with Thread Sensitivity 30+ (can perceive Thread structures).
- At least one Player Character or officer with relevant History (Einhir Scholar, Natural Philosophy, Expedition Leader). Ob 3 to plan the expedition.
...
|--------|-------|-----------|
| 1 | Approach | Travel to Southernmost border territory. Cohesion check Ob 1 for military escort (Thread proximity unsettles troops). Thread Sensitivity 30+ character: automatic Discovery Event. |
...
| 3 | Discovery | Reach the primary site (the locus of the original Einhir catastrophe). Practitioner with Thread Sensitivity 50+: Diagnosis automatically reveals the nature of the damage. Below Thread Sensitivity 50: Diagnosis Ob 3 to gain partial understanding. |
| 4+ | Repair (optional) | Extraordinary Weaving per existing rules. Each season of successful Weaving: Thread Tension −2 permanent from Southernmost contribution. |
...
**Expedition failure:**
- If military escort routes or all practitioners are incapacitated: expedition retreats. All participants: Certainty −1. Thread Tension +1 (disturbing the Southernmost without completing the work).
...
- Each successful Extraordinary Weaving season: Thread Tension permanent reduction. After 4–5 successful seasons: Southernmost fully stabilized (see VG doc's repair arc).

# CHURCH TERRITORIAL SEIZURE — TC 80 Threshold Event
## Date: 2026-03-25 (Session 5)
## Status: Designed — approved
## Integrates with: G-050 (event deck, TC 80 threshold), Church faction card, Löwenritter coup triggers

---

## Trigger
TC reaches 80 at Accounting.

## Eligible Territories
Every territory where Church Mandate > Crown Mandate at the moment TC hits 80.

## Procedure
The Church rolls to seize each eligible territory. One roll per territory, resolved sequentially (Church player or AI chooses order of attempts).
...
| Failure | Uprising fails. Church Mandate -1 in territory. Crown Mandate +1 (rally effect). TC -2 globally (overreach visible). |
...
**Ongoing seizure:** While TC remains >= 80, the Church may attempt seizure of additional eligible territories in subsequent seasons using the same procedure. Previous failures on the same territory add +1 Ob per attempt.
...
| Sovereign Authority Doctrine | Hafenmark | TC -2 or -3 — may drop below 80 threshold |
...
## Co-Movement
The TC 80 seizure event draws a Co-Movement Card (mass institutional upheaval disturbs configurational environment per P-01). Additionally, each individual territory seized draws a separate Co-Movement Card — a successful multi-territory seizure generates significant Thread consequences.

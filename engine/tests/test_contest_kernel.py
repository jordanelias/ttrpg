...
# re-skin + wrapper ADD checks on top. The count only grows.
# ⚠ "All 151 stay green" stood here until 2026-08-27 and is RETRACTED. ED-SC-0031 re-pinned FOUR
# checks inside the original 151 — the `degree bands` check and the three `sigma-leverage:` band
# checks — because Jordan's 2026-08-14 ladder ruling moved their expected values by exactly one
# step. They are green, and they are not the same assertions. The floor below is still the right
# guard (the suite must never SHRINK); the claim that no original check has ever changed value
# was simply no longer true, and a floor that passes is not evidence for it.
_KERNEL_FLOOR = 151
# moved 385->385 by ED-IN-0093 Wave 1: OI-18a/OI-19 stub conversions changed scaffold contract
# raise->StubResult; declared count-pin move (measured post-conversion: RESULT: 385 passed, 0
# failed — the 5 converted checks were rewritten 1-for-1, so the total held).
_KERNEL_EXPECTED = 389   # moved 385->389 by ED-SC-0031 (sigma_leverage.degree migrated to the
                         # owner's ladder). NET +2, and the arithmetic is worth stating because
                         # six checks CHANGED without the count moving: three band pins re-pinned
                         # one step (degree(3,3) 2->1 and the two float-Ob cases), one
                         # de-saturation comparison re-derived against the owner's ladder rather
                         # than the deleted pool-less bar, and the GOLDEN_TRACE literal
                         # re-recorded. The +2 is two NEW checks the migration earned: a PAIRED
                         # exactness check that the extension is inert below pool 8, and its
                         # non-vacuity twin that it does bite at pool 8+. The adversarial pass on
                         # ED-SC-0031 added two more (+4 total): a PAIRED owner-vs-extension
                         # measurement at pool 30, replacing a table that had no committed
                         # reproducer, and a check that the inertness crossover is a function of
                         # `ob` rather than the constant 8 an earlier draft asserted.

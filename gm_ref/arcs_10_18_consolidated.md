...
- **Einhir policy:** Almud manages the Einhir question as a governance problem — mistreated populations cause instability — while privately doubting whether a Valnese kingdom should have a caste system at all. The doubt is genuine but unresolved. He does not act because he has not decided whether acting is right, compounded by the structural costs: TC +3 (Church), Mandate −2 (northern Valnese nobility), IP +1 (Altonia benefits from Einhir suppression). His restraint is governance pragmatism plus ethical uncertainty, not secret sympathy.
- **TS 0:** Fully Church-culturally integrated. No Thread experience. If Discovery Event fires through external contact (player characters bringing Thread reality to Valorsplatz), the rupture is total — not confirmation of hidden sympathies but disruption of an administrative worldview that had no category for what he is now perceiving.
...
graph TD
    ...
    B -->|Success/Overwhelming/Partial| C["Church must respond politically"]
    B -->|Failure| D["Almstedt CONSERVATIVE:<br>PI recovery +1<br>Arc stalls — retry next season"]
    C --> E["Church Excommunication of Baralta<br>Church Mandate (5) vs Baralta Mandate (4)"]
    E --> F{Excommunication result?}
    F -->|Success| G["Baralta excommunicated<br>Strips Circles bonus<br>Hafenmark Mandate −1<br>Haelgrund PROCEDURALIST: +1 season"]
    F -->|Failure| H["Church Mandate −1, TC −1<br>Baralta vindicated"]
    G --> I["Season N+2: Grand Debate reversal<br>+ Sovereign Authority Doctrine"]
    I --> J{Doctrine result?}
    J -->|Overwhelming| K["TC −3, Church Mandate −1<br>Heresy Investigation blocked<br>Excommunication reversed<br>PI −2 (sympathy)"]
    J -->|Success| L["TC −2, Church Mandate −1<br>Heresy Investigation opens<br>Excommunication reversed, PI −1"]
    J -->|Partial| M["TC −1, Heresy Investigation<br>immediate, Excommunication stands<br>PI +1"]
    J -->|Failure| N["TC +1, Heresy Investigation<br>immediate, Baralta Mandate −1<br>PI +2, arc catastrophe"]
```
...
```mermaid
graph TD
    A["Crown Stability → 1<br>THE KING'S GAMBIT fires"] --> B{Almud's choice}
    B -->|HOLD THE LINE<br>No offensive actions<br>Stability +1 if unattacked| C["Crown survives<br>but cedes initiative<br>TC/IP/PI advance unchecked"]
    B -->|CALCULATED STRIKE<br>One action at +2D<br>Success: Stability +1<br>Failure: Stability → 0| D{Strike result?}
    B -->|ORDERLY TRANSITION<br>Torben Loyalty +1<br>Coup Counter frozen<br>Regency Council formed| E["Managed succession<br>Strand receives instructions<br>Brandt ordered to hold internal"]
    D -->|Success| C
    D -->|Failure| F["Crown Stability → 0<br>Elimination cascade"]
    F --> G["Strand OVERPERFORMER:<br>Crown admin continues<br>at normal Ob for 2 seasons"]
    G --> H["Brandt EXTERNAL THREAT FIXATED:<br>Military to T3+T10<br>Interior ungarrisoned"]
    H --> I["Season 3: Strand breaks<br>Crown admin +1 Ob<br>IP +2 (Laskaris reads signal)"]
    I --> J{Laskaris: Elske Loyalty > 2?}
    J -->|Yes| K["IP +1/season (sandbagging)"]
    J -->|No| L["Laskaris flips: IP +3"]
    I --> M["Strand flattery vulnerability:<br>First faction to approach<br>with genuine respect (−1 Ob)<br>redirects his capacity"]
```
...
```mermaid
graph TD
    A["Virke proposes shared warehouse<br>(Niflhel Diplomacy, Ob 2/faction)"] --> B["Feldhaus endorses (PROFIT-MAXIMISING: +1 Wealth)<br>Prudence Cardinal endorses (OPTIMISER: +1 Wealth)<br>Crown accepts (provisioning −1 cost)"]
    B --> C["Warehouse active Seasons 1–4<br>Thread-touched goods commingling<br>RS −0.5/season in territory (fractional)"]
    C --> D["Prudence Cardinal redistributes<br>Thread-touched artefacts to Church charities<br>Southern Einhir cultural goods<br>stripped of context"]
    D --> E{Detection: practitioner<br>in T12/T8?}
    E -->|Yes: Cognition vs Ob 3| F{Diagnosis?}
    E -->|No| G["Season 5+: effects<br>become visible"]
    F -->|Success| H["THE FIRE IS LIT"]
    G --> H
    H --> I["SIMULTANEOUS COLLAPSE:<br>Crown Wealth −1<br>Guilds Wealth −2<br>Church Wealth −1<br>Church Mandate −2 in territories<br>with southern Einhir populations"]
    I --> J["Evidence documented publicly:<br>RM +1 Presence (validates claims)<br>Almud Mandate cost for RM<br>drops from −2 to −1<br>CV −1 in affected territories"]
```

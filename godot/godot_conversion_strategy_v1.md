...
| 26 | articulation_layer | deterministic_accounting ⚠️ [CORRECTED 2026-08-23, plan S5c: this cell read `armature_dot_product`, the THIRD copy of a value verification RU-4 had already overturned on the contract row — "the armature dot-product is the SUBSTRATE interpretation primitive (key_substrate §2.5/§4.1), NOT a system resolver". S5c folded the second copy (`wiring_manifest.yaml`) away; leaving this one would rebuild the disagreement the fold removed. Value only — the port-phase column is untouched, and is NOT `wiring.port_rank` (this table's phases run 1-4+R, that field runs 0-9).] | personal/scene/provincial | `ArticulationLayerV30.gd` (exists; refit to substrate v2) | **4 — last runtime** | Universal `*` reader; the K8 fan-out question lives here; Chronicle/cut-scene tiers already ported (v30) — refit not rebuild. |

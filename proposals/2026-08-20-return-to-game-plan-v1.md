...
>   (`engine/autoload/game_state.py:163`), bucketed to a 0–4 index by `ACCORD_MAP` (`:61`); the port
...
|---|---|---|---|
| **Q1** | **The faction-stats packet** — the roster disagrees across **five** surfaces now: `descriptor_registry.yaml:103-111` (5 stats, Influence 1-7), `engine/autoload/game_state.py:104-114` (6), `canonical_registry.md:113` (Wealth/Military 1-7), `generation_sourcebook_v1.md:34` (a fourth), and **`valoria-game/systems/util/Constants.gd:85-86` (`MIN=0/MAX=7`, a fifth)**. ED-FA-0004 open. | M1 junctures 1 and 4; the `score/2` obstacle derivation; C1's `KNOWN_DIVERGENT` list; C2's roster export | `HANDOFF.md:209+` — still "NEEDS JORDAN … not ruled" |

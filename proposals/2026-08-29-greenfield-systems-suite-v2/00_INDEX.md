...
| **`references/content_registry.yaml`** *(new)* | **the catalogue of kinds the world can contain**: institution/presence kinds (`07`), project kinds (`09`), world-event rows (`11`), the `(post_kind, caste) → open \| gated \| closed` matrix (`04`) | the same pattern; one exporter, one blocking `--check` |

...
| **C-4** | The person's normative form table in `01 §1.1` gains **`beliefs`** | this suite's own `01` | §6. A creed-Belief is a **marked subset of a person's Holdings** (`proposals/2026-08-18-epistemic-propositions-and-provenance.md §3.2`, ruled), capped at canon's active-set limit; dropping one is a form transition. **Net field count is unchanged**: C-2 removes one member, this adds one. **A standalone `credence` Gauge is cut** — §6.2 executes the cut this document's own first draft pre-committed to once a general epistemic layer existed. |

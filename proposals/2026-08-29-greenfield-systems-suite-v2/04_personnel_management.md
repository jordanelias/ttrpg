...
gating matrix now gates `pm.candidates`. Everything in §§1–8 not marked `v2` is v1's design, carried
with its reasoning. The new material is §3.2 (the caste gate itself), the `class:` field on every
...
| **Guilds** | full | full | variable; Free Master examination biased; Guild Master/Comptroller near-closed | `:665` |
| **Niflhel** | full | full | **favoured — "caste-blind by necessity"** | `:613`, `:666` |
| **Warden of the Thread** | **gated** — lower baseline Thread Sensitivity makes the initiation threshold harder | slightly gated | **favoured** — higher baseline TS, cultural familiarity | `:636`, `:667` |
| **Restoration Movement** | ideologically suspect (gated) | variable | ideologically favoured — the RM's base | `:668` |
...
*(v1, with one corrected write — O-04-1.)* The principal grants the post to one member of the candidate
set. Four writes, the fourth conditional:
...
| what the player is asked to decide | which module | how often |
|---|---|---|
| whom to appoint from a published, caste-annotated candidate list | `pm.appoint` | when a vacancy the player's post can fill is on the Slate |
| whether to recall a holder, citing a Tag | `pm.recall` | when the player holds a post whose remit includes it, and chooses to spend budget |
| whether to attempt custody of a post they do not hold | `pm.custody` | same — a deliberate, budgeted attempt |
...
appointment-adjacent action the player learns.
...
**Scope**, per `01 part2 §13`'s own framing: `pm.vacancy`, `pm.candidates`, `pm.appoint`, `pm.tenure`
and `pm.recall` resolve by threshold and **roll nothing** — diagnosed on P-iii and P-v only, **no
N/R/S/E verdict offered**. `pm.audit` and `pm.custody` roll and are diagnosed on all five.

| property | verdict | reasoning |
|---|---|---|
| **P-i** legible odds | pass | Pool is the actor's own attribute pair; obstacle is `target_score/2`, both published (§5.1). The audit's dossier terms are published per tag. The caste gate is now the *most* legible eligibility term in the module — full institution/kind/caste/verdict disclosure (§3.3), the one ruled exception to "publish a band, never a number" |
| **P-ii** uniform leverage | pass | Tag weight enters through `net_boost`, never as a flat obstacle shift. `caste_gate` never enters a roll at all (§3.2.3) — a stronger property than uniform leverage, since it is not leverage, it is a filter |
| **P-iii** bounded, monotonic | pass | Grudge is doubly bounded (§4.1). `derive_ob`'s floor prevents a cliff. Recall is capped per principal per season and priced escalatingly. The caste matrix is a **finite table** — bounded by construction, not by tuning |
| **P-iv** graded, recoverable | pass | Four audit outcomes, total over the bands (§5.2). The worst outcome removes a post, never a person. A caste-gated candidate is excluded from *this* post, never from the person's whole future — they remain a candidate wherever the matrix says `open`, and a `gated`/`closed` verdict names the exact `Precedent` that would open it |
| **P-v** right engine | pass | Appointment and candidacy are thresholds on visible state → `gate`. The audit is genuinely uncertain judgement over accumulated evidence → `d_sigma`. Custody is a contested attempt → `d_sigma`. The caste gate adds a fifth `AND` term to an existing `gate` module — it does not create a new resolver kind |
...
**Necessary** — the caste term is one row in an existing conjunction; it does not add a module, a
resolver kind, or a write leaf. **Robust** — the two failure directions the corpus measured (an

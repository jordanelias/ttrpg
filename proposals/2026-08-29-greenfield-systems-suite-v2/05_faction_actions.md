...
| **O-5.13** | **`11 §6`'s `we.eligible` / `we.fire` module pair** *(accepted, not imposed — `11`'s author retired them and specified what he needed back)* | this suite's own `11`, by its author's own proposal | A second `gate`/`d_sigma` dispatcher for the same schema. **Accepted on the same evidence as O-5.11 and refused the same test twice would have been the inconsistency**: field by field the world-event row *is* an action row with `remit_kinds: []`. `part 2 §5.6` makes the two extensions it needs — `fa.gate` iterates a **declared iteration domain** (posts, or targets), `fa.resolve` sources its pool from the row (`attr` pair, or `hazard_pool`) — and gives G-1's target-keyed exclusion a **named home** in `fa.select`. **The merge is not declared done without that rule**, which is the one piece with real friction and the one this page could have quietly dropped |
...
⚠ **The gate iterates a *declared iteration domain*, not a rung ladder** — generalized for O-5.13.
For a row with a non-empty `remit_kinds` the domain is `posts(faction, tier)` and the loop above is
verbatim. For a `remit_kinds: []` row (`11`'s world events) there is no post to walk, so the domain is
`targets(row.scope)` — the declared place nodes or factions — and the gate applies the row's own `gate:`
predicates to each. **One loop over a domain the row names, rather than two loops in two modules.** The
precondition below binds only the first case, because a hazard has no leader to lack (`part 2 §5.6`).

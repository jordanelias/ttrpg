...
| **P0-10** | **23 of the suite's 52 ` ```yaml ` blocks do not parse.** Measured 2026-08-29 with `yaml.safe_load` over every fenced yaml block in the suite. The cause is uniform and deliberate: schema sketches column-align several key/value pairs on one line for readability (`transition: <id>          entity_kind: <kind>      field: <...>`), which YAML rejects | whoever builds P0-6's registries and exporters from these shapes | DOC | **open, and NOT a defect today** — under `CLAUDE.md` §0.05 a design document is *reference*, nothing loads these blocks, and the alignment genuinely reads better on the page. **It becomes a defect the moment Phase 0 builds the registries**, because the shapes are the input and the exporters carry a blocking `--check`. Resolve by writing the registry files as valid YAML and letting the documents keep their aligned sketches — **do not "fix" 23 blocks into machine form**, which would trade the reference layer's readability for a property nothing reads |
| **P0-11** | **The epistemic layer is now load-bearing and rests on an unbuilt module plus an unspecified rule.** `01`'s `Holding` (replacing `Memory`), `02`'s creed-Belief (a marked subset of Holdings), and `10`'s witness channel all resolve to the ruled `Proposition`/`Holding` grammar — whose home is **`npc_memory`, one of the nine `doc: null` modules, declared and entirely unbuilt** | `01`, `02`, `10` — the belief layer end to end | DOC → INERT | **open, and it is TWO blockers, not one.** (a) Until `npc_memory` exists, a creed-belief grant produces a `prop_id` with **no `Holding` anywhere to give it confidence**. (b) **The content-address hashing rule is unspecified** — the ruled proposal calls it *"the first thing to nail down"* and with ruling P1 (NPCs hold propositions too) it now spans the whole population: two agents hashing the same claim differently makes comparison, contradiction and corroboration all fail **silently**. `10 §2.2`'s fixed cross-platform `candidate_id` hash is the in-suite precedent to copy. ⚠ A third, independent blocker sits under both: **`Key.causes` is unpopulated** (three sites, only one conditionally non-empty), so the ruled *independence* metric cannot compute and `confidence` cannot yet mean what P3 says it means |

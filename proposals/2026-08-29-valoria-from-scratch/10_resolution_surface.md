...
**5.5 Spend Momentum.** Adds exactly one d10, sourced specifically from the Conviction being pursued (§8.2) — a pool-source addition, not a flat success. Resets to 0 each session regardless of use, so hoarding it buys nothing; there is no cross-session compounding to check for dominance.
...
- **Momentum (§5.5)** survives on the same sourced-die logic, capped at one die and reset per session so it cannot compound.
- **A leader affecting a group** (relevant wherever this document's `roll`/`obstacle` pair is reused at settlement or faction scale by another document) is licensed here as a **fraction of the group's own pool**, never a flat addend: `ΔPool_group = round(f × Pool_group_base)`, f small (0.05–0.15 by the leader's own rating). The *same* leader then contributes more absolute dice to a large body than a small one — leverage that scales *with* force size instead of against it, which is the fix the corpus's arithmetic demands and any consuming document must implement through this formula rather than inventing its own.
...
+1 for pursuing a Conviction in-scene; 0–4, resets per session; spends for exactly one extra d10 (§5.5).

**N-line.** Cut it and a player's authored Convictions have zero resolver consequence — playing to your values becomes pure flavour text, which is precisely the R-failure this design cannot afford: a "choice with real impact" that never touches a die is not one.
**Closed loop.** Produced by scene-level detection of Conviction-consistent action; carried on the person, capped and volatile; consumed by the §5.5 manoeuvre.
...
A per-scene resource depleted by the *margin of loss* in each losing exchange of an opposed social contest (§4); at 0, the person must concede or take a Disposition/Coherence hit. Resets between distinct contests — it is not a permanent stat, matching Momentum's within-session-flow shape, not a compounding punishment across the whole game.

**N-line.** Cut it and every opposed social contest collapses to one roll with no attrition — a skilled arguer instantly demolishes anyone regardless of resolve, and there is no mechanical difference between a jab and a sustained interrogation. That is an R loss: no meaningful choice of "press on or disengage" mid-argument.
...
Take doc01's own trace: Greta, Pool 8 (Agility 4 + Wayfinding/Stealth 3, +1 die from Momentum spent on her Community conviction), against the Knight of the Peace on checkpoint duty, Pool 7 (Acuity 4 + Vigilance 3) — a person, so this is an opposed contest (§4), gap 1, well inside manoeuvre-rich territory.

**One `attempt_id`, one substream.** Say it yields Greta 5 successes, the Knight 4. Margin +1 → **Clean Success** for Greta.

- **Played.** The player controls Greta directly. They see both pools before committing, choose the Momentum spend (a real, resource-costed decision, §5.5), watch the dice, see the margin and band resolve live.
- **Witnessed.** The player controls someone else nearby — a neighbour glimpsing the run. `choose` supplies Greta's manoeuvre declaration (her own Momentum spend, decided by her own decision function, not a human at this table). The *identical* substream produces the *identical* dice. The player sees one line: "Greta slips past the checkpoint." No dice, no menu — but it is not a different formula, it is the same `resolve` call with the render truncated.

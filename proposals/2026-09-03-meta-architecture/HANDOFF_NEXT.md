...
| 1g | ⚠ **THE APPEND-SIDE INTEGRITY CHECK, WHICH IS `ID-9`'S OWN MECHANISM AND HAS NO IMPLEMENTATION.** Stage 4 `§C.2` says *only the gate mints a Receipt* and *the log's append asserts every receipt id is in the gate's minted set for this tick*, so **an Event reporting a write the gate did not apply fails at append**. In the tracer `Event.changes` is a fold-built `StateChange[]` and the log is appended to with a bare `list.append` at **five** sites (`shape.py` 1985, 3973, 4010, 4173, 4852 — this row said `season()`'s alone until 2026-09-04, which reads as a one-line fix and is not); `Event.__post_init__` checks only `causes`. **Nothing refuses a fabricated change**, and the fix is ONE append-side owner every site routes through, not a check bolted to the driver. | a planted Event whose `changes[]` names a receipt the gate never minted raises at append |

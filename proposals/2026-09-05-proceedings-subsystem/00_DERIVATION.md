...
|---|---|---|
| **carriers** | **0** | `Proposition` (the matter) · `Record` (the record, the summons, the writ) · **`Office` + a `hold` Tenure** (the bench) · `Tenure` (`commit`, `oblige`, `hold`) · `Person` · `Rung` (the venue) · `Date` (the occasion) · `Act` · `Event` · `Claim`. **Every one already rostered** ⚠ **CORRECTED 2026-09-07: this cell read `Seat`, which is not a carrier.** The claim it supports — that the design adds none — was true, and its evidence named a type the tracer does not have. `04_CODE_ARCHITECTURE.md` §B.7 calls the thing a `Seat` and the chain implements it as `Office`; a bench member is an `Office` plus a live `hold`, which is what the rest of this directory says |
| **fields on existing carriers** | ⭐ **0** | ⚠ **CORRECTED 2026-09-07, and this cell was wrong in three ways at once.** It read *1, and it is contested · `Proposition.rung` … See B.2, where it is argued against and then **admitted** with its price.* **§B.2 is titled *THE ONE FIELD IS WITHDRAWN. THERE IS NO NEW FIELD*** — it retracts rather than admits, it argues about **`Tenure.degree`** rather than `Proposition.rung`, and `13_ADVERSARIAL.md` books the result as *the count improves to zero new fields*. **So the retraction propagated everywhere except the table it was a retraction OF**, which is the count `README.md` calls *the whole argument*. See §B.2 |
...
| **verbs** | ⭐ **0 new** · 14 reused · 2 given a body · 1 corrected · 1 landed · 1 split into five | ⚠ **CORRECTED 2026-09-07: this cell read *4 new*.** `04_VERBS.md` PART A's headline is *the game structure adds **ZERO** new verbs*, and `README.md`'s cost table says *new verb names invented: **ZERO***. **Two files said zero and this one said four.** What the design actually does: `speak` and `determine` gain a body, `convene` is corrected, `release` is landed (it was specified and the table never carried it), and the six investigation acts become five rows. **Every name was already in the tree** |

...
| **what they were told about you** | ⛔ you cannot read it. ⚠ **You could `interview` them until 2026-09-06** — at the price of their learning what you asked — **and that act went to the `FI` lane with the other five** (`04_VERBS.md` §B.3). **In the room you now have no channel to it at all**, which sharpens the row rather than blunting it | correct it: tell them your own firsthand **before the sitting** | ⭐ this is *"reaching the bench before the sitting"* — **which the design calls corrupt and cannot distinguish from diligence.** A live question, not a defect. **And it is now the ONLY route**, so the corrupt-looking move is the only move — which is a better dilemma and a worse gap, depending on whether `FI` lands the acts |
...
| **7** | 🔨 **`Tenure.term`** — the one new field, **and it may be zero** | the return day, the summons, contumacy, overdue pressure, the inquisition's indefiniteness, **and the adjudicator's declared time limit** (Jordan, 2026-09-06). ⚠ **Cut to ONE column 2026-09-06** — `declared_by` is `Tenure.conferrer`, deleted from the tracer for reaching no reader; `closer` is `T-o` + `T-m`. `P-04` asks whether `Date.due_at` already carries it |

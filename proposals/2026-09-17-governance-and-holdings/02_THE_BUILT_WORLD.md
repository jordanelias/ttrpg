...
⚠ **2026-09-17 NERS pass: `matured stages` has no carrier.** `Record.matured` is ONE bool for the whole Record (`carriers.py:445`; `matter.py:64-109` sets the same flag once per ripening stage), so the ratio above reads a per-stage maturation `01` §A.12 and §B.2 list as **DEFERRED**. `ceiling` is buildable today only as `condition_scale × matured` (nothing or full), or it waits on the per-stage shape — decide in the commit that lands it.
...
| a built kind never regrows unauthored | ~~**MECHANICAL** — an explicit `regrowth: 0` row per built kind, plus a test asserting it~~ ⛔ **STRUCK 2026-09-17 (NERS pass): no `regrowth` table exists anywhere under `engine/` and nothing would read one** — a row nobody reads is `ID-13`'s dead carrier, and a test over it cannot observe the failure it excludes (an unauthored positive `Site.condition` delta comes from a code path, not a fixture cell). **MECHANICAL** through what already emits: `wear()` is the only MATTER writer of `(Site, condition)` (`matter.py:229-246`) and `restore` the only RESOLVE one, so assert the SIGN of every `Site.condition` delta by its step | the polarity rule makes a missing row RAISE (`rosters.yaml:809-812`), so the absence of a row is a crash rather than a silent zero. **Load-bearing on the game** — it is the line between authored and unauthored construction — so the test earns its existence under §0.1 pt 5 |
...
| **BW-14** | The ontology stands on cardinality, not on a destroy cascade | name ~~an object~~ **a `Rung` or a `Site`** in this tree that a verb destroys (⚠ narrowed 2026-09-17: `destroy_record` does `del w.records[rid]`, `effects.py:293-301`, so *an object* was falsified by a resolvable verb the day it was written). **There is none of either: `(Rung, exists)` and `(Site, exists)` have zero producers.** If one is found, A.1.1's struck argument must be re-argued rather than re-instated |

...
fog); d.1's propagation into the engine — `engine.py:344-347`'s `morale_start` formula, changed to read faction
Stability — and its own PP-711-supersession ledger entry. `mass_battle_v30.md` is reference and is not where any
of this is fixed. None of this blocks Part A, which was already clear to build.

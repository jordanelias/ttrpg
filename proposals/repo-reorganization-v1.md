...
5. **P4 — `systems/` by subsystem + intra-system sim** (§2a): rehome `designs/*` → `systems/*` (incl. the `character/` nesting and the piety split); distribute `sim/` subpackages; apply the 14 cross-folder import rewrites. Full citation + import tooling. Do last. ✓ **Slice 1 EXECUTED 2026-07-17 (separate PR):** the three **doc-only clean** subsystems `designs/{npcs,articulation,ui}/` → `systems/{npcs,articulation,ui}/` (RULED 1:1, no sim, no cross-subdir splitting) — chosen first to isolate the doc-move/citation tooling from the sim-distribution/import surgery. This is the FOUNDATIONAL slice: it stood up the top-level `systems/` primary and **taught the whole toolchain about it** — added `systems/` to `broken_dependency_checker` (`extract_file_refs`), `currency_consistency_check` (path regex), `validate_ed_citations` (`SCAN_PREFIXES`), `ci_co_file_checker` (design-doc regex), `workplan_status` (`RELEVANT_PREFIXES`), `audit_staleness` (4 broad scopes), `build_decisions` (`SWEEP_DIRS`), `build_proposals`+`dashboard_data` (globs); pointed `ci_editorial_checker` `EDITORIAL_PATHS` at `systems/npcs/` (all npcs docs pre-verified to carry markers). Functional layer rewritten (183 path refs + 14 `canonical_sha__` key labels across 26 files, incl. `canonical_sources`/`propagation_map`/`module_contracts`/`CURRENT.md`/registers); **prose provenance + append-only `.jsonl` ledgers left to the `restructure_ledger.md` alias pointer.** Caught + fixed a **single-hop remap chaining** gap: 7 pre-existing `designs/systems/*` → `designs/npcs/*` restructure rows had to be repointed at `systems/npcs/*` (the checker doesn't chain, so live ledger entries citing the 2-generations-old path would otherwise dead-end). obs/dashboard regenerated; `freshness --update` (0 stale, no duplicate SHA lines). **Verified:** broken_dependency + freshness + currency + co-file + editorial + ed-citations + module-shape + `valoria_local` all green; `tests/valoria`+`tests/contracts` **335 passed**. **Deferred to later P4 slices:** the doc-only `_architecture`; the sim-bearing subsystems (threadwork/world/settlements — doc+sim template); the cross-subdir splits (scene→combat/social_contest/fieldwork, provincial→factions/mass_battle, personal→character/*) + the RULED ED-644 piety rename; `mechanics_index_gen`'s `systems/` scan (report-only, non-trivial repo-walk root). ✓ **Slice 2 EXECUTED 2026-07-17 (separate PR) — the doc+sim TEMPLATE:** `designs/threadwork/` (9 docs) + `sim/thread/` (8 py, a self-contained island) → `systems/threadwork/` + `systems/threadwork/sim/`, imported as `systems.threadwork.sim.*` (was `sim.thread.*`) — 10 import sites rewritten (6 internal, + `engine/autoload/game_state` ×2 lazy, `sim/personal/knots` ×1 lazy, the CI-run `sim/tests` regression ×1). Made `systems/` a **Python package** (`systems/__init__.py` + `systems/threadwork/__init__.py`); frozen `tests/sim/` + non-CI `tests/index/` fixtures deliberately NOT rewritten (dormant refs → alias). `ci_co_file_checker` gained a **pure-rename exemption** (a path-only move of a params-bearing `_v30` doc has no added lines, so it no longer demands a spurious params co-change) — general fix for every params-bearing subsystem move. Same single-hop remap chain-fix (8 rows). **Verified:** all gates green; `sim/tests` (the oracle, incl. the moved package) + `tests/valoria` + `tests/contracts` pass **in isolation (CI-equivalent)**. ⚠️ **KNOWN TEMPORARY LANDMINE — the combat slice must resolve it:** the new top-level `systems/` package collides with `designs/scene/combat_engine_v1/systems.py`, imported bare as `import systems` (a pre-existing combat anti-pattern). In one shared process, whichever `systems` is imported first wins, so running `sim/tests` + `tests/valoria` **together** breaks 14 combat tests. **CI is unaffected** — it runs each test dir as a separate `python -m pytest` process, and no `tests/valoria` test imports the top-level `systems` package, so combat's `import systems` (with `sys.path.insert(0, combat_dir)`) always wins there. Full-repo `pytest` was already broken independently (`tests/hooks` imports the retired `github_ops`). The **combat P4 slice permanently fixes this** when `combat_engine_v1/systems.py` becomes `systems/combat/sim/…` and the bare `import systems` disappears; until then combat and threadwork sims must not be collected in one process. **Still deferred:** `world`/`settlements` (sim-bearing); the cross-subdir splits + ED-644 piety rename; `mechanics_index_gen`'s `systems/` scan. ✓ **Slice 3 EXECUTED 2026-07-17 (separate PR):** the substrate **design docs** `designs/architecture/` (35 files) → `systems/_architecture/` (RULED underscore-prefix substrate tier). **Doc-only** (the executable substrate already moved to `engine/` in P3) and **not editorial-governed** — the cleanest remaining slice. **Pointers-for-prose applied harder** (per Jordan's steer): only the machine-read + governing layer rewritten (90 path + 13 `canonical_sha__` refs across 16 files); the **frozen reports NO tool reads** (`collation_report_summary.yaml` alone had 246 refs, + `numeric_bounds_report`, the `_r2` docs, `canonical_sources_provenance`) and the lane-handoff continuity prose ride the `restructure_ledger.md` alias **pointer** instead of a ~260-ref churn. **ED-1051 coupling handled explicitly:** `propagation_spec_v1` / `key_substrate` / `key_type_registry` are the substrate docs the pending M1 engine_clock ratification targets — `HANDOFF.md`'s live "Next actions" instruction + `CURRENT.md` + `module_contracts` forward-refs were repointed at `systems/_architecture/…` (path-only; the ratification *decision* is unaffected). **Made the dir-prefix alias pointer robust:** `broken_dependency_checker`'s restructure remap gained **longest-dir-prefix resolution** (`_resolve_remap`) — a single `designs/architecture/ → systems/_architecture/` pointer row now resolves all 8 live-ledger refs (ED-931/IN-0019/0020/0062/0070) without enumerating each moved file (exact-match still tried first; a prefix match to a nonexistent path stays BROKEN, so it never false-passes). **Verified:** broken_dependency + freshness (0 stale) + currency + co-file + ed-citations + module-shape + `valoria_local` green; `test_gen_audit` (33) + `tests/valoria`+`tests/contracts` pass. **Still deferred:** `world`/`settlements` (sim-bearing); the cross-subdir splits (scene/provincial/personal) + the RULED ED-644 piety rename; `mechanics_index_gen`'s `systems/` scan. ✓ **Slice 4 EXECUTED 2026-07-17 (separate PR) — first editorial-governed sim-bearing subsystem:** `designs/world/` (23 files) + `sim/world/` (5 py) → `systems/world/` + `systems/world/sim/`, imported as `systems.world.sim.*`. **`sim/peninsular/` DEFERRED** — it's imported by `sim/provincial` + `engine/mc_v18` and its design doc lives in `designs/provincial/`, so its home is resolved with the provincial slices (§2a routes only its *tracks* to world); moving `sim/world` alone required the one cross-folder rewrite of `sim/peninsular/accounting`'s `from sim.world…` → `systems.world.sim`. **#165 lesson applied up front:** swept `engine/`+`sim/`+`tools/`+CI-run tests for runtime path reads (incl. component-form joins) — none for world docs except `tools/geography/jsx_to_canonical.py`'s argparse default (rewritten; `adjacency_map.jsx` moves with the dir), and **ran `sim/tests` (the oracle), not just tests/valoria**. **Two general gate fixes** for the co-location layout: (1) `ci_editorial_checker` now scopes marker checks to **`.md` files** — pointing `EDITORIAL_PATHS` at `systems/world/` swept the co-located `sim/*.py` + `adjacency_map.jsx` + `__init__.py`, but editorial markers govern content, not code (every future sim-bearing editorial subsystem hits this); (2) the 3 substantive world docs that lacked a marker (`worldbuilding_v30_{index,infill}`, `southernmost_v30_infill`) got honest `[EDITORIAL: ED-IN-0071 — relocated]` markers so `EDITORIAL_PATHS` → `systems/world/` **preserves** (rather than silently drops) world editorial coverage. Pointers-for-prose held (audit scripts / skills / non-CI tests / frozen reports ride the alias). Same chain-fix (14 rows) + `_resolve_remap` dir-prefix pointer. **Verified:** all gates green; `sim/tests` (the oracle) + `tests/valoria` + `tests/contracts` pass **in isolation**. **Still deferred:** `settlements` (sim-bearing + ED-644 piety rename); `sim/peninsular/` home; the cross-subdir splits (scene/provincial/personal); `mechanics_index_gen`'s `systems/` scan.

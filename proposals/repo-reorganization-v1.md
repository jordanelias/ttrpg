...
4. **P3 — `engine/` assembly**: `sim/mc_v18.py` + `params/` + `engine_params/` + `sim/substrate/` + `sim/cross_scale/`; rewrite `mc_v18` imports in lockstep with P4. ✓ **P3 seed EXECUTED 2026-07-16 (separate PR):** `references/engine_params/` → `engine/engine_params/` (the typed Class-C export; 14 refs across 12 files; `export_engine_params.py` `OUT_PATH` dir-join edge + the round-trip CI gate verified against the new path). **Still pending:** `params/` (recon showed **304 live-file refs** — every sim module cites `params/X.md`; high-churn, gate-coupled via `patch_propagation_checker`) and the `sim/` cross-scale armature (`mc_v18`/`substrate`/`cross_scale`), whose import rewrites couple to P4's `sim/` subsystem split — deferred to the coordinated P3/P4 pass.

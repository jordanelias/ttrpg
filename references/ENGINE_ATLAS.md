...
A canonical identifier is only a usable handle if searching for it returns its references and little else. **Key types satisfy this by construction** — dotted and distinctive, median 26 occurrence(s). **Contract names largely do not**: several are ordinary English words, so the count below is dominated by unrelated prose and identifiers (median 112). This is evidence for a naming rule, not the rule itself — nothing is enforced here.
...
| `threadwork` | 901 | 0 |
| `victory` | 811 | 0 |
| `social_contest` | 716 | 0 |
| `npc_behavior` | 632 | 0 |
| `faction_state` | 319 | 0 |
| `settlement_layer` | 315 | 0 |
| `articulation_layer` | 311 | 0 |
| `scene_slate` | 192 | 0 |

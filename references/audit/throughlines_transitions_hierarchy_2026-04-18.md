...
| TL-7 | Warden × Thread Stress Test | ED-629 P0 integration: Warden Thread operations under RS pressure | Cross-reference note only; full stress test remains open | Open (P0) |
...
|-----|------|-----------------|-------------------|
| **T1** | Settlement-Level Thread | Thread operations producing settlement-level consequences (Order/Prosperity/Defense shifts) alongside province-level RS effects | settlement_layer §4.4. Cap: ±1 per settlement stat per season from Thread ops. Dissolution → Order −1; Mending → Order +1; Gap → Defense −1. |
...
| **Rendering Stability Pressure** | RS decline from Thread operations → Calamity Drift → PT erosion → faction strategic recalculation | threadwork §5.1–5.5 (RS track), conviction_track §1.3 (Calamity Drift), mass_battle §A.14 (battle RS cost), peninsular_strain §3.1 (Substrate Fracture) |
...
|-------|----------------|----------------|------------------|
| **Peninsula** | Season / Arc | Global clocks: RS, TC, IP, PI, Peninsular Strain | Victory conditions, faction elimination, Altonian invasion |
...
| **DE-4** | Thread operation | Faction stats | RS change ≥1, or Conviction Scar fires, or Gap/Lock/Knot at Territorial+ | Thread Domain Echo (scale_transitions §5.6): Dissolution → Stability −1, Mending → Mandate +1, etc. | 1 Thread Echo/scene/faction |
...
| **DE-11** | Battle | Peninsula | Any inter-faction battle on Valorian soil | RS −1 (Campaign/War: RS −2), IP +2, Peninsular Strain +1 (mass_battle §A.14) | Per season |
...
| **ZI-6** | Clock threshold | Scene Slate | RS, TC, IP cross band threshold | player_agency §4.2 Step 2b: Thread-State scenes (Priority 1–3) |
...
| **Calamity Drift** | RS crosses threshold (≤50, ≤35, ≤20) | PT −1 in affected territories at Accounting | Accounting | Ignores ±1/season PT cap; stacks with Thread Op CV Drift | conviction_track §1.3 |
...
ROUND SCALE
  ...
  Coherence −1 (Mending base cost)
    │
SCENE SCALE
  ├── RS +1 (Mending Success, threadwork §2.4)
...
TERRITORY SCALE
  ...
  └── Co-Movement card effect applies (RS ±N per card)
    │
PENINSULA SCALE
  ├── RS +1 may shift RS band → Calamity Drift threshold changes at next Accounting
  ├── If RS crosses 40/60/80 band: Thread Revelation Curve triggers (threadwork §5.6)
  └── If RS recovers above threshold: PT erosion from Calamity Drift ceases
```
...
|-------|-------|----------------|
| Rendering Stability (RS) | 0–100 | Thread world-health; drives Calamity Drift, Revelation Curve, faction strategy |
...
| **Co-Movement card deck management** | ED-577-02: reshuffle timing unspecified. This affects card-counting strategy and long-campaign RS variance. | P2 |

...
|-----------|------|--------------|-------------|
| Piety Track | CT* | 0–10 | Debate position tracker. Side A wins at ≥ 7; Side B wins at ≤ 3. Compromise zone: 4–6. Canonical doc: `systems/characters/reference/conviction_track_v1.md` (promoted PP-681). ⚠ NAMING: `systems/social_contest/reference/social_contest_v30.md` calls this same debate tracker **Persuasion Track** — unresolved glossary↔social-contest name collision (also tangles with the ED-IN-0075 character-Piety→Truth rename); flagged for reconciliation. |
...
| Disposition | — | NPC-attached attitude state toward another entity (PC, faction, settlement). Drives behaviour-tree branching in NPC AI. Canonical in `systems/npcs/reference/npc_behavior_v30.md`; no separate first-class doc. Heavy cascade-terminal (Mode D — 391 chains). |
| Domain Action | — | Faction-level strategic action resolved at scale-transition phase. Mechanically the unit of strategic-layer decision. Canonical in `systems/_architecture/reference/scale_transitions_v30.md` + `designs/provincial/faction_layer_v30.md`; no separate first-class doc. Heavy cascade-terminal (Mode D — 346 chains). |
...
| Zoom In | — | Scale transition from larger (faction/territorial) to smaller (personal/scene) scale. Mechanically implemented as a transition phase during Domain Action resolution. See `systems/_architecture/reference/scale_transitions_v30.md` §4. |
| Zoom Out | — | Scale transition from smaller (personal/scene) to larger (faction/territorial) scale. Reverse of Zoom In. |
| Cardinal | — | Title for a senior Church official with Great Influence in the Church of Solmund. Four-Cardinal Structure governs the Church hierarchy. See `systems/npcs/reference/npc_roster_v30.md` for named Cardinals (Prudence, Justice, Gifts, Temperance). |
...
| Non-Player Character | NPC | A character not controlled by the player. In implementation: AI-driven entity with behaviour-tree decision logic per `systems/npcs/reference/npc_behavior_v30.md`. |
...
| Artificial Intelligence | AI | Behaviour-tree decision logic for NPC-controlled factions/characters. See `systems/npcs/reference/npc_behavior_v30.md`. |
...
| Conflict Architecture | `systems/_architecture/reference/conflict_architecture_proposal.md` | Architectural specification for conflict resolution across scales. Niflhel dissolution + Löwenritter graduated autonomy + Tensions Deck + Royal Assassination Fuse all anchored here. |
| Campaign Architecture | `systems/_architecture/reference/campaign_architecture_v30.md` | Architectural specification for campaign-scale structure: phase ordering, season cadence, scale-transition orchestration. |

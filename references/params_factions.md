...
## ED-005 Resolution (PP-257)
Restoration Movement primary NPC contact: **Maret Vossen** (confirmed from params_factions §Restoration Named NPCs).
Secondary: Aldric Hann. Full stat blocks deferred to campaign development as documented.
ED-005 resolved — names confirmed, stat blocks remain deferred (separate editorial if needed).

## ED-006 Resolution (PP-258)
Riskbreakers: extralegal arm of the Löwenritter. Small-cadre elite consequentialists loyal to Valoria-as-concept.
Will go extralegal when judged necessary. Already documented in params_factions §Riskbreakers. ED-006 resolved.

## ED-034 Resolution (PP-266)
Ceiral Ritual RS +6–10 vs BG Co-Movement cap RS +1: **not a contradiction**.
BG Co-Movement cap applies only to BG-layer Thread abstraction. Ceiral Ritual is a TTRPG Zoom In scene — it bypasses the BG cap entirely. The cap governs faction-level orders; the ritual is a personal-scale operation with full TTRPG resolution.
ED-034 resolved — no patch to mechanics needed.

## ED-036 Resolution (PP-267) [FLAGGED FOR DESIGNER REVIEW]
Altonian Vanguard unit stats (provisional, for simulation unblocking):
| Unit | Size | Command | Discipline | Power | DR |
|------|------|---------|-----------|-------|----|
| Vanguard Infantry | 6 | 4 | 5 | 3 | 2 |
| Vanguard Cavalry | 4 | 5 | 4 | 4 | 1 |
| Vanguard Siege | 3 | 3 | 3 | 5 | 3 |
Altonian faction: Mandate 6, Military 7, Stability 5 (external aggressor, no Influence/Wealth/Intel stats in Valoria context).
[FLAGGED: confirm unit stats before Altonian engagement can be canonically simulated.]

## ED-143 through ED-146 Resolution (PP-299) [FLAGGED FOR DESIGNER REVIEW]
PC simulation constructs adopted as **canonical test characters** (not world-canonical player characters):
- **Mira Sondhal** (ED-143): Practitioner-Scholar. TS 61. Confirmed for simulation use.
- **Arend Voss** (ED-144): Crown Agent-Soldier. Confirmed for simulation use.
- **Sister Dagmara Kuhl** (ED-145): Church Renegade. Confirmed for simulation use.
- **Theron Ault** (ED-146): Guilds Fixer. Confirmed for simulation use.
These characters are canonical for mechanical testing. Designer retains right to revise names, backstory, and Beliefs before player-facing publication.
[FLAGGED: confirm canonical adoption or request full redesign.]

## ED-154 through ED-162 Review Flag Resolution (PP-300)
All provisional patches from previous session confirmed as applied:
- ED-154 (Community Weaving two-form): PP-247 confirmed.
- ED-155 (Discipline asymmetry threshold): PP-248 confirmed — strict Power > Power.
- ED-156 (BG Overwhelming 2×Ob + floor 3): PP-249 confirmed.
- ED-157 (AMPLIFY cap ×2): PP-250 confirmed.
- ED-158 (Scene→Mass modifiers): PP-251 confirmed.
- ED-159 (Domain Echo clock magnitudes): PP-252 confirmed — Overwhelming ±3, Success ±2 accepted as working values pending playtesting.
- ED-160 (Collective Forgetting Anchor 1:1): PP-253 confirmed.
- ED-162 (Mending RS gain by scale): provisional scale progression confirmed (+1/+1/+2/+3/+4/+5 per tier). Subject to playtesting calibration.
[FLAGGED: all to be revisited after first full campaign playtest.]

---
<!-- PP-301 2026-04-04: Defector Cultivation Domain Action (PROVISIONAL ED-171) -->
<!-- PP-302 2026-04-04: Intelligence Asset Neutralisation (PROVISIONAL ED-172) -->

## Defector Cultivation (PP-301) [PROVISIONAL — ED-171]
Special Domain Action available to Crown and Varfell only.
Pool: Influence. TN 7. Ob = target faction Intel + 2.
- Success: gain one hidden data point about target faction per season.
- Overwhelming: permanent +1D on all future intelligence actions against that faction.
- Failure: defector burns; target faction learns an approach was made (Mandate −1 own faction).
Frequency: once per season per target faction. Cannot be combined with standard Investigate.
[FLAGGED: confirm faction availability and Ob values before compilation.]

## Intelligence Asset Neutralisation (PP-302) [PROVISIONAL — ED-172]
Extension of Niflhel/Quiet assassination mode; available to any faction with Intel stat.
Target: a documented intelligence asset (report, dossier, evidence cache) rather than a person.
- Asset location known: Pool = Intel. Ob = target faction Intel + 2. Success: asset destroyed.
- Asset location unknown: Pool = Intel. Ob = target faction Intel + 4. Partial: location revealed (can target next season). Failure: no effect.
[FLAGGED: confirm Ob values (+2/+4 differential).]

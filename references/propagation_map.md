...
## `registers/patch_register_index.md` retired — 2026-08-23 (S6/6f, ED-IN-0194)

**Not a patch change; a generated snapshot removed.** The file was a rendered index of the patch
register, *"auto-generated by index_gen.py — 2026-05-10"*, listing 199 entries. `index_gen.py` went
to the fork in S6/6a, so since then it was a three-month-old rendering that nothing could refresh —
and `registers/patch_register_active.yaml`, the register it indexed, remains live and authoritative
at 5,300 / 15,000 tokens.

**Propagation surface: none.** No PP moved, no PP status changed, no citation was rewritten. It is
recorded here because `ci_co_file_checker` rule 2 matches on the `registers/patch_register` path
PREFIX and therefore fires on the index as well as on the active register. The rule is correct to
be broad and was left alone: narrowing a blocking gate so that one commit stops tripping it is how
gates die, and the honest cost of the breadth is this paragraph.

Resolve the old path through `references/restructure_ledger.md` (`FORK:baf29d5`).

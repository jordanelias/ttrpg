...
- **Phase-6 item 6d LANDED — the `beneficiary:` column (`ED-IN-0248`, closes `CAT-2` in code).**
  ...
  **5,345 candidates · 3,796 (71.0%) resolve a personal beneficiary · ZERO declared-but-unresolved.**
  ⚠ **That zero is STRUCTURAL, not measured** (corrected 2026-09-20, `ED-IN-0257`; this read
  *"that zero is the result"*). `actor` → `p.id`, `subject` is set by `opening_set` on every
  ...
  `ED-IN-0257`): `convictions` has no verb producer either and is term 1 of the live score. `Person`

# HANDOFF_IN — HISTORY (pre-2026-09-13 narrative)

**Lane:** `IN`. **Split out of `HANDOFF_IN.md` on 2026-09-17 (`ED-IN-0240`), on Jordan's instruction:** *"TRIM THE HANDOFFS"* — given across multiple sessions and not acted on, because each session adopted `ED-IN-0221`'s move-only-marker-free predicate, found it exhausted, and reported back instead of trimming.

## What this file is

The dated IN-lane narrative from before the current generation, **verbatim and in original order**. It holds units that carry OPEN markers as well as finished ones — which is why it is called history and not `_closed` (`CLAUDE.md` §4: a file named *closed* holding open items is the vocabulary failure that rule forbids). `HANDOFF_IN_closed.md` beside it keeps its own meaning: work the marker predicate proved finished.

**Every open marker in here is indexed in `HANDOFF_IN.md`, verbatim.** Orient from that index, not from this file.

## The predicate, stated so it can be re-run

A unit moved here if its NEWEST date is before **2026-09-13**, the current generation. Undated units stayed in the live file — an undated unit cannot be shown stale. The closed-work index stayed. Granularity is `ED-IN-0221`'s: per SECTION, except the `Pending` / `Decisions` / `Next actions` logs, which split per ENTRY because they label every entry.

⚠ **43 of the 65 marker-carrying units in the pre-trim file were pre-September, worth 49,075 tokens of 82,021.** Age is why they moved; it is NOT a finding that they are dead. Each still needs §0's five-step gate run on it.
...
# PASS 2 — the session narrative of the current generation

Moved 2026-09-17 on Jordan's *"why wouldn't you rely on the markers themselves instead of the body for a handoff"*. These sections are recent; recency is not a reason for a BODY to sit in a continuity surface. Their markers are in `HANDOFF_IN.md`'s index.
...
## Pre-pointer body of HANDOFF_IN.md, moved verbatim 2026-09-22
...
- **Phase-6 item 6e LANDED — the FIRST write to any `Person` interior field in the tree
  ...
  *written at WITNESS* to *written at RESOLVE by the outcome that names the person*, S9.3 being the
  ...
  held `ED-IN-0248` and `§7.3e`; this took `ED-IN-0249` and `§7.3f`, and `next_free` jumped to 250.
  ⚠ While that PR was unmerged the id was deliberately NOT spelled here — a citation whose ledger
  row the tree does not carry is NONEXISTENT to `validate_ed_citations.py`. #416 merged first, so
  both rows are now in the ledger and both citations resolve.

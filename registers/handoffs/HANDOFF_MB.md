...
| Squad-engagement concept v5: build slate (Part A) + all six Part C items ruled by Jordan 2026-09-25 | `ED-MB-0067` (4th row) / `proposals/2026-09-25-squad-engagement-synthesis.md` | d.1 is a NEW derivation at `massbattle.py:_faction_to_unit` (PP-711 was never implemented, not an existing formula to edit) — replace its hardcoded `morale=5`/`morale_start=5` with a value from `Faction.Sta`; `mass_battle_v30.md` is reference, not the fix; work out the C2/C3 coupling formula and C4's roll |

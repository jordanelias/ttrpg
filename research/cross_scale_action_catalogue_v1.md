...
| **Negotiate Quota** (Levy method) | 2 | Charisma/Cognition + history vs Local Actors as a mass social contest | §7 | **BI** | Converts variable Levy into a fixed multi-season **Compact** (4–6 seasons), carried as a `Debt` subtype per §2.4. Castilian *encabezamiento*. Grants Local Actors one Petition per term the governor must Hold Court on. ED-SE-0019 |
...
The enum is closed at five (`systems/settlements/sim/ledger.py:30`). A **recurring term-limited
claim — a Compact — is a `Debt` subtype**, `Debt(key="compact:<quota>", ttl=term, recurs=True)`,
which is where the "fires every season of its term" behaviour lives; ED-IN-0046 D3 ruled this
2026-07-13 (`registers/supersession_register.yaml:406-418`). Four fiscal proposals — Encabezamiento,
Salt Certificate, State Arsenal, Borrow — were adjudicated before that ruling propagated and are
**unjudged** against the `Debt` form.

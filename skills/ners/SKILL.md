...
| **B — the resolution diagnostic** (the `resolution-diagnostic` skill, Phases 0–6) | the target resolves an outcome by a **draw** | stress points and property violations, against the **one** engine: σ-leverage μ-shift over the d10 substrate, fractional pool and fractional Ob |

Instrument B lives in its own skill and **is evidence, not a verdict.** Invoke
`resolution-diagnostic` when the target rolls; its Phase 0–6 findings come back here and enter the
ledger below — they do not short-circuit it. A target with no draw runs instrument A alone — that is
a normal NERS pass, not an out-of-scope one.

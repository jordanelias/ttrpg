...
- **Distinguish from:** Lispector's repetition-as-ontological-intensity and Ishiguro's repetition-with-variation, which return to the same scene with new detail. These carry difference; hammering does not.
...
- **Mannerism risk:** Austere negation-dialectic. Once: the contradiction holds ("I can't go on. I'll go on."). Repeatedly: every sentence becomes affirmation-negation toggle.
- **Deployment constraint:** Beckett's permutational enumeration, anaphoric litany, and dialectical succession all carry *difference*. Each item in a Beckett series differs from the last. If the prose cycles the same word or clause without variation, it has fallen into iteration (see I.5), not Beckett. Beckett's minimalism is economical, not empty — every word displaced from its expected position does work.

### V.10 Lem
**Primary:** Q2. The clinical-substrate author.
- **Mannerism risk:** Scientific-clinical register applied to everything. Once: the precision illuminates the alien. Repeatedly: the prose reads as a lab report.
- **Deployment constraint:** Lem's technique is the scholarly apparatus confronting something that doesn't yield to the apparatus. The precision is the point — flawless description that doesn't quite track because the object exceeds the frame. At Q2 specifically (high C, high TS), Lem occupies the middle layer of the within-observation gradient (III.1). The clinical register must be motivated by the PC's training or disposition, not applied generically.

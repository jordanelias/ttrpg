...
Read the following files from the working tree (use the Read tool) before running any compliance check. The checkout is authoritative — do not fetch from GitHub and do not work from memory. If a listed file is absent from the working tree, stop and report it.
...
Do not use remembered Foundations content — the working-tree file is authoritative.
...
4. For FAIL: cite specific Foundations text (by section, from the read file), explain philosophical reasoning, propose repair
...
**Foundations ref:** §[N], key concept [cite from the read file]
...
- All Foundations citations must reference the read file, not memory.

...
**Re-read after writes:** after any commit, re-read all modified files from the working tree before referencing them again. The in-context version and the on-disk version may differ.

- Patch log is append-only (never delete entries; mark reverted if needed)
- All source values cited from the working-tree files

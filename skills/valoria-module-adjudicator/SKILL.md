...
This skill reads canonical architecture docs; never adjudicate from memory or chat-session maps. Read the following from the working tree (use the Read tool), index-first; the checkout is authoritative — do not fetch from GitHub and do not work from memory. If a listed file is absent from the working tree, stop and report it: `designs/architecture/key_substrate_v30.md` (§2.3, §4.1, §4.6, §8), `designs/architecture/key_type_registry_v30.md`, `designs/architecture/scale_transitions_v30.md` (§3, §5), `designs/scene/derived_stats_v30.md` (§1, §11, §14 — PROPOSAL status; re-verify before relying), plus the target module's canonical doc via `references/canonical_sources.yaml` lookup.
...
Operator provides: one module, a scale slice (e.g., "everything at scene scale"), or `full graph`. Resolve target docs via `canonical_sources.yaml` in the working tree.

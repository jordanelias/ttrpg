...
This skill reads the engine mechanics from a **canonical reference baseline** AND the **session-local target**; never diagnose from *memory* — read the actual artifact, never a remembered or hallucinated version. Read the canonical reference baseline below from the working tree (use the Read tool); the checkout is authoritative — do not fetch from GitHub and do not work from memory. If a listed file is absent from the working tree, stop and report it. Then read the target (which may be session-local work, not canon — see the last bullet):

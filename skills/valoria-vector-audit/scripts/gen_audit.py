...
THE CORE PARTITION (get this right — it is the whole point of the tool):
  ...
  **Tie-break, made explicit (not hidden):** the AUTHORITATIVE historical
  ...
  `systems/factions/reference/faction_systems_overview_v30.md`, a registered REFERENCE head,
...
def _leading_status_token(s):
    ...
    and `systems/_architecture/reference/governance_consolidation_v1.md`'s
...
def run(root, out):
    ...
    L.append('These are flagged because their `## Status:` HEADING leads with CANONICAL/CURRENT, and '
             ...
             '`systems/factions/reference/fractional_province_ownership_v30.md`). "Declares a canonical-family '

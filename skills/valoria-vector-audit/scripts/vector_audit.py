...
def _canonical_paths(root):
    ...
    for p in ('canon/00_philosophical_foundations.md',
              ...
              '.designs/systems/_architecture/reference/complete_systems_reference.md'):
        paths.add(p)
    return sorted(paths)
...
# .designs/systems/_architecture/reference/throughlines_complete.md carries a SECOND, richer throughline→systems membership than
...
def parse_throughlines_complete(root):
    ...
    .designs/systems/_architecture/reference/throughlines_complete.md (whole doc — T-01..T-41, letter-suffixed T-15b/T-15c included; the
    ...
    # MOVED 2026-08-23 (S6/6f): references/ -> systems/_architecture/. It is a design artifact —
    # the map of every causal chain crossing 3+ systems — and was under references/ by accident of
    # history. Note what the `return []` below does on a wrong path: this source contributes NO
    ...
    fp = root / '.designs' / 'systems' / '_architecture' / 'reference' / 'throughlines_complete.md'
    if not fp.exists():
        return []
    rows = []

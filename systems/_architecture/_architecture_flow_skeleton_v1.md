...
| `TickScheduler.emit` | `engine/substrate/keys.py:510 emit` | `engine/cross_scale/echo_transport.py:343`, `:438`; `systems/factions/sim/faction_action.py:414`; `systems/factions/sim/parliamentary_transfer.py:240` |
...
| `echo_transport.classify_scene_outcome` | `engine/cross_scale/echo_transport.py:144 classify_scene_outcome` | `engine/cross_scale/echo_transport.py:448` (inside `emit_scene_echo`, def at `:360`; internal) |
| `echo_transport.make_scheduler` | `engine/cross_scale/echo_transport.py:183 make_scheduler` | `engine/mc_v18.py:243` |
| `echo_transport.emit_scene_echo` | `engine/cross_scale/echo_transport.py:370 emit_scene_echo` | `engine/cross_scale/scene_dispatch.py:392` (inside `_resolve_slot`); `engine/cross_scale/parliamentary_bridge.py:183` (inside `run_parliamentary_scene`, def at `:180`) |
...
| Cooked Key-type registry (`key_types.json`) | file | `tools/export_key_types.py` cook of `systems/_architecture/key_type_registry_v30.md` | `engine/cross_scale/echo_transport.py:78-79 _REGISTRY_PATH` |
...
| `engine_clock` — the `module_contracts.yaml`-declared contract this scheduler notionally realizes — carries `sim_module: none`; no code was ever joined to `TickScheduler`/`KeyLog` under that contract name | `references/module_contracts.yaml:908-911` |
...
| `engine_clock`'s declared emit `mechanical.season_change` is itself a dangling emit — the only `from: [engine_clock]` consumer row in the file is for its sibling `mechanical.accounting`. | `references/module_contracts.yaml:921 mechanical.season_change`; `references/module_contracts.yaml:127` |

...
| `domain_echo.compute_accord_echo` | `engine/cross_scale/domain_echo.py:128 compute_accord_echo` | `engine/cross_scale/echo_transport.py:452` (inside `emit_scene_echo`, def at `:360`) |
...
| `echo_transport.classify_scene_outcome` | `engine/cross_scale/echo_transport.py:146 classify_scene_outcome` | `engine/cross_scale/echo_transport.py:450` (inside `emit_scene_echo`, def at `:360`; internal) |
  ...
  - S3.2 `scene_dispatch.run_scene_phase(world, rng)` — `engine/mc_v18.py:141`
    ...
    - S3.2.2 `[loop]` `dispatch_scenes` drains `scene_slate` one slot at a time — `engine/cross_scale/scene_dispatch.py:403-415`
      ...
      - S3.2.2.4 `[branch]` if `world.echo_scheduler` present: `echo_transport.emit_scene_echo(scene_type, result, ctx, world)` — `engine/cross_scale/scene_dispatch.py:392-395`
        ...
        - S3.2.2.4.4 `[gate]` `scope_met` (read at `engine/cross_scale/echo_transport.py:392`, threaded into `compute_domain_echo` at S3.2.2.4.2) gates the entire Accord leg below — `engine/cross_scale/echo_transport.py:449`
        - S3.2.2.4.5 `[gate]` `classify_scene_outcome` requires an explicit caller-declared `echo['scene_outcome']` — `engine/cross_scale/echo_transport.py:450`, `engine/cross_scale/echo_transport.py:166-172`
        - S3.2.2.4.6 `[branch]` if classified: `domain_echo.compute_accord_echo` — `engine/cross_scale/echo_transport.py:452`
...
| registration (was lateral) | `systems.social_contest.sim.parliamentary_vote` (SC) | `run_parliamentary_vote` — **no longer imported**: `engine/` names the ROLE `parliamentary_vote` and `references/module_contracts.yaml` names the module (plan S5a, 2026-08-22) | `references/module_contracts.yaml:84 parliamentary_vote`; resolved at `engine/cross_scale/parliamentary_bridge.py:169-170 run_parliamentary_scene` |
| registration (was lateral) | `systems.factions.sim.parliamentary_transfer` (FA) | `propose_transfer` — by ROLE. The **CB-availability helpers are no longer reached at all**: the derivation that read them moved into that module as `derive_transfer_candidate` (plan S5a) | `references/module_contracts.yaml:98 territory_transfer_proposal`; resolved at `engine/cross_scale/parliamentary_bridge.py:138-143 _run_transfer_motion` |
...
| `engine_clock` — the `module_contracts.yaml`-declared contract this scheduler notionally realizes — carries `sim_module: none`; no code was ever joined to `TickScheduler`/`KeyLog` under that contract name | `references/module_contracts.yaml:992-995` |
...
| `engine_clock`'s declared emit `mechanical.season_change` is itself a dangling emit — the only `from: [engine_clock]` consumer row in the file is for its sibling `mechanical.accounting`. | `references/module_contracts.yaml:1005 mechanical.season_change`; `references/module_contracts.yaml:211` |

...
from the geography YAML — `engine/autoload/game_state.py:286 create_world`, called at
`engine/mc_v18.py:232 run_campaign`; settlements down-seam `engine/autoload/game_state.py:286-287 create_world`.
...
`CampaignResult.final_state` alongside the telemetry counters — `engine/autoload/game_state.py:337 serialize_world`,
...
| a | The world save/restore **read** direction is test-only: the write half runs every campaign, the read half has no production caller | characters, world, settlements | `engine/autoload/game_state.py:407 restore_world` |
...
| g | Two uncoordinated settlement-scale entity families are built in the same world-creation call from two different sources, cross-validated by nothing but a report-only probe | settlements, world | `engine/autoload/game_state.py:286-287 create_world` |

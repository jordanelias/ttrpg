...
| `subscribe_all(scheduler)` | `engine/cross_scale/articulation.py:152 subscribe_all` | `engine/mc_v18.py:266 subscribe_all` (production); `engine/tests/test_pipeline_reach.py:677 subscribe_all` and `:681 run_campaign` (test reach probes); `tests/valoria/test_articulation_subscriber.py:233 subscribe_all` (unit tests) |
...
| `scheduler` (`TickScheduler`) | arg | `world.echo_scheduler`, built by `echo_transport.make_scheduler` | `engine/mc_v18.py:266 subscribe_all` |
| `ECHO_TRANSPORT` (params/env flag; default ON) | flag | `effective_params` / `os.environ` | `engine/mc_v18.py:65-75 _echo_transport_on` (definition; call site `engine/mc_v18.py:249 _echo_transport_on`) |
...
- **S1** `[gate]` Campaign boot resolves the `ECHO_TRANSPORT` flag (params override, else env var,
  default `'1'`). Resolution logic: `engine/mc_v18.py:65-75 _echo_transport_on`; called at
  `engine/mc_v18.py:249 _echo_transport_on`
  - **S1.1** `[branch]` If ON: attach `world.echo_scheduler = echo_transport.make_scheduler(...)`.
    `engine/mc_v18.py:251 make_scheduler`
  - **S1.2** `[write]` `subscribe_all(world.echo_scheduler)` runs once at boot, inside the same
    `if _echo_transport_on(...)` branch. `engine/mc_v18.py:266 subscribe_all`
...
| `stubwire.invocations` delta | telemetry counter | `CampaignResult.stub_hits` | `engine/mc_v18.py:308 stub_hits` |
...
| `out` | `engine.mc_v18` (campaign driver) | Telemetry only: the campaign reads the `stubwire.invocations` delta this subsystem's fired callbacks contribute to, into `CampaignResult.stub_hits`. No state crosses — the counter is the entire payload | `engine/mc_v18.py:308 stub_hits` |

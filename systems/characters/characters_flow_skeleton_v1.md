...
| `conviction.ConvictionState.to_dict` | `systems/characters/sim/conviction.py:125 to_dict` | `engine/autoload/game_state.py:337 serialize_world` (duck-typed via `hasattr`) |
| `conviction.ConvictionState.from_dict` | `systems/characters/sim/conviction.py:136 from_dict` | `engine/autoload/game_state.py:407-408 restore_world` — see §7 gap 4 |
...
| `beliefs.Belief.to_dict` | `systems/characters/sim/beliefs.py:60 to_dict` | `engine/autoload/game_state.py:337-338 serialize_world` (duck-typed via `hasattr`) |
| `beliefs.Belief.from_dict` | `systems/characters/sim/beliefs.py:78 from_dict` | `engine/autoload/game_state.py:407 restore_world` — see §7 gap 4 |
...
| `world.convictions` | registry | `engine/autoload/game_state.py:264 convictions` field | `systems/characters/sim/conviction.py:78 _store` |
| `world.beliefs` | registry | `engine/autoload/game_state.py:265 beliefs` field | `systems/characters/sim/beliefs.py:108 _store` |
...
| snapshot dict entries `'convictions'`, `'beliefs'` | world-state (deserialized) | `engine/autoload/game_state.py:407 restore_world` argument | `engine/autoload/game_state.py:407 restore_world`, `engine/autoload/game_state.py:407 restore_world` |
...
- S7.1 `create_world` builds a fresh `World`; `convictions`/`beliefs` default to empty dicts and are not populated during world-gen. `engine/autoload/game_state.py:238-239 World`, `engine/autoload/game_state.py:286-297 create_world`
- S7.2 `[write]` At the end of every campaign run, `serialize_world` duck-type-calls `.to_dict()` on every value in `world.convictions` / `world.beliefs` (empty in production — see §7 gap 2) into the snapshot dict. `engine/autoload/game_state.py:337-341 serialize_world`
...
- S8.1 `[branch]` If `'convictions'` is present in the snapshot, late-import `ConvictionState` and rebuild `world.convictions` via `from_dict`. `engine/autoload/game_state.py:407-410 restore_world`
- S8.2 `[branch]` If `'beliefs'` is present, late-import `Belief` and rebuild `world.beliefs` via `from_dict`. `engine/autoload/game_state.py:407-410 restore_world`
...
| `world.convictions` (dict of `ConvictionState`) | world-state (write) | `engine/autoload/game_state.py` snapshot | `engine/autoload/game_state.py:337-338 serialize_world` |
| `world.beliefs` (dict of list[`Belief`]) | world-state (write) | `engine/autoload/game_state.py` snapshot | `engine/autoload/game_state.py:337-339 serialize_world` |
...
| `world.convictions` | RW | `engine/autoload/game_state.py` (field), `systems/characters/sim/conviction.py` (accessor) | `engine/autoload/game_state.py:244`, `systems/characters/sim/conviction.py:75-80 _store` |
...
| down | `engine.autoload` (core) | `game_state.py` imports `ConvictionState`/`Belief` directly to deserialize a campaign snapshot | `engine/autoload/game_state.py:407-414 restore_world` |

...
| 3. Code↔contract divergence: `references/module_contracts.yaml`'s `piety_track` entry declares `conviction.py` as a Key-substrate consumer of 9 Key types and an emitter of `state.scar_acquired`, but `conviction.py`/`beliefs.py`/`companion.py` contain no Key subscription, `echo_scheduler`/`TickScheduler` reference, or `.emit(` call anywhere — the module is a plain function-call API. | `references/module_contracts.yaml:302`, `references/module_contracts.yaml:309-320` (`sim_module: .../conviction.py`, `consumes:` 9 entries, `emits: state.scar_acquired`); `grep -rn "scar_acquired\|echo_scheduler\|TickScheduler\|\.emit(" systems/characters/sim/*.py` → no matches. |

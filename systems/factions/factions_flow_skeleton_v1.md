...
|---|---|---|
| `faction_take_action(faction, world, rng)` | `systems/factions/sim/faction_action.py:191 faction_take_action` | `engine/mc_v18.py:130 faction_take_action` (inside `_faction_actions_callback`) |
...
**S2** `[emit-dispatch]` Call `faction_take_action(faction, world, world.rng)`, guarded per-faction by a try/except that logs to stderr rather than aborting the season — `engine/mc_v18.py:130-136 faction_take_action`.
  ...
  **S2.1** Compute four RNG-free state signals: conquest targets/mil-advantage, undergoverned share, threat — `systems/factions/sim/faction_action.py:206-210 targets`.

  **S2.2** `[gate]` Turn signals into per-bucket multipliers, re-weight the four base weights, renormalize to a probability vector — `systems/factions/sim/faction_action.py:213-226 conquest_mult`.

  **S2.3** `[gate]` Single draw `roll = rng.random()` against the cumulative thresholds — `systems/factions/sim/faction_action.py:234 roll`.

  **S2.4** `[branch]` `roll < cum_unique` → `_try_faction_unique` — `systems/factions/sim/faction_action.py:237-241 _try_faction_unique`.
    ...
    - **S2.4.3** `[branch]` If still NOOP, fall through to Conquest — `systems/factions/sim/faction_action.py:241` (fall through).

  **S2.5** `[branch]` `roll < cum_conquest` → `_try_conquest` — `systems/factions/sim/faction_action.py:244-247 _try_conquest`.
    - **S2.5.1** `[gate]` No-op unless a reachable target exists — `systems/factions/sim/faction_action.py:437`. The second half of this gate, `faction.Mil >= CONQUEST_MIN_MIL`, was DELETED 2026-08-14 by Jordan ruling; a faction with any Military may open a conquest and the battle decides it.
    - **S2.5.2** `[lateral]` Resolve via `mass_battle.sim.massbattle.resolve_mass_battle` — `systems/factions/sim/faction_action.py:445-452 resolve_mass_battle`.
    - **S2.5.3** `[emit]` Emit `scene.battle_concluded` Key (log-only, no `apply=`) — `systems/factions/sim/faction_action.py:473 _emit_battle_concluded`.
...
| `Faction.territories` mutation | world-state write | same victory check; also read back by `_conquest_targets`/`_undergoverned_share` next season | `systems/factions/sim/faction_action.py:484-485 territories.append` |
...
| `world.battle_count` increment | world-state write | no reader found in this trace | `systems/factions/sim/faction_action.py:509 battle_count` |
| `scene.battle_concluded` Key | emit (log-only) | declared 4 consumers in `references/key_graph.json` per in-code comment, but no live subscriber found — see §7 | `systems/factions/sim/faction_action.py:473 _emit_battle_concluded` |
...
| `Faction.Sta` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:562 adjust` |
| `Faction.W` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:529-532 adjust` |
| `Faction.I` | R | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:554 faction.I` |
...
| `Territory.accord` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:496 adjust_accord` |
| `Territory.garrison` | W | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:486 garrison` |
| `Territory.entry_terms_l_seed` | W (dynamic, not a dataclass field) | none — see §7 | `systems/factions/sim/faction_action.py:503 entry_terms_l_seed` |
...
| `world.battle_count` | W | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:509 battle_count` |
...
| `world._battle_key_seq`, `world._parl_key_seq` | RW (dynamic attrs) | none (ad hoc counters) | `systems/factions/sim/faction_action.py:368-369 _battle_key_seq` |
...
| lateral | `mass_battle` | `resolve_mass_battle(faction_a, faction_b, terrain, world)` for Conquest resolution | `systems/factions/sim/faction_action.py:445-452 resolve_mass_battle` |
...
| Mass-battle resolution for Conquest always passes a permanent placeholder for terrain, never a real value. | `systems/factions/sim/faction_action.py:450 terrain` |

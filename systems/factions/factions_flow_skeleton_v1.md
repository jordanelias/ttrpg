...
| `world.territories`, `ADJACENCY` | world-state | `systems/settlements/sim/adjacency.py` | `systems/factions/sim/faction_action.py:135-140 ADJACENCY` |
...
**S2** `[emit-dispatch]` Call `faction_take_action(faction, world, world.rng)`, guarded per-faction by a try/except that logs to stderr rather than aborting the season — `engine/mc_v18.py:137-144 composition.require`.
  ...
  **S2.1** Compute four RNG-free state signals: conquest targets/mil-advantage, undergoverned share, threat — `systems/factions/sim/faction_action.py:212-216 targets`.

  **S2.2** `[gate]` Turn signals into per-bucket multipliers, re-weight the four base weights, renormalize to a probability vector — `systems/factions/sim/faction_action.py:219-232 conquest_mult`.

  **S2.3** `[gate]` Single draw `roll = rng.random()` against the cumulative thresholds — `systems/factions/sim/faction_action.py:240 roll`.

  **S2.4** `[branch]` `roll < cum_unique` → `_try_faction_unique` — `systems/factions/sim/faction_action.py:243-247 _try_faction_unique`.
    - **S2.4.1** `[branch]` Dispatch by `faction.name`: `Crown` → `crown_initiative.select_initiative_mode` then one of 3 modes; `Church` → priority chain Excommunication → Council → Absolution; any other name → immediate NOOP — `systems/factions/sim/faction_action.py:294-335 _faction_specific_unique`.
    - **S2.4.2** `[branch][gate]` If the faction-specific chain returned NOOP and `faction.parliamentary`, fall back to `parliamentary_action.propose_censure` — `systems/factions/sim/faction_action.py:278-282 propose_censure`.
    - **S2.4.3** `[branch]` If still NOOP, fall through to Conquest — `systems/factions/sim/faction_action.py:247` (fall through).

  **S2.5** `[branch]` `roll < cum_conquest` → `_try_conquest` — `systems/factions/sim/faction_action.py:250-253 _try_conquest`.
    - **S2.5.1** `[gate]` No-op unless a reachable target exists — `systems/factions/sim/faction_action.py:443`. The second half of this gate, `faction.Mil >= CONQUEST_MIN_MIL`, was DELETED 2026-08-14 by Jordan ruling; a faction with any Military may open a conquest and the battle decides it.
    - **S2.5.2** `[lateral]` Resolve via `mass_battle.sim.massbattle.resolve_mass_battle` — `systems/factions/sim/faction_action.py:451-458 resolve_mass_battle`.
    - **S2.5.3** `[emit]` Emit `scene.battle_concluded` Key (log-only, no `apply=`) — `systems/factions/sim/faction_action.py:479 _emit_battle_concluded`.
    - **S2.5.4** `[branch][write]` On attacker win: transfer territory ownership, apply a loser Legitimacy penalty; then fork Terms (`deg == 'Success'`) vs Storm on Accord penalty and an `entry_terms_l_seed` proxy write — `systems/factions/sim/faction_action.py:467-503 attacker_wins`.

  **S2.6** `[branch][write]` `roll < cum_muster` → `_try_muster`: pay Wealth up front, roll `pool = Mil + floor(W / MUSTER_WEALTH_TO_POOL_DIV)`, apply Mil gain on success — `systems/factions/sim/faction_action.py:256-259 _try_muster`, `systems/factions/sim/faction_action.py:520-548 _try_muster`. Also reached by fall-through when S2.5's `_try_conquest` returns the `_NOOP` sentinel — since `cum_muster >= cum_conquest`, the same `roll` still satisfies this step's own condition, the analogous fall-through documented for the faction-unique slot at S2.4.3 — `systems/factions/sim/faction_action.py:250-259` (fall through), sentinel return at `systems/factions/sim/faction_action.py:443-444 _NOOP`.

  **S2.7** `[branch][write]` Unconditional terminal call — not gated on any threshold, reached whenever S2.4-S2.6 all failed to return a non-sentinel result → `_try_govern`: roll `pool = faction.I` against a fixed Ob, apply Accord gain or Stability loss — `systems/factions/sim/faction_action.py:262 _try_govern`, `systems/factions/sim/faction_action.py:551-570 _try_govern`. The three preceding branches at S2.4/S2.5/S2.6 are independent `if` statements, not `elif` — a roll landing in an early bucket whose branch returns the sentinel falls through every remaining bucket to this line, and the return here is itself not checked by the caller — see §7 for the compound cascade and its reachability.
...
| `Faction.territories` mutation | world-state write | same victory check; also read back by `_conquest_targets`/`_undergoverned_share` next season | `systems/factions/sim/faction_action.py:490-491 territories.append` |
...
| `world.battle_count` increment | world-state write | no reader found in this trace | `systems/factions/sim/faction_action.py:515 battle_count` |
| `scene.battle_concluded` Key | emit (log-only) | declared 4 consumers in `references/key_graph.json` per in-code comment, but no live subscriber found — see §7 | `systems/factions/sim/faction_action.py:479 _emit_battle_concluded` |
...
|---|---|---|---|
| `Faction.L` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:477 adjust` |
| `Faction.Sta` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:568 adjust` |
| `Faction.W` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:533-536 adjust` |
| `Faction.I` | R | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:560 faction.I` |
| `Faction.Mil` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:530 adjust` |
| `Faction.standing` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/crown_initiative.py:97 standing` |
| `Faction.territories` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:484-485 territories` |
...
| `Territory.owner` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:482 owner` |
| `Territory.accord` | RW | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:502 adjust_accord` |
| `Territory.garrison` | W | `engine/autoload/game_state.py` | `systems/factions/sim/faction_action.py:492 garrison` |
| `Territory.entry_terms_l_seed` | W (dynamic, not a dataclass field) | none — see §7 | `systems/factions/sim/faction_action.py:509 entry_terms_l_seed` |
...
| lateral | `mass_battle` | `resolve_mass_battle(faction_a, faction_b, terrain, world)` for Conquest resolution | `systems/factions/sim/faction_action.py:451-458 resolve_mass_battle` |
...
| in/out | `engine` substrate (Key log) | `scene.battle_concluded` / `da.public_governance` Key emission, gated on `world.echo_scheduler` | `systems/factions/sim/faction_action.py:354-356 echo_scheduler` |
...
| Mass-battle resolution for Conquest always passes a permanent placeholder for terrain, never a real value. | `systems/factions/sim/faction_action.py:456 terrain` |

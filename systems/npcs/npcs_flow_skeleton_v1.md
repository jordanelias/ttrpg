...
| `world.npc_counter` increment | write | `world` state; read back as `CampaignResult.npcs_generated` telemetry | `systems/world/sim/npe.py:107-109 _next_npc_id` / `engine/mc_v18.py:307 npcs_generated` |
...
|---|---|---|---|
| `World.npcs` | RW | `engine/autoload/game_state.py` (declared) / `systems/world/sim/npe.py` (read+written) | `engine/autoload/game_state.py:210 npcs` |
| `World.npc_counter` | RW | `engine/autoload/game_state.py` (declared) / `systems/world/sim/npe.py` (incremented) | `engine/autoload/game_state.py:211 npc_counter` |
...
| serialized `snapshot['npcs']` / `snapshot['npc_counter']` | W (serialize) / R (restore) | `engine/autoload/game_state.py` | `engine/autoload/game_state.py:328-331 npc_counter` / `engine/autoload/game_state.py:401-404 restore_world` |

## 6. Seams

| Direction | Peer | Mechanism | Anchor |
|---|---|---|---|
| up | `engine/autoload/game_state` | `World.npcs`/`.npc_counter` registry fields owned by game_state, populated by npe | `engine/autoload/game_state.py:210-211 npcs` |
...
| declared, unimplemented | `systems/factions` (npc_behavior contract's `consumes`/`emits` Key edges: `scene.gossip`, `scene.witness`, `state.opinion_revised`, etc.) | contract declares edges against a `sim_module: none` — see §7 gap 3 | `references/module_contracts.yaml:255 npc_behavior` / `references/module_contracts.yaml:291 npc_behavior` |

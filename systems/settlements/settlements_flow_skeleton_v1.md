...
|---|---|---|
| `populate_from_geography(world, path=None)` | `systems/settlements/sim/registry.py:215 populate_from_geography` | `engine/autoload/game_state.py:331 create_world` (by the `world_gen_settlements` role since plan S5a, not imported) |
...
| `serialize_world(world)` settlements branch | `engine/autoload/game_state.py:397 settlements` | `engine/mc_v18.py:315 serialize_world` (end of `run_campaign`) |
| `restore_world(snapshot)` settlements branch | `engine/autoload/game_state.py:486-488 settlements` | no production caller found (save/restore round-trip exercised only by `engine/tests/test_world_population.py`) |
...
| `ACCORD_MAP` / `STARTING_ACCORD` / `STARTING_OWNER` | `param` | `engine/autoload/game_state.py` module constants, used to build `Territory` objects (a separate, uncoordinated source from the geography YAML — see §7) | `engine/autoload/game_state.py:294-304 Territory` |
...
|---|---|---|---|
| `world.settlements: dict[sid, Settlement]` | `registry` | `serialize_world`, `province_members`/`province_accord` readers, `echo_transport._apply_accord_echo` | `engine/autoload/game_state.py:279 settlements` |
...
| `world.territory_infrastructure[tid]: InfrastructureState` | `registry` | `count_infrastructure`/`seizure_ob_modifier` readers (`mass_seizure.py`), `serialize_world`/`restore_world` | `engine/autoload/game_state.py:469-471 InfrastructureState` |
...
|---|---|---|---|
| `World.settlements` | `RW` | `engine/autoload/game_state.py` (declared), written by `registry.py` | `engine/autoload/game_state.py:279 settlements` |
| `World.territory_infrastructure` | `RW` | `engine/autoload/game_state.py` (declared), written by `infrastructure.py` | `engine/autoload/game_state.py:268 territory_infrastructure` |
| `World.npc_drift_state` | `RW` (declared only — no live writer other than the module fallback; see §7) | `engine/autoload/game_state.py` (declared) / `temperaments.py` (`_drift_store`) | `engine/autoload/game_state.py:269 npc_drift_state` |
...
| `Territory.accord` | `RW` | `engine/autoload/game_state.py` (dataclass); written directly by `systems/factions/sim/parliamentary_transfer.py:344` and `systems/factions/sim/mass_seizure.py:293`, bypassing `Settlement.order` entirely; read (never written) by the S5 drift probe | `engine/autoload/game_state.py:215 accord` |

## 6. Seams

| Direction | Peer | Mechanism | Anchor |
|---|---|---|---|
| `up` | `engine/autoload/game_state.py` (World) | `world.settlements` / `world.territory_infrastructure` / `world.npc_drift_state` registries declared on `World`, populated/serialized by settlements-owned modules | `engine/autoload/game_state.py:248-259` |
...
| `restore_world`'s `settlements` branch has no production caller — only exercised by `engine/tests/test_world_population.py`'s explicit serialize/restore round-trip | `engine/autoload/game_state.py:489-491 restore_world`; `engine/tests/test_world_population.py:1-18` |
...
| **The `settlement_layer` contract's `executes:false` flag is correct for the file it names, but that file is the wrong one.** Its declared `sim_module` is `settlement.py`, the orphan this skeleton independently proves has zero importers; the live registry code (`registry.py`) that actually runs at boot and every `loop.s3` has no contract entry of its own. The contract pointing at the dead file while the live file goes uncontracted is the real defect. | `references/module_contracts.yaml:823-826 settlement_layer`; `systems/settlements/sim/settlement.py:95 compute_settlement_state`; `systems/settlements/sim/registry.py:215 populate_from_geography` |

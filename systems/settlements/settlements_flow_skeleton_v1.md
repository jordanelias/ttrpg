...
|---|---|---|
| `populate_from_geography(world, path=None)` | `systems/settlements/sim/registry.py:215 populate_from_geography` | `engine/autoload/game_state.py:283 create_world` (by the `world_gen_settlements` role since plan S5a, not imported) |
...
| `ACCORD_MAP` / `STARTING_ACCORD` / `STARTING_OWNER` | `param` | `engine/autoload/game_state.py` module constants, used to build `Territory` objects (a separate, uncoordinated source from the geography YAML — see §7) | `engine/autoload/game_state.py:248-256 Territory` |
...
|---|---|---|---|
| `world.settlements: dict[sid, Settlement]` | `registry` | `serialize_world`, `province_members`/`province_accord` readers, `echo_transport._apply_accord_echo` | `engine/autoload/game_state.py:233 settlements` |
...
|---|---|---|---|
| `World.settlements` | `RW` | `engine/autoload/game_state.py` (declared), written by `registry.py` | `engine/autoload/game_state.py:233 settlements` |
| `World.territory_infrastructure` | `RW` | `engine/autoload/game_state.py` (declared), written by `infrastructure.py` | `engine/autoload/game_state.py:222 territory_infrastructure` |
| `World.npc_drift_state` | `RW` (declared only — no live writer other than the module fallback; see §7) | `engine/autoload/game_state.py` (declared) / `temperaments.py` (`_drift_store`) | `engine/autoload/game_state.py:223 npc_drift_state` |
...
| `Territory.accord` | `RW` | `engine/autoload/game_state.py` (dataclass); written directly by `systems/factions/sim/parliamentary_transfer.py:344` and `systems/factions/sim/mass_seizure.py:293`, bypassing `Settlement.order` entirely; read (never written) by the S5 drift probe | `engine/autoload/game_state.py:169 accord` |

## 6. Seams

| Direction | Peer | Mechanism | Anchor |
|---|---|---|---|
| `up` | `engine/autoload/game_state.py` (World) | `world.settlements` / `world.territory_infrastructure` / `world.npc_drift_state` registries declared on `World`, populated/serialized by settlements-owned modules | `engine/autoload/game_state.py:202-213` |
...
| `restore_world`'s `settlements` branch has no production caller — only exercised by `engine/tests/test_world_population.py`'s explicit serialize/restore round-trip | `engine/autoload/game_state.py:441-443 restore_world`; `engine/tests/test_world_population.py:1-18` |

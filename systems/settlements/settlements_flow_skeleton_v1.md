...
|---|---|---|
| `populate_from_geography(world, path=None)` | `systems/settlements/sim/registry.py:215 populate_from_geography` | `engine/autoload/game_state.py:284 create_world` (by the `world_gen_settlements` role since plan S5a, not imported) |
| `register_settlement(s, world=None)` | `systems/settlements/sim/registry.py:171 register_settlement` | `systems/settlements/sim/registry.py:264 register_settlement` (only production call, from inside `populate_from_geography` itself); `register_settlement` (RETIRED 2026-08-21 → `FORK:1e4c6f4`, culling plan v1 wave 1/3; was tools/sim_harness/adapters/pr119_governance/pr119_integrated_campaign.py:112) (prototype harness, not wired to CI) |
...
| `province_accord(province_id, world=None)` | `systems/settlements/sim/registry.py:184 province_accord` | `systems/overview/sim/accounting.py:87 province_accord` (the only production caller — read-only probe); `province_accord` (RETIRED 2026-08-21 → `FORK:1e4c6f4`, culling plan v1 wave 1/3; was tools/sim_harness/adapters/pr119_governance/pr119_integrated_campaign.py:222) (prototype harness) |
| `province_effective_prosperity(province_id, world=None)` | `systems/settlements/sim/registry.py:193 province_effective_prosperity` | `—` (no production caller; `province_effective_prosperity` (RETIRED 2026-08-21 → `FORK:1e4c6f4`, culling plan v1 wave 1/3; was tools/sim_harness/adapters/pr119_governance/pr119_integrated_campaign.py:262) only) |
| `succeed_governor(sid, new_governor, world=None, season=0)` | `systems/settlements/sim/registry.py:198 succeed_governor` | `—` (no production caller; `succeed_governor` (RETIRED 2026-08-21 → `FORK:1e4c6f4`, culling plan v1 wave 1/3; was tools/sim_harness/adapters/pr119_governance/pr119_promote_ready_oversight.py:84) only) |
...
| `build_infrastructure(territory_id, infra_type, world=None)` | `systems/settlements/sim/infrastructure.py:139 build_infrastructure` | `—` (no importer found in `engine/`, `systems/`, `tests/`; `build_infrastructure` (RETIRED 2026-08-21 → `FORK:1e4c6f4`, culling plan v1 wave 1/3; was tools/sim_harness/adapters/pr119_governance/pr119_integrated_campaign.py:198-199) and `build_infrastructure` (RETIRED 2026-08-21 → `FORK:1e4c6f4`, culling plan v1 wave 1/3; was tools/sim_harness/adapters/pr119_governance/pr119_subnational_factions.py:98) (prototype harness, not wired to CI)) |
...
| `serialize_world(world)` settlements branch | `engine/autoload/game_state.py:350 settlements` | `engine/mc_v18.py:315 serialize_world` (end of `run_campaign`) |
| `restore_world(snapshot)` settlements branch | `engine/autoload/game_state.py:439-441 settlements` | no production caller found (save/restore round-trip exercised only by `engine/tests/test_world_population.py`) |
...
| `ACCORD_MAP` / `STARTING_ACCORD` / `STARTING_OWNER` | `param` | `engine/autoload/game_state.py` module constants, used to build `Territory` objects (a separate, uncoordinated source from the geography YAML — see §7) | `engine/autoload/game_state.py:249-257 Territory` |
...
- **S2.2 Scene → Domain Echo → settlement write (deferred)** — when `world.echo_scheduler` is set, `echo_transport.emit_scene_echo` runs (`engine/cross_scale/echo_transport.py:362`):
  - S2.2.1 `[gate]` requires an explicit `echo` block with `actor_faction`/`most_relevant_stat`; else returns `{}` (`engine/cross_scale/echo_transport.py:381-389`).
...
Two production sites write `Territory.accord` directly, never going through `Settlement.order` or `registry.province_accord`:
  - `systems/factions/sim/parliamentary_transfer.py:344` (terr.accord = ACCORD_MAP[accord_level]).
...
`run_campaign` calls `game_state.serialize_world(world)` (`engine/mc_v18.py:307`), which dict-serializes `world.settlements` via each `Settlement.to_dict()` (`engine/autoload/game_state.py:330-331`, `systems/settlements/sim/registry.py:112-133`) and carries `accord_drift_probe_hits` into `CampaignResult` (`engine/mc_v18.py:304`).

## 4. OUT

| Output | Kind | Consumer | Anchor |
|---|---|---|---|
| `world.settlements: dict[sid, Settlement]` | `registry` | `serialize_world`, `province_members`/`province_accord` readers, `echo_transport._apply_accord_echo` | `engine/autoload/game_state.py:234 settlements` |
...
| `world.territory_infrastructure[tid]: InfrastructureState` | `registry` | `count_infrastructure`/`seizure_ob_modifier` readers (`mass_seizure.py`), `serialize_world`/`restore_world` | `engine/autoload/game_state.py:422-424 InfrastructureState` |
...
|---|---|---|---|
| `World.settlements` | `RW` | `engine/autoload/game_state.py` (declared), written by `registry.py` | `engine/autoload/game_state.py:234 settlements` |
| `World.territory_infrastructure` | `RW` | `engine/autoload/game_state.py` (declared), written by `infrastructure.py` | `engine/autoload/game_state.py:223 territory_infrastructure` |
| `World.npc_drift_state` | `RW` (declared only — no live writer other than the module fallback; see §7) | `engine/autoload/game_state.py` (declared) / `temperaments.py` (`_drift_store`) | `engine/autoload/game_state.py:224 npc_drift_state` |
...
| `Settlement.order` | `RW` | `registry.py` (dataclass); written by `echo_transport._apply_accord_echo`'s deferred `_apply` (dormant, S2.2.4); read by `registry.province_accord` | `systems/settlements/sim/registry.py:65 order`, `engine/cross_scale/echo_transport.py:337-343` |
...
| `Territory.accord` | `RW` | `engine/autoload/game_state.py` (dataclass); written directly by `systems/factions/sim/parliamentary_transfer.py:344` and `systems/factions/sim/mass_seizure.py:293`, bypassing `Settlement.order` entirely; read (never written) by the S5 drift probe | `engine/autoload/game_state.py:170 accord` |

## 6. Seams

| Direction | Peer | Mechanism | Anchor |
|---|---|---|---|
| `up` | `engine/autoload/game_state.py` (World) | `world.settlements` / `world.territory_infrastructure` / `world.npc_drift_state` registries declared on `World`, populated/serialized by settlements-owned modules | `engine/autoload/game_state.py:203-214` |
...
| `lateral` | `engine/cross_scale/echo_transport.py` | Personal-scene Accord Echo resolves a target `Settlement` by id and writes `Settlement.order` through a deferred Key (`sched.emit`), reachable-but-dormant absent a `scene_outcome`-declaring caller | `engine/cross_scale/echo_transport.py:293-345` |
...
| `lateral` | `systems/factions/sim/parliamentary_transfer.py` | Writes `Territory.accord` directly on a successful transfer (not through settlements) | `systems/factions/sim/parliamentary_transfer.py:344` |
...
| `restore_world`'s `settlements` branch has no production caller — only exercised by `engine/tests/test_world_population.py`'s explicit serialize/restore round-trip | `engine/autoload/game_state.py:442-444 restore_world`; `engine/tests/test_world_population.py:1-18` |
...
| **The `settlement_layer` contract's `executes:false` flag is correct for the file it names, but that file is the wrong one.** Its declared `sim_module` is `settlement.py`, the orphan this skeleton independently proves has zero importers; the live registry code (`registry.py`) that actually runs at boot and every `loop.s3` has no contract entry of its own. The contract pointing at the dead file while the live file goes uncontracted is the real defect. | `references/module_contracts.yaml:819-822 settlement_layer`; `systems/settlements/sim/settlement.py:95 compute_settlement_state`; `systems/settlements/sim/registry.py:215 populate_from_geography` |

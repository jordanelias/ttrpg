...
| `STARTING_OWNER` / `STARTING_STATS` / `STARTING_ACCORD` / `STARTING_PT` / `STARTING_GARRISON` | registry | module-level starting-state tables | `engine/autoload/game_state.py:44-91` |
...
| `world.insurgencies` / `world.uncontrolled_streaks` | world-state | `World` dataclass fields | `engine/autoload/game_state.py:183-184` |
| `world.npcs` / `world.npc_counter` | world-state | `World` dataclass fields | `engine/autoload/game_state.py:185-186` |
...
| `world.insurgencies` / `world.uncontrolled_streaks` mutations | world-state | `serialize_world` → `CampaignResult.final_state`; `mc_v18.insurgencies_formed` telemetry | `engine/autoload/game_state.py:294-298`, `engine/mc_v18.py:298` |
...
| `world.npcs` / `world.npc_counter` mutations | world-state | `serialize_world` → `final_state`; `mc_v18.npcs_generated` telemetry | `engine/autoload/game_state.py:299-302`, `engine/mc_v18.py:299` |
| serialized snapshot `dict` | file/registry | `CampaignResult.final_state`; any save-game caller | `engine/autoload/game_state.py:262`, `engine/autoload/game_state.py:272`, `engine/mc_v18.py:307` |
...
|---|---|---|---|
| down | `systems.settlements.sim.registry` | `create_world` calls `populate_from_geography(world)` to build `world.settlements` | `engine/autoload/game_state.py:257-258` |
...
| lateral | `systems.factions.sim` (`parliamentary_transfer`, `mass_seizure`, `faction_action`) | Write `Territory.owner`/`.accord` directly on the world-owned dataclass; this subsystem's own code only ever reads those fields | `systems/factions/sim/parliamentary_transfer.py:278`, `systems/factions/sim/parliamentary_transfer.py:293`, `systems/factions/sim/mass_seizure.py:292`, `systems/factions/sim/mass_seizure.py:295`, `systems/factions/sim/faction_action.py:468` |
...
| in | `engine.autoload.game_state` (`restore_world`) | Late-imports `systems.world.sim.insurgency_pipeline.InsurgencyRecord` and `systems.world.sim.npe.NPC` to reconstruct registries from a snapshot | `engine/autoload/game_state.py:369`, `engine/autoload/game_state.py:376` |

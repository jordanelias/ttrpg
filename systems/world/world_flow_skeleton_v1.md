...
|---|---|---|
| `create_world(seed)` | `engine/autoload/game_state.py:236 create_world` | `engine/mc_v18.py:232 run_campaign` |
...
| `canonical_pt(continuous_pt)` | `engine/autoload/game_state.py:76 canonical_pt` | `systems/factions/sim/mass_seizure.py:256 resolve_mass_seizure`, `systems/overview/sim/ci_track.py:133 compute_seasonal_ci_delta` |

## 2. IN

| Input | Kind | Origin | Anchor |
|---|---|---|---|
| `seed: int \| None` | arg | caller of `create_world` | `engine/autoload/game_state.py:236 create_world` |
| `snapshot: dict` | arg | caller of `restore_world` | `engine/autoload/game_state.py:354 restore_world` |
| `STARTING_OWNER` / `STARTING_STATS` / `STARTING_ACCORD` / `STARTING_PT` / `STARTING_GARRISON` | registry | module-level starting-state tables | `engine/autoload/game_state.py:48-95` |
| `world.territories` (dict) | world-state | `World` dataclass | `engine/autoload/game_state.py:190 World.territories` |
...
| `world.insurgencies` / `world.uncontrolled_streaks` | world-state | `World` dataclass fields | `engine/autoload/game_state.py:189-190` |
| `world.npcs` / `world.npc_counter` | world-state | `World` dataclass fields | `engine/autoload/game_state.py:189-190` |
...
- **S1** `engine.mc_v18.run_campaign` calls `game_state.create_world(seed)` to build the
  ...
  - **S1.1** `[write]` Builds `Faction`/`Territory` maps from the `STARTING_*` module tables.
    `engine/autoload/game_state.py:240-257 create_world`
  - **S1.2** `[write]` Initializes `World.clocks` (CI/MS/IP/PI/Strain/Turmoil). `engine/autoload/game_state.py:268 create_world`
  - **S1.3** `[emit]` Down-seam: calls `systems.settlements.sim.registry.populate_from_geography(world)`
    to populate `world.settlements` before returning. `engine/autoload/game_state.py:281-282 create_world`
...
|---|---|---|---|
| `World` instance | world-state | `engine.mc_v18.run_campaign` and everything downstream in the season loop | `engine/autoload/game_state.py:236 create_world`, `engine/mc_v18.py:224` |
...
| serialized snapshot `dict` | file/registry | `CampaignResult.final_state`; any save-game caller | `engine/autoload/game_state.py:266`, `engine/autoload/game_state.py:276`, `engine/mc_v18.py:307` |
...
| `canonical_pt(continuous_pt) -> int` | registry (leaf fn) | production bucketing callers in factions and overview | `engine/autoload/game_state.py:76 canonical_pt`, `systems/factions/sim/mass_seizure.py:256 resolve_mass_seizure`, `systems/overview/sim/ci_track.py:133 compute_seasonal_ci_delta` |

## 5. State touched

| Field | R/W/RW | Owning module | Anchor |
|---|---|---|---|
| `World.territories` | W (created) | `engine.autoload.game_state` | `engine/autoload/game_state.py:246-257 create_world` |
...
| `World.clocks` | W (created) | `engine.autoload.game_state` | `engine/autoload/game_state.py:268 create_world` |
| `World.settlements` | W (created, via down-seam) | `engine.autoload.game_state` → `systems.settlements.sim.registry` | `engine/autoload/game_state.py:281-282 create_world` |
...
|---|---|---|---|
| down | `systems.settlements.sim.registry` | `create_world` calls `populate_from_geography(world)` to build `world.settlements` | `engine/autoload/game_state.py:261-262` |

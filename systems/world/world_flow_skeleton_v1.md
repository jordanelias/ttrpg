...
|---|---|---|
| `create_world(seed)` | `engine/autoload/game_state.py:237 create_world` | `engine/mc_v18.py:232 run_campaign` |
| `serialize_world(world)` | `engine/autoload/game_state.py:285 serialize_world` | `engine/mc_v18.py:315 run_campaign` |
| `restore_world(snapshot)` | `engine/autoload/game_state.py:355 restore_world` | — (no production caller; only `engine/tests/test_world_population.py:82`) |
...
| `canonical_pt(continuous_pt)` | `engine/autoload/game_state.py:80 canonical_pt` | `systems/factions/sim/mass_seizure.py:256 resolve_mass_seizure`, `systems/overview/sim/ci_track.py:133 compute_seasonal_ci_delta` |

## 2. IN

| Input | Kind | Origin | Anchor |
|---|---|---|---|
| `seed: int \| None` | arg | caller of `create_world` | `engine/autoload/game_state.py:237 create_world` |
| `snapshot: dict` | arg | caller of `restore_world` | `engine/autoload/game_state.py:355 restore_world` |
| `STARTING_OWNER` / `STARTING_STATS` / `STARTING_ACCORD` / `STARTING_PT` / `STARTING_GARRISON` | registry | AUTHORED tables — `references/world_initial_state.yaml` since plan S5b, cooked by `tools/export_world_initial_state.py` and read by `engine/substrate/world_initial_state.py`; `game_state` re-exports them under the same names | `engine/autoload/game_state.py:53-55 world_initial_state`, `engine/autoload/game_state.py:95-97 world_initial_state` |
| `world.territories` (dict) | world-state | `World` dataclass | `engine/autoload/game_state.py:191 World.territories` |
...
| `world.insurgencies` / `world.uncontrolled_streaks` | world-state | `World` dataclass fields | `engine/autoload/game_state.py:190-191` |
| `world.npcs` / `world.npc_counter` | world-state | `World` dataclass fields | `engine/autoload/game_state.py:190-191` |
...
- **S1** `engine.mc_v18.run_campaign` calls `game_state.create_world(seed)` to build the
  ...
  - **S1.1** `[write]` Builds `Faction`/`Territory` maps from the `STARTING_*` module tables.
    `engine/autoload/game_state.py:241-258 create_world`
  - **S1.2** `[write]` Initializes `World.clocks` (CI/MS/IP/PI/Strain/Turmoil). `engine/autoload/game_state.py:269 create_world`
  - **S1.3** `[emit]` Down-seam: calls `systems.settlements.sim.registry.populate_from_geography(world)`
    to populate `world.settlements` before returning. `engine/autoload/game_state.py:282-283 create_world`
...
- **S6** `[write]` At campaign end (winner found or season cap reached), `run_campaign` calls
  ...
  `engine/mc_v18.py:315 run_campaign` → `engine/autoload/game_state.py:285 serialize_world`
  - **S6.1** `[gate, default-off]` `restore_world` exists as the inverse of S6 but is exercised only
    by its own round-trip test, never by a production caller. `engine/autoload/game_state.py:355 restore_world`

## 4. OUT

| Output | Kind | Consumer | Anchor |
|---|---|---|---|
| `World` instance | world-state | `engine.mc_v18.run_campaign` and everything downstream in the season loop | `engine/autoload/game_state.py:237 create_world`, `engine/mc_v18.py:224` |
...
| `world.insurgencies` / `world.uncontrolled_streaks` mutations | world-state | `serialize_world` → `CampaignResult.final_state`; `mc_v18.insurgencies_formed` telemetry | `engine/autoload/game_state.py:297-301`, `engine/mc_v18.py:298` |
...
| `world.npcs` / `world.npc_counter` mutations | world-state | `serialize_world` → `final_state`; `mc_v18.npcs_generated` telemetry | `engine/autoload/game_state.py:302-305`, `engine/mc_v18.py:299` |
| serialized snapshot `dict` | file/registry | `CampaignResult.final_state`; any save-game caller | `engine/autoload/game_state.py:267`, `engine/autoload/game_state.py:277`, `engine/mc_v18.py:307` |
...
| `canonical_pt(continuous_pt) -> int` | registry (leaf fn) | production bucketing callers in factions and overview | `engine/autoload/game_state.py:80 canonical_pt`, `systems/factions/sim/mass_seizure.py:256 resolve_mass_seizure`, `systems/overview/sim/ci_track.py:133 compute_seasonal_ci_delta` |

## 5. State touched

| Field | R/W/RW | Owning module | Anchor |
|---|---|---|---|
| `World.territories` | W (created) | `engine.autoload.game_state` | `engine/autoload/game_state.py:247-258 create_world` |
...
| `World.clocks` | W (created) | `engine.autoload.game_state` | `engine/autoload/game_state.py:269 create_world` |
| `World.settlements` | W (created, via down-seam) | `engine.autoload.game_state` → `systems.settlements.sim.registry` | `engine/autoload/game_state.py:282-283 create_world` |
...
|---|---|---|---|
| down | `systems.settlements.sim.registry` | `create_world` calls `populate_from_geography(world)` to build `world.settlements` | `engine/autoload/game_state.py:262-263` |
...
| in | `engine.autoload.game_state` (`restore_world`) | Late-imports `systems.world.sim.insurgency_pipeline.InsurgencyRecord` and `systems.world.sim.npe.NPC` to reconstruct registries from a snapshot | `engine/autoload/game_state.py:372`, `engine/autoload/game_state.py:379` |

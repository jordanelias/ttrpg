...
## 2026-09-16 — two modules COPIED OUT of the audit archive (ED-IN-0231)

`audit/` became `.audit/`, a hidden tree, and a hidden tree must not be load-bearing: a dependency
reaching into it is invisible to anyone searching normally. The two that did were copied out —
**copied, not moved**, because deleting from a historical record destroys evidence, so the archived
originals remain as history and are read by nothing.

| new live path | archived original | read by |
|---|---|---|
| `engine/reference/contest-groundup/engine.py` | `.audit/2026-06-03-contest-groundup/engine.py` | `tools/gen_sigma_parity_goldens.py`, which regenerates `engine/tests/goldens/sigma_leverage_parity.json` for the **blocking** `Sim Reference Regression` job |
| `tests/sim/reverse_pair_symmetry.py` | `.audit/2026-07-22-mass-battle-stress-test/reverse_pair_symmetry.py` | `tests/valoria/test_gauge_invariants.py`, which imported it via a `sys.path` insert into the archive |

The oracle's destination was not chosen here: `tools/evacuation_plan.py`'s **R-REL-ORACLE** had
already ruled `engine/reference/contest-groundup/` and carried the execution steps. Those were
followed, and the golden regenerated byte-identically apart from the one line recording its source —
all 1,758 parity rows unchanged.

⚠ **Both landed inside `ci_sim_fabrication_check`'s scan scope, which `.audit/` was not.** Nothing
about either file's code changed; they simply became visible to a gate they had never faced. Their
constants were traced to live owners and cited — see
`engine/reference/contest-groundup/sim_verification_ledger.json`. **Two could not be traced and are
recorded as oracle-local rather than given an invented source:** `OB_MAX = 20`, which has no other
owner anywhere in the tree, and `OVERWHELM_SIGMA = 0.85`, whose live copy at
`systems/social_contest/sim/contest/degree_extension.py:46` cites *this oracle* as its source. The
second one matters beyond bookkeeping: it means the file is not merely a frozen reference, it is a
canonical source that shipped code points at.

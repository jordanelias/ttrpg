...
LINE_ASPECT = 1.4           # generator: Line width:depth ratio (from the per-tier tables' progression)
...
PC_NODE_COHESION = _sigma_os.environ.get('PC_NODE_COHESION', '0') == '1'   # step 2: node-relational cohesion (cells=nodes at live positions, held by relational offsets); default OFF -> byte-exact
...
PC_KITE_STANDOFF = 5  # [class-B] kiter retreat-trigger (Chebyshev distance): open the gap when nearest enemy is nearer than this; hold+volley in [this, VOLLEY_MAX_RANGE]; close when beyond. Calibrate by measurement. §13.

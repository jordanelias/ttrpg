...
#
# Original: .audit/2026-07-22-mass-battle-stress-test/reverse_pair_symmetry.py
# Read by:  tests/valoria/test_gauge_invariants.py, which imports `symmetry` and `SIGMA_GATE` from it
#
# It imports `gauge_mb`, which already lives in tests/sim/, so the copy sits beside its own
# dependency and the sys.path insert into the archive disappears.
#
...
def symmetry(sa, sb, ka, kb, mode='multi', n=60, seed_base=1_000_000):
    ...
    def _se(res):
        ...
        if k <= 0:
            return None
        p = res['decA'] / 100.0
        return 100.0 * math.sqrt(max(p * (1 - p), 1e-9) / k)  # [canonical: n/a §numerical-floor — 1e-9 keeps the sqrt argument positive at p in {0,1}; not a game quantity]
...
if __name__ == '__main__':
    ...
    ap.add_argument('--n', type=int, default=60)  # [canonical: this module's docstring §The invariant — n=60 is named there as the authoritative run; a CLI default, not a mechanic]

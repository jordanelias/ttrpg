...
| T-02 | M-033 + M-038 (TT↔CI↔IP at triple threshold + seasonal accounting) | BG | BG | Global → Faction | Mode C scenario |
| T-03 | M-019 + M-009 + M-029 (Past-Oriented Pulling → Certainty collapse → The Forgetting) | TTRPG | TTRPG | Personal → Exploration | Mode B + D |
| T-04 | M-037 + M-031 + M-036 (Grand Debate → CI spike → Parliamentary Vote void) | HYB | BG | Faction → Global | Mode C |
...
**Tracks:** CE (both chars), TS, DD (Deniability Debt), CI, COMP
...
**Temporal:** PRES + FUT
**Tracks:** TT, CI, IP — all at threshold simultaneously
**Factions:** All
**NPC:** Baralta (CI suppressor), Almud (Royal Decree)
**Archetype:** Faction Leader ×3

### Scenario: Three Clocks Hit Threshold Simultaneously at Accounting

Starting state: TT 48, CI 49, IP 49 (all one step from major thresholds — TT 50, CI 50, IP 50).
...
Baralta CI Suppression: ACTIVE (Mandate 4+)
```

### Accounting Trigger Sequence

At accounting, clocks update based on:
- Church Mandate 5+ → CI +1 (rule: §8.3)
- Himlensendt institutional driver active → CI +1
...
The ruleset specifies TT↔CI↔IP cross-effects but does **not** specify resolution order when multiple thresholds are crossed in the same accounting phase.
...
**CI 50 effect**: Templars become deployable without royal authorisation. Church Military actions lose the "royal authorisation" constraint.
...
**Conflict:** CI 50 enables unilateral Templar deployment. IP 50 mandates a political response. If the Church deploys Templars as its IP-response action, that is legal under CI 50 but violates the IP response's implicit expectation of diplomatic engagement.

**Baralta's CI suppression** at Mandate 4+: suppresses CI at −1/season. But the threshold was crossed at accounting, *then* suppression would apply. Does suppression prevent the threshold effect? The rules are silent on whether suppression prevents threshold crossing vs. reduces the post-crossing value.

### Critical Ambiguity: Suppression Timing

| Interpretation A | CI grows to 50 → threshold fires → suppression reduces to 49 (threshold already fired) |
|---|---|
| **Interpretation B** | CI grows to 49 + Baralta suppresses to 48 → threshold does NOT fire |
...
Starting from TT 28, CI 15, IP 20 (Session Zero values), reaching all three at 48–49 simultaneously requires ≈ 20–25 seasons of campaign play. This is late-game, but the late game is where BG mode is decided. **Frequency: low but peak-stakes.**
...
**P2:** Crown's primary counter to CI threshold crossing has only 45% success rate at exactly the moment it's most needed (consecutive use fatigue). This feels designed, but the interaction between Decree fatigue and threshold timing means the Crown's ability to suppress CI crossing is significantly worse in late-game than early-game — which may be intended as escalation design, but should be confirmed.

### Findings Summary
| # | Severity | Finding |
|---|----------|---------|
| 1 | **P1** | Baralta CI suppression timing vs. threshold crossing: no rule specifies whether suppression prevents crossing or applies after. Decisive military consequence. |
...
| 3 | P2 | CI 50 + IP 50 together allow Church to use Templar deployment as its mandatory IP response — diplomatically contradictory but mechanically legal |
...
## T-04: GRAND DEBATE → CI SPIKE → PARLIAMENTARY VOTE VOID
**Mechanics:** M-037 (Grand Debate), M-031 (Theocracy Clock), M-036 (Parliamentary Vote)
**Mode:** Hybrid → Board Game
**Temporal:** PRES + FUT
**Tracks:** CI, PI (Parliament Integrity), FSTAT (Church Mandate, Crown Mandate)
**Factions:** Crown, Church, Hafenmark
**NPC:** Himlensendt (Confessor), Almud (King), Baralta (Duchess)
**Archetype:** Faction Leader × 3

### Scenario: Inquisition Conviction → CI Spike Damages Parliament

An Inquisitor investigation reaches Stage 3 (Conviction Hearing — Grand Debate). A practitioner PC is convicted. Per §13.6: Conviction → CI +2. Parliament Integrity is currently 5 (degraded from starting 7).

Following conviction: Church Mandate +1 (institutional victory). Himlensendt drives CI further per his CI generation rule: Church Mandate 5+ → CI +1/season at accounting.
...
**Post-conviction:** CI +2. Current CI 22 → 24.

### CI Spike Into Parliament Integrity Interaction

At Parliament Integrity 5: Parliamentary Votes operate at standard Ob (per §14.1). But **what does CI 24 do to Parliamentary function?**

The CI threshold effects in the ruleset:
- CI 50: Templars deployable without royal auth
- CI 75: Forced prayer obligations
- CI 100: Full theocracy

CI 24 is mid-range. **No explicit PI degradation trigger from CI** is listed at this level.
...
In Hybrid mode, the Grand Debate resolves in TTRPG (full exchange narration). The CI +2 and Church Mandate +1 feed forward into the BG faction sheet.

**BG accounting at next season:**
- Church Mandate 6 now (was 5 post-conviction +1)
- CI generation: Church Mandate 5+ → CI +1. With Mandate 6: does this apply twice? The rule says "Mandate 5+" not "Mandate 5 exactly." With Mandate 6, the base generation still fires once.
- Baralta CI suppression: −1/season if Mandate 4+. Her Mandate is 4 — suppression holds.

**Net CI movement per season:** +1 (Church base) − 1 (Baralta suppression) = net 0. CI stable at 24.

**P3:** A conviction that cost CI +2 and elevated Church Mandate is neutralised by Baralta's passive suppression within one season. This may feel anticlimactic — the narrative weight of conviction doesn't produce lasting CI movement if Baralta remains functional.
...
| 3 | P3 | Baralta suppression neutralises conviction CI spike within 1 season — narrative/mechanical weight mismatch |
| 4 | P3 | CI 24 has no explicit Parliamentary effects — mid-range CI may need some friction effects below threshold |
...
If Church detects the Gap: Investigation opens. Church Intel +1D vs Varfell for 1 season. CI +1 (heresy evidence in ducal possession).

**Interaction chain:** Vaynard uses Collection → Discovery Event → Shifting Object deteriorates → Gap forms → Church Intel detects → CI +1 → Vaynard's TK advances (he now knows what he has is dangerous) → TK 3 → 4 (CI +2).

**CI total from this chain:** CI 22 + 1 (detection) + 2 (TK 4 advancement) = CI 25.

**This is a single Collection use that produces a 3-step CI cascade.** The rules contain no warning about this interaction. A player deploying the Collection repeatedly in mid-game could inadvertently drive CI 15 points in a few seasons.

**P1 finding:** Private Collection → Shifting Object deterioration → Gap → Church detection → CI cascade is not modelled as a risk in the Collection's rules text. Players are warned only about "+1 TS per use" not about the structural risk of the objects themselves.
...
| # | Severity | Finding |
|---|----------|---------|
| 1 | **P1** | Private Collection repeated use → Shifting Object deterioration → Gap → CI cascade not modelled as risk in Collection rules text |
...
**Tracks:** TD, TT, CI, TS (all participants), COMP, Knot strain
...
If Church detects: CI +2 (major practitioner operation in public space), CI 22 → 24. Investigation opened.

**BG-mode collective operation visibility:** In BG mode, does a Structural-scale operation automatically trigger Church detection, or is it a contested roll? The distinction matters: if automatic, Restoration can never do Structural work without a CI cost, which may make M-055 non-viable in BG. **P2.**
...
| ID | Test | Finding |
|----|------|---------|
| F-01 | T-02 | Baralta CI suppression timing vs. threshold crossing: no resolution — decisive military consequence |
...
| F-04 | T-06 | Private Collection → Shifting Object decay → Gap → CI cascade: risk not modelled in Collection rules |
...
| F-11 | T-02 | CI 50 + IP 50: Templar deployment as mandatory IP response legally allowed but diplomatically contradictory |
...
| ID | Test | Finding |
|----|------|---------|
| F-25 | T-02 | CI 24 has no Parliamentary friction effects |
| F-26 | T-02 | Baralta suppression neutralises conviction CI spike in 1 season — weight mismatch |
| F-27 | T-04 | CI 24 below all named thresholds — mid-range clock may need some effects |

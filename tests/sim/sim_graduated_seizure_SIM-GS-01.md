...
- **Gate:** Post-CI 75 only (inherited from CI 75 seizure section)
- **At CI=75 (frozen):** Pool = Influence + 5 (constant)

---

## MODE A — ISOLATION TABLES

### A1. Pool vs CI (Influence=6, CV=3, Ob=4)
| CI | Pool | P(OW) | P(Suc) | P(Part) | P(Fail) | E[CV+] |
|-----|------|-------|--------|---------|---------|--------|
...
### A2. Pool vs CV (CI=75, Influence=6)
...
| CV | Ob | CI=75 | CI=80 | CI=85 | CI=90 | CI=99 |
|----|-----|-------|-------|-------|-------|-------|
...
### A4. Influence degradation impact (CI=75)
...
**D2. CV=0 territory:** Ob=7, Pool=11 (CI=75). P(Fail)=0.753. Effectively unsieizable — 75% Stability drain rate. CV=0 territories function as a natural structural firewall. Good design.
...
**D4. [FINDING D4-P2]:** At Influence=0, CI=75: Pool = floor(75/15) = 5D. Church cannot be mechanically locked out of seizure by Influence damage alone. CI/15 component provides a non-trivial floor. Casus Belli cost still applies per attempt.

**D5. [FINDING D5-P1 — P1]:** Graduated Seizure pre-CI 75 availability is AMBIGUOUS. PP-494 is filed under the CI 75 section, implying post-75 gate. But the formula (Influence + floor(CI/15)) is defined at CI=28 in params_board_game, suggesting it may be a general formula applicable earlier. If available pre-75, it interacts with standard Assert/Suppress and CI generation — major precedent implications.

**D6. CI frozen pool cap:** Post-freeze, floor(CI/15) = 5 (constant). Church pool caps at Influence + 5 = 11D forever. Seizure success removes territories from Conviction Yield pool (PP-473 dead zone), reducing future Conviction Yield. CI stays frozen regardless. Structurally self-consistent.
...
**L1. [FINDING L1-P2]:** Composite pool (stat + clock/15) has no precedent in any other faction action. Risk of players forgetting the CI/15 bonus (+5D at freeze). Faction mat reminder needed.
...
**L3. Stability sustainability:** At CV=3 targets (Influence=6, CI=75): expected drain 0.12 Stability/season — sustainable across a long game. At CV=0 targets: 0.75 Stability/season — unsustainable, eliminates Church in 6–7 seasons. Seizure strategy should rationally avoid CV=0 territories. Good natural deterrent.
...
**L5. [FINDING L5-P1 — P1]:** Battle trigger on Graduated Seizure success in contested territory is unspecified. Cascade Test 4 in bg_v05 establishes a precedent that CI-threshold seizure success triggers Battle when military units are present. PP-494 is silent on whether this applies to Graduated Seizure.

**L6. [FINDING L6-P2]:** Post-CI 75, mandatory Assert (PP-480, mandatory at CI 50–74) has no CI effect (CI frozen). Church must still spend the action but gains nothing. It is not specified whether mandatory Assert is suspended in the seizure phase. Significant action economy impact.
...
|----|----------|-------------|--------|
| D5 | P1 | Graduated Seizure pre-CI 75 availability ambiguous — no CI gate stated in PP-494 | ED required |
...
| D4 | P2 | Influence=0 does not lock out seizure — CI floor component still provides dice | Note |
...
| L6 | P2 | Mandatory Assert post-CI 75 has no CI effect — action economy waste | ED required |
...
|----|-------------|--------|
| ED-NEW-A | Graduated Seizure availability: pre-CI 75 vs post-CI 75 only | Simulation correctness |
...
| ED-NEW-E | Mandatory Assert (PP-480) suspension post-CI 75 | Action economy |
...
- Pool growth (Influence + floor(CI/15)) rewards early investment in Influence.
- Stability drain on failure is appropriately calibrated for median-CV targets (0.12/season).
- Post-freeze pool cap (11D) is clean: no runaway scaling after CI=75.
# SIM-MB-06 v14 — Phase / tick structure wired into simulation
# Session: 2026-05-13 | Iteration v13 -> v14
# [canonical: Jordan design — phase = gallop-and-charge cycle ≈ 6 ticks;
#  hook points for stamina, rally, reform, threadwork at phase boundaries]
#
# v14 changes (one structural addition, no combat-mechanic changes):
#
#   PHASE / TICK STRUCTURE.  Adds TICKS_PER_PHASE = 6 and a phase_boundary
#   function called every 6 ticks within run_battle. Per the historical
#   research (Operational Studies Group's "three-hex charge would take about
#   5 minutes" plus reform-to-position 1-2 ticks; hoplite phalanx exhaustion
#   bound ~10-15 minutes of sustained close combat before rotation/breakdown),
#   a phase = one gallop-and-charge cycle = approach + gallop + impact +
#   contact resolution + disengage/reform. The same 6-tick frame transfers
#   to infantry: ephodos → krousis → doratismos → othismos → pararrhexis.
#
#   The phase_boundary function runs six empty hook stubs in canonical order:
#     1. stamina_check   — depletion accumulated over the 6 ticks, applied
#     2. morale_check_phase — accumulated casualties + stamina vs threshold
#     3. rout_resolution — units with broken morale flee the field
#     4. rally_check     — non-routed units with degraded morale attempt rally
#     5. reform_check    — formation drift correction, broken sub-units re-cohere
#     6. threadwork_check — narrative/agent threadwork at phase seams
#
#   Empty hooks mean v14 has identical combat output to v13. Battery confirms
#   12/13 unchanged. The structure is the deliverable; the hooks land in
#   subsequent cycles.
#
#   The return dict from run_battle now includes 'phases' and 'tick_in_phase'
#   so callers can observe phase progression. Existing callers using only
#   'winner' and 'turns' are unaffected.
#
#   NOTE: depth-as-replacement (the v14 design I started writing earlier in
#   the session) was reverted. Historical research showed that depth's real
#   role is stamina-replacement, not casualty-replacement — rear ranks rotate
#   forward to give exhausted front-rankers a chance to recover. That belongs
#   on the stamina_check hook in a later cycle, not on a recalc_size override.
#
...
# Phase-boundary hooks. Each is called once per phase boundary with both units
# and the current phase index. v14 wires the structure; the hooks themselves
# are empty until later cycles (stamina, rally, reform, morale, threadwork).
# Order at a phase boundary:
#   1. stamina_check    — depletion accumulated over the 6 ticks, applied
#   2. morale_check     — accumulated casualties + stamina state vs threshold
#   3. rout_resolution  — units with broken morale flee the field
#   4. rally_check      — non-routed units with degraded morale attempt rally
#   5. reform_check     — formation drift, broken sub-units re-cohere
#   6. threadwork_check — narrative/agent threads that fire at phase seams
# [canonical: Jordan design — extension points for stamina, rally, reform,
#  threadwork at phase boundaries]
def stamina_check(unit_a, unit_b, phase_idx):  # noqa: ARG001
    """Empty hook — stamina depletion lands here in a later cycle."""
    pass

def morale_check_phase(unit_a, unit_b, phase_idx):  # noqa: ARG001
    """Empty hook — phase-boundary morale evaluation lands here.
    NOTE: per-tick morale degradation already runs at end of each tick
    (the existing morale logic at the end of the turn loop). This hook
    is for accumulated/phase-level morale state."""
    pass

def rout_resolution(unit_a, unit_b, phase_idx):  # noqa: ARG001
    """Empty hook — broken-morale units flee here. Pursuit damage applies."""
    pass

def rally_check(unit_a, unit_b, phase_idx):  # noqa: ARG001
    """Empty hook — non-routed units with degraded morale attempt to rally."""
    pass

def reform_check(unit_a, unit_b, phase_idx):  # noqa: ARG001
    """Empty hook — formation drift correction, broken sub-units re-cohere."""
    pass

def threadwork_check(unit_a, unit_b, phase_idx):  # noqa: ARG001
    """Empty hook — narrative/agent threadwork at phase seams."""
    pass
...
def run_battle(unit_a, unit_b, max_turns=15):
    """Run a battle to resolution or max_turns cap.

    v14 wires phase structure: every tick is one resolution cycle (volley +
    advance + engagement); every TICKS_PER_PHASE ticks marks a phase boundary
    where stamina, morale, rout, rally, reform, and threadwork hooks fire.
    ...
    for t in range(1, max_turns + 1):
        ...
        # v14: phase boundary — every TICKS_PER_PHASE ticks. Empty hooks for now;
        # this is where stamina/rally/reform/threadwork land in later cycles.

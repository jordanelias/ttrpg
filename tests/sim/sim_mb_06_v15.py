# SIM-MB-06 v15 — Stamina (G-1) + Rout-at-casualty-threshold (G-2)
# Session: 2026-05-13 | Iteration v14 -> v15
# [canonical: mass_battle_v30.md §A.4, §A.7, §A.12; params/mass_combat.md;
#  audit_sim_mb_06_v14.md §G-1, §G-2]
#
# v15 changes (two mechanisms on existing phase-boundary hooks):
#
#   G-1 STAMINA — unit-level stamina attribute (0-100). Drains per contact
#   tick at flat rate. Recovers at phase boundary proportional to formation
#   depth (rear ranks rotate to front — historical and genre-standard).
#   Low stamina degrades combat pool (-1D to -3D). Deeper formations sustain
#   longer: RF (7 rows, recovery 36/phase) outlasts HS (4 rows, recovery 18).
#   [ASSUMPTION: stamina constants (drain=20/tick, recovery=12/rank, pool
#    penalty thresholds 70/40/exhausted, max penalty -2D) are tuning
#    parameters — no canonical source specifies stamina values;
#    audit G-1 identifies the gap without prescribing numbers]
#
#   G-2 ROUT-AT-CASUALTY-THRESHOLD — phase-boundary morale check using
#   stamina state. Exhausted units (stamina=0) take -1 morale per phase
#   boundary. Exhausted units with ≥15% casualties lose the general's morale
#   floor (min becomes 0 instead of 1). This lets battles end at historical
#   casualty rates (~15-30%) rather than running to 100% destruction.
#   [canonical: mass_battle_v30.md §A.4 morale triggers, §A.12 rout/pursuit;
#    rout threshold concept from audit G-2; general morale floor §A.4]
#
#   Hooks populated: stamina_check, morale_check_phase, rout_resolution.
#   rally_check, reform_check, threadwork_check remain empty (future cycles).
#
#   Per-tick morale degradation (size thresholds) UNCHANGED — runs exactly
#   as in v14. Phase-boundary morale is ADDITIONAL (stamina-driven pressure).
#
# v14 (base): phase/tick structure, 6 empty hooks, TICKS_PER_PHASE=6.
#   NOTE: depth-as-replacement was reverted in v14. Depth's role is
#   stamina-replacement — rear ranks rotate forward to refresh exhausted
#   front-rankers. That mechanism now lands here on the stamina_check hook.
#
...
def morale_check_phase(unit_a, unit_b, phase_idx):  # noqa: ARG001
    """G-2: phase-boundary morale check.
    Exhausted units take additional morale hits at phase boundary.
    Floor override is casualty-based (same logic as per-tick).
    [canonical: mass_battle_v30.md §A.4 — morale floor 1 while general present;
    ...
    for u in [unit_a, unit_b]:
        ...
        # G-2: casualty-based floor override (same as per-tick)
        if loss_pct >= ROUT_FLOOR_LOSS_PCT:
            morale_floor = 0
        else:
            morale_floor = 1
...
class Unit:
    ...
    def base_combat_pool(self):
        ...
        # G-1: stamina penalty degrades pool when exhausted
        stam_pen = _stamina_pool_penalty(self.stamina)
        return max(1, min(self.size, self.command) + self.command + pen + stam_pen)
...
def run_battle(unit_a, unit_b, max_turns=36):
    """Run a battle to resolution or max_turns cap.

    v15: max_turns increased from 15 to 36 (6 phases). With stamina + rout
    mechanics, battles end via morale rout rather than 100% HP destruction,
    but need enough ticks for stamina differentiation to emerge (2-4 phases).
    ...
    for t in range(1, max_turns + 1):
        ...
        for u in [unit_a, unit_b]:
            ...
            cap = 3; adj = 0
            # Canonical size-based triggers (unchanged from v14)
            # [canonical: params/mass_combat.md §Morale Degradation Triggers —
            #  "Size < 50% max: −1", "Size < 25% max: −1 additional"]
            if u.size < u.size_max // 4 and u.size > 0: adj += -min(2, cap); cap -= 2
            elif u.size < u.size_max // 2: adj += -min(1, cap); cap -= 1
            # Morale floor = 1 while general present (canonical, unchanged)
            # [canonical: mass_battle_v30.md §A.4]

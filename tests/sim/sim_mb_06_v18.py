# SIM-MB-06 v18 — Continuous TroopCount + G-3 lethality recalibration
# Session: 2026-05-13 | Iteration v17 -> v18
...
#   LETHALITY_SCALE applied to all combat damage before HP application.
...
# ─── G-3 LETHALITY RECALIBRATION ────────────────────────────────────────────
# [canonical: Jordan direction — "3-phase battles should be damaging, not
#  eliminating"; "30% rout threshold, not 10-15%"]
# All combat damage (engagement + volley) scaled by this factor before HP
# application. Calibrated so 18 ticks (3 phases) of contact produce ~30%
# casualties on the losing side. Current unscaled damage: ~1.5 HP/tick/side.
# Target: ~0.33 HP/tick for loser. Scale factor: ~0.25.
# [ASSUMPTION: LETHALITY_SCALE is a tuning parameter — calibrated for
#  ~15% casualties per 3-phase engagement turn]
# [canonical: designs/provincial/mass_battle_v30.md §A.7 — battle turn structure]
LETHALITY_SCALE = 0.10

# ─── CASUALTY-BASED MORALE TRIGGERS (G-3) ───────────────────────────────────
# [canonical: designs/provincial/mass_battle_v30.md §A.4 morale triggers —
#  reinterpreted for continuous TroopCount and multi-turn battles.
#  "Size dropped below 50% of max: −1" → loss_pct >= 0.30.
#  "Size dropped below 25%: −1 additional" → loss_pct >= 0.50.]
# These REPLACE the integer Size-based triggers, which are too coarse at Size 4.
MORALE_LOSS_PCT_THRESHOLD = 0.30   # [canonical: designs/provincial/mass_battle_v30.md §A.4]
MORALE_HEAVY_LOSS_PCT_THRESHOLD = 0.50  # [canonical: designs/provincial/mass_battle_v30.md §A.4]
...
STAMINA_MAX = 100
STAMINA_DRAIN_PER_CONTACT_TICK = 16  # flat drain per tick when unit has contacts
...
def morale_check_phase(unit_a, unit_b, phase_idx):  # noqa: ARG001
    ...
    for u in [unit_a, unit_b]:
        ...
        morale_delta = min(ROUT_EXHAUSTION_MORALE_HIT, MORALE_PHASE_CAP)
        u.morale = max(1, u.morale - morale_delta)  # floor=1 here; per-tick overrides
...
def discipline_check_phase(unit_a, unit_b, phase_idx):  # noqa: ARG001
    ...
    a_loss = (unit_a.hp_max - unit_a.hp) / unit_a.h_per_size if unit_a.h_per_size else 0
    b_loss = (unit_b.hp_max - unit_b.hp) / unit_b.h_per_size if unit_b.h_per_size else 0
...
class Unit:
    ...
    def recalc_size(self):
        # v16: effective_size is continuous (float). Size is display/label only (int).
        # [canonical: derived_stats architecture — "TroopCount = Size × block_size",
        #  "Size = floor(TroopCount / block_size)"; HP maps to TroopCount at scale]
        self.effective_size = self.hp / self.h_per_size if self.h_per_size > 0 else 0.0
...
def run_battle(unit_a, unit_b, max_turns=18):
    ...
    for t in range(1, max_turns + 1):
        ...
        # G-1: per-tick stamina drain. Any contacts this tick means both units
        # are fighting — drain stamina for both engaged sides.
        if pairs:
            if not unit_a.routed:
                unit_a.stamina = max(0, unit_a.stamina - STAMINA_DRAIN_PER_CONTACT_TICK)
            if not unit_b.routed:
                unit_b.stamina = max(0, unit_b.stamina - STAMINA_DRAIN_PER_CONTACT_TICK)
        ...
        # v16: G-3 lethality recalibration — scale all damage before application.
        # [canonical: Jordan direction — "3-phase battles should be damaging, not eliminating"]
        scaled_dmg_a = result["dmg_a"] * LETHALITY_SCALE
        scaled_dmg_b = result["dmg_b"] * LETHALITY_SCALE
        scaled_vol_a = volley_dmg_a * volley_hp_scale(unit_a) * LETHALITY_SCALE
        scaled_vol_b = volley_dmg_b * volley_hp_scale(unit_b) * LETHALITY_SCALE
        unit_a.hp = max(0, unit_a.hp - scaled_dmg_a - scaled_vol_a)
        unit_a.recalc_size()
        unit_b.hp = max(0, unit_b.hp - scaled_dmg_b - scaled_vol_b)
        unit_b.recalc_size()
        # v18: discipline degradation moved to phase_boundary (D-6).
        # Per-tick integer Size changes are too rare at LETHALITY_SCALE=0.10.
        ...
        for u in [unit_a, unit_b]:
            ...
            cap = 3; adj = 0
            # v16: casualty-percentage morale triggers (replace integer Size triggers).
            # [canonical: mass_battle_v30.md §A.4 morale triggers — reinterpreted for
            #  continuous TroopCount; "30% rout threshold" per Jordan direction]
            # -2/tick at 30%: morale 6 → 0 in 3 ticks. Rout at ~33-36% casualties.
            loss_pct = 1.0 - (u.hp / u.hp_max) if u.hp_max > 0 else 1.0
            if loss_pct >= MORALE_HEAVY_LOSS_PCT_THRESHOLD and cap > 0:
                adj += -min(3, cap); cap -= min(3, cap)
            elif loss_pct >= MORALE_LOSS_PCT_THRESHOLD and cap > 0:
                adj += -min(2, cap); cap -= min(2, cap)
            # Morale floor: canonical floor = 1 while general present.
            # [canonical: mass_battle_v30.md §A.4 — "while general is present: Morale floor = 1"]
            # v16: floor drops to 0 at 30% casualties — army is breaking, general
            # can't hold them. Enables rout at ~30-35% casualties.
            if loss_pct >= MORALE_LOSS_PCT_THRESHOLD:
                morale_floor = 0
            else:
                morale_floor = 1
            u.morale = max(morale_floor, u.morale + adj)

...
def resolve_engagements(unit_a, unit_b, pairs):
    ...
    for p in pairs:
        ...
        # Bii pool: base × (atom_troops/unit_troops) × (engaged/atom_max_width)
        ...
        a_pool_raw = unit_a.base_combat_pool() * a_troops_frac * (a_engaged / a_width)
        b_pool_raw = unit_b.base_combat_pool() * b_troops_frac * (b_engaged / b_width)
    ...
    for t in range(1, max_turns + 1):
        ...
        for p in pre_pairs:
            for cell in p["a_cells"]:
                # Find local coords for this cell

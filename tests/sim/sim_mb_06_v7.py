...
# Phase E exploration: tip-support constraint
# Fast cells (speed > min_speed in pattern) cannot advance more than TIP_SUPPORT_GAP
# offsets past the slowest non-zero-speed cell. Affects Arrowhead only currently.
TIP_SUPPORT_ENABLED = True  # [canonical: structural — Phase E exploration]
TIP_SUPPORT_GAP = 2  # X=1 strict-wedge, X=2 default (no-op currently), [canonical: structural — Phase E exploration]
...
class Atom:
    ...
    def advance_cells(self, discipline: int, target_centroid: Optional[Tuple[float, float]]):
        ...
        Phase E: tip-support constraint — fast cells cannot advance >X past slow cells.
        [canonical: structural — tip support per Jordan E tension]"""
        if self.stance == "hold":
            return
        op = oriented_pattern(self.shape, self.tier, self.advance_dir)
        disc_mult = 1.0 if discipline >= 5 else (0.7 if discipline >= 3 else 0.4)
        stance_mod = STANCE_SPEED_MOD[self.stance]
        # Tip support: compute minimum non-zero speed in this atom's pattern
        # Cells with speed > min cannot advance more than TIP_SUPPORT_GAP past the
        # minimum-offset cell with min_speed.
        ...
        for orig_r, orig_c, or_r, or_c in op:
            ...
            if actual_speed == 0:
                continue
            # Phase E tip-support gate
            # [canonical: structural — tip support constraint]
            if TIP_SUPPORT_ENABLED and base_speed > min_speed:
                ...
                if slow_offsets and current_offset >= min(slow_offsets) + TIP_SUPPORT_GAP:
                    continue  # tip too far ahead — hold this turn

# SIM-MB-06 v9 — Unit types + Phase 2 Volley (ranged at range)
# Session: 2026-05-12 | Iteration v8 -> v9
# [canonical: Jordan handoff — unit_type proper + ranged-not-adjacent]
#
# v9 changes (unit-type infrastructure + ranged):
#   - Atom.unit_type ('melee'|'ranged') properly used (was stored, unused in v8)
#   - Phase 2 Volley: ranged atoms fire BEFORE Phase 3 movement at any enemy
#     in line of sight (no adjacency requirement). Pool = Power dice vs TN 6
#     per PP-503; ranged DR via mass_combat.md Ranged DR Table.
#   - Melee atoms (default) behave exactly as v8 (no regression).
#   - Battery against historical-precedent bands; see sim_mb_06_v9_historical_spec.md.
...
def volley_phase(unit_a, unit_b):
    ...
    for atom in unit_a.atoms:
        dmg, tgt = fire(atom, unit_a, unit_b.atoms)
        if tgt is not None:
            loss_b += dmg
            shots += 1
    for atom in unit_b.atoms:
        dmg, tgt = fire(atom, unit_b, unit_a.atoms)
        if tgt is not None:
            loss_a += dmg
            shots += 1

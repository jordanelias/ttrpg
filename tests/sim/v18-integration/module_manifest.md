...
| T0-7 | `sim/provincial/treaty.py` | `faction_balance_convergence_v12c §4.5 + §4.7` | **partial** — process_treaty_expirations against §4.5 (lapse_rate=0.90); register_treaty/get_active_treaties helpers; propose_treaty raises (canon formation = Senator Outward, treaty_expiration_v30 §2 CANONICAL) |
...
## Pass 2l Stub Infill — parliamentary_transfer (Lane C 2026-05-31)

| Module | Depends On | Canonical Sources | Status |
|---|---|---|---|
| `sim/provincial/parliamentary_transfer.py` | `parliamentary_vote` (§10); `parliamentary_stay` (§10.1, post-roll); `dice_engine`; `game_state` | `designs/provincial/parliamentary_transfer_v30.md` (CANONICAL Pass 2h, v12c N=1000) §§1-4; `social_contest_v30 §10/§10.1` | **verified** — `propose_transfer`: CB-gated territorial transfer wrapped in §10 vote. Pool=Proposer Influence(±1D vote bloc), Ob=Holder Legitimacy+2; Continuous-Engine degree → §1.2 outcomes (OW/Success transfer + Accord set, OW holder L-1; Partial fail + proposer CB; Failure proposer Sta-1 / holder L+1, mode-adjusted). 4 modes (§2), 8 CB sources (§3), §1.3 protections. Applies mutations + returns TransferResult (crown_initiative convention). NOT P1-gated (self-specified resolution, independent of disputed Domain Action resolver). [ASSUMPTION] CB ledger duck-typed (world.casus_belli) + auto Crown-restoration. [FLAG] vote-bloc/mode effects canon-PROVISIONAL; Territory.owner not synced (matches faction_action.py). Verified: 4 blocks + 80-seed sweep (24/41/15) + transfer mutation + mode effects. |

"""
Valoria Campaign Simulation Engine v4.0 (foundational per-settlement migration)
ED-724: per-settlement data model. Replaces engine_v3's per-territory aggregate.
Forked from engine_v3 2026-04-20. Non-destructive — engine_v3 remains available.

v4.0 scope (per engine_v4_per_settlement_proposal.md):
- Settlement dataclass with religious_building axis (ED-722)
- Territory.settlement_ids + Territory.sw as derived property
- init_state populates settlements from settlement_layer_v30 §1.6a seed table
- sack_settlement_building / build_settlement_building helpers operate at settlement scale
- All other mechanics (CI, Mass Seizure, Battle, AI) unchanged from v3 — they read
  Territory.sw / Territory.templar as aggregates (SW now derived, Templar still
  territory-level pending v4.1).

v4.1+ (deferred): per-settlement seizure, Capture sequence, Bishop appointment, black markets.
"""
import random, math
...
# ═══════════════════════════════════════════════════════════════
# SETTLEMENT SEEDS (ED-722 §1.6a — per-settlement religious-building assignments)
# [canonical: designs/territory/settlement_layer_v30.md §1.6a]
# Σ religious_building_tier per territory = SW seed value from geography.md
# ═══════════════════════════════════════════════════════════════
SETTLEMENT_SEEDS = {
    # T1 Valorsplatz SW=2 → S-003 Church (2)
    'S-001': dict(id='S-001', name='Valorsplatz Palace',    territory_id='T1', settlement_type='Seat',     controller='Crown',    religious_building='None'),
    'S-002': dict(id='S-002', name='Valorsplatz Riverside', territory_id='T1', settlement_type='Port',     controller='Crown',    religious_building='None'),
    'S-003': dict(id='S-003', name='Valorsplatz Cathedral', territory_id='T1', settlement_type='Cathedral',controller='Church',   religious_building='Church'),
    # T2 Kronmark SW=2 → S-004 Chapel + S-005 Chapel
    'S-004': dict(id='S-004', name='Kronmark',              territory_id='T2', settlement_type='Town',     controller='Crown',    religious_building='Chapel'),
    'S-005': dict(id='S-005', name='Kronmark Watchtower',   territory_id='T2', settlement_type='Fortress', controller='Crown',    religious_building='Chapel'),
    # T3 Lowenskyst SW=2 → S-006 Chapel + S-007 Chapel
    'S-006': dict(id='S-006', name='Lowenskyst Fortress',   territory_id='T3', settlement_type='Fortress', controller='Crown',    religious_building='Chapel'),
    'S-007': dict(id='S-007', name='Lowenskyst Garrison',   territory_id='T3', settlement_type='Town',     controller='Crown',    religious_building='Chapel'),
    # T5 Feldmark SW=2 → S-008 Church
    'S-008': dict(id='S-008', name='Feldmark',              territory_id='T5', settlement_type='Town',     controller='Crown',    religious_building='Church'),
    'S-009': dict(id='S-009', name='Feldmark Storehouse',   territory_id='T5', settlement_type='Mine',     controller='Crown',    religious_building='None'),
    # T6 Stillhelm SW=1 → S-010 Chapel
    'S-010': dict(id='S-010', name='Stillhelm',             territory_id='T6', settlement_type='Town',     controller='Crown',    religious_building='Chapel'),
    'S-011': dict(id='S-011', name='Stillhelm Watch',       territory_id='T6', settlement_type='Outpost',  controller='Crown',    religious_building='None'),
    # T14 Ehrenfeld SW=3 → S-012 Church + S-013 Chapel
    'S-012': dict(id='S-012', name='Ehrenfeld Citadel',     territory_id='T14',settlement_type='Fortress', controller='Crown',    religious_building='Church'),
    'S-013': dict(id='S-013', name='Ehrenfeld Market',      territory_id='T14',settlement_type='City',     controller='Crown',    religious_building='Chapel'),
    'S-014': dict(id='S-014', name='Ehrenfeld Barracks',    territory_id='T14',settlement_type='Fortress', controller='Lowenritter', religious_building='None'),
    # T8 Gransol SW=3 → S-015 Church + S-017 Chapel
    'S-015': dict(id='S-015', name='Gransol Parliament',    territory_id='T8', settlement_type='Seat',     controller='Hafenmark',religious_building='Church'),
    'S-016': dict(id='S-016', name='Gransol Harbor',        territory_id='T8', settlement_type='Port',     controller='Hafenmark',religious_building='None'),
    'S-017': dict(id='S-017', name='Gransol Market',        territory_id='T8', settlement_type='City',     controller='Guilds',   religious_building='Chapel'),
    # T7 Rendstad SW=2 → S-018 Church
    'S-018': dict(id='S-018', name='Rendstad',              territory_id='T7', settlement_type='Town',     controller='Hafenmark',religious_building='Church'),
    # T10 Spartfell SW=2 → S-019 Chapel + S-020 Chapel
    'S-019': dict(id='S-019', name='Spartfell Fortress',    territory_id='T10',settlement_type='Fortress', controller='Hafenmark',religious_building='Chapel'),
    'S-020': dict(id='S-020', name='Spartfell Village',     territory_id='T10',settlement_type='Town',     controller='Hafenmark',religious_building='Chapel'),
    # T17 Halvarshelm SW=2 → S-022 Church
    'S-021': dict(id='S-021', name='Halvarshelm Mines',     territory_id='T17',settlement_type='Mine',     controller='Hafenmark',religious_building='None'),
    'S-022': dict(id='S-022', name='Halvarshelm Town',      territory_id='T17',settlement_type='Town',     controller='Hafenmark',religious_building='Church'),
    # T9 Himmelenger SW=5 → S-023 Cathedral (3) + S-025 Church (2)
    'S-023': dict(id='S-023', name='Himmelenger Cathedral', territory_id='T9', settlement_type='Cathedral',controller='Church',   religious_building='Cathedral'),
    'S-024': dict(id='S-024', name='Himmelenger City',      territory_id='T9', settlement_type='City',     controller='Church',   religious_building='None'),
    'S-025': dict(id='S-025', name='Himmelenger Seminary',  territory_id='T9', settlement_type='Cathedral',controller='Church',   religious_building='Church'),
    # T12 Sigurdshelm SW=2 → S-026 Church
    'S-026': dict(id='S-026', name='Sigurdshelm Keep',      territory_id='T12',settlement_type='Seat',     controller='Varfell',  religious_building='Church'),
    'S-027': dict(id='S-027', name='Sigurdshelm Cove',      territory_id='T12',settlement_type='Port',     controller='Varfell',  religious_building='None'),
    # T4 Grauwald SW=1 → S-028 Chapel
    'S-028': dict(id='S-028', name='Grauwald',              territory_id='T4', settlement_type='Town',     controller='Varfell',  religious_building='Chapel'),
    'S-029': dict(id='S-029', name='Grauwald Lodge',        territory_id='T4', settlement_type='Outpost',  controller='RM',       religious_building='None'),
    # T11 Halvardshelm SW=1 → S-030 Chapel
    'S-030': dict(id='S-030', name='Halvardshelm',          territory_id='T11',settlement_type='Town',     controller='Varfell',  religious_building='Chapel'),
    # T13 Oastad SW=1 → S-031 Chapel
    'S-031': dict(id='S-031', name='Oastad',                territory_id='T13',settlement_type='Town',     controller='Varfell',  religious_building='Chapel'),
    'S-032': dict(id='S-032', name='Oastad Shrine',         territory_id='T13',settlement_type='Outpost',  controller='RM',       religious_building='None'),
    # T15 Askeheim SW=0 (Calamity ground)
    'S-033': dict(id='S-033', name='Askeheim Ruins',        territory_id='T15',settlement_type='Outpost',  controller='Wardens',  religious_building='None'),
    'S-034': dict(id='S-034', name='Askeheim Gate',         territory_id='T15',settlement_type='Outpost',  controller='Uncontrolled', religious_building='None'),
}
...
class Territory:
    ...
    fort: int = 0
    garrison: bool = False
    # v4.0: settlements populated from settlement_layer_v30 §1.6a at init
    settlement_ids: List[str] = field(default_factory=list)
    # Legacy field retained for backward compat with v3 code paths (seeded from SW seed table)
    _sw_fallback: int = 0
    # Church infrastructure aggregates (kept at territory scale pending v4.1 per-settlement migration)
    church_building: int = 0   # highest-tier (legacy; used by seizure modifier)
    templar: bool = False       # v4.1 will move this to per-settlement
    inquisitor: bool = False
    church_governor: bool = False

    # Back-reference set at init_state (Territory has no direct GameState access)
    _gs_ref = None

    @property
    def sw(self) -> int:
        """ED-722 dynamic SW: Σ religious_building_tier over settlements in this territory, capped 5.
        Falls back to _sw_fallback if no settlements populated (legacy/test init path)."""
        if self._gs_ref is None or not self.settlement_ids:
            return self._sw_fallback
        total = sum(self._gs_ref.settlements[sid].religious_building_tier
                    for sid in self.settlement_ids
                    if sid in self._gs_ref.settlements)
        return min(5, total)

    @sw.setter
    def sw(self, value: int):
        """Legacy setter — updates _sw_fallback only. New code should mutate settlements directly."""
        self._sw_fallback = max(0, min(5, value))

    @property
    def church_infra_modifier(self) -> int:
        # [canonical: editorial_decisions §4 — Chapel -1, Church -2, Cathedral -3, Inquisitor -1, Templar -1]
        mod = 0
        mod -= self.church_building
        if self.templar: mod -= 1
        if self.inquisitor: mod -= 1
        return mod

# v4.0 (ED-724): Settlement as first-class entity
@dataclass
class Settlement:
    id: str                       # e.g., 'S-023'
    name: str
    ...
    controller: str
    religious_building: str = 'None'  # None / Chapel / Church / Cathedral (ED-722 Axis 1)
    prosperity: int = 3
    defense: int = 2
    order: int = 3
    ...
    def religious_building_tier(self) -> int:
        """ED-722: None=0, Chapel=1, Church=2, Cathedral=3."""
...
class GameState:
    ...
    settlements: Dict[str, Settlement] = field(default_factory=dict)  # v4.0 (ED-724)
...
def init_state(seed: int = 42) -> GameState:
    random.seed(seed)
    gs = GameState()
    # Territories (sw removed from positional; now derived from settlements)
    for tid, (name, pv, sw, pr, ctrl, accord, pt) in TERRITORIES.items():
        t = Territory(id=tid, name=name, pv=pv, pr=pr, controller=ctrl, accord=accord, pt=pt)
        t._sw_fallback = sw  # legacy fallback; overridden by settlements once populated
        t._gs_ref = gs       # back-ref for sw property resolution
        gs.territories[tid] = t
    # v4.0: populate settlements from settlement_layer_v30 §1.6a seed table (ED-722 seeds)
    for s_id, s_data in SETTLEMENT_SEEDS.items():
        s = Settlement(**s_data)
        gs.settlements[s_id] = s
        # Register with parent territory
        if s.territory_id in gs.territories:
            gs.territories[s.territory_id].settlement_ids.append(s_id)
    # Church infrastructure starting state (legacy per-territory aggregates)
...
# ═══════════════════════════════════════════════════════════════
# DYNAMIC SW MUTATIONS (ED-722) — v4.0 per-settlement
# [canonical: ci_political_v30 §1, settlement_layer_v30 §1.5/§1.6a]
# ═══════════════════════════════════════════════════════════════

_BUILDING_TIERS = ['None', 'Chapel', 'Church', 'Cathedral']

def sack_settlement_building(gs: GameState, settlement_id: str, reason: str = 'sack'):
    """Destroy (downgrade to None) the religious building at a specific settlement.
    Updates Territory.sw automatically via the property."""
    s = gs.settlements.get(settlement_id)
    if not s or s.religious_building == 'None':
        return
    t = gs.territories.get(s.territory_id)
    old_sw = t.sw if t else None
    old_building = s.religious_building
    s.religious_building = 'None'
    # Update legacy territory.church_building to highest remaining in settlements
    if t:
        remaining = [gs.settlements[sid].religious_building_tier
                     for sid in t.settlement_ids if sid in gs.settlements]
        t.church_building = max(remaining) if remaining else 0
        gs.log.append(f"S{gs.season}: {reason} @ {s.name} — {old_building}→None, SW({t.name}) {old_sw}→{t.sw}")

def build_settlement_building(gs: GameState, settlement_id: str, target: str = 'upgrade',
                              reason: str = 'Ecclesiastical Appointment'):
    """Construct or upgrade religious building in a settlement.
    target='upgrade': advance one tier (None→Chapel→Church→Cathedral).
    target='Chapel'/'Church'/'Cathedral': set directly (only if current is lower)."""
    s = gs.settlements.get(settlement_id)
    if not s:
        return
    t = gs.territories.get(s.territory_id)
    ...
    old_building = s.religious_building
    if target == 'upgrade':
        idx = _BUILDING_TIERS.index(s.religious_building)
        if idx < 3:
            s.religious_building = _BUILDING_TIERS[idx + 1]
    elif target in _BUILDING_TIERS:
        target_idx = _BUILDING_TIERS.index(target)
        current_idx = _BUILDING_TIERS.index(s.religious_building)
        if target_idx > current_idx:
            s.religious_building = target
    # Update legacy territory.church_building
    if t:
        remaining = [gs.settlements[sid].religious_building_tier
                     for sid in t.settlement_ids if sid in gs.settlements]
        t.church_building = max(remaining) if remaining else 0
        if old_building != s.religious_building:
            gs.log.append(f"S{gs.season}: {reason} @ {s.name} — {old_building}→{s.religious_building}, SW({t.name}) {old_sw}→{t.sw}")

# Territory-level compatibility helpers (for v3-style callers)
def sack_religious_building(gs: GameState, territory_id: str, tier_lost: int = 3, reason: str = 'sack'):
    """v3 compat: sack the highest-tier religious building in the territory.
    Iterates to match tier_lost (typically 3 = 1 Cathedral, 2 = 1 Church, 1 = 1 Chapel)."""
    t = gs.territories.get(territory_id)
    if not t:
        return
    # Find highest-tier settlement building and destroy
    candidates = sorted(
        [(gs.settlements[sid].religious_building_tier, sid)
         for sid in t.settlement_ids
         if sid in gs.settlements and gs.settlements[sid].religious_building != 'None'],
        reverse=True
    )
    lost_so_far = 0
    for tier, sid in candidates:
        if lost_so_far >= tier_lost:
            break
        sack_settlement_building(gs, sid, reason)
        lost_so_far += tier

def build_religious_building(gs: GameState, territory_id: str, tier_added: int = 1, reason: str = 'Ecclesiastical Appointment'):
    """v3 compat: upgrade or construct in the lowest-tier non-Cathedral settlement."""
    t = gs.territories.get(territory_id)
    if not t:
        return
    candidates = sorted(
        [(gs.settlements[sid].religious_building_tier, sid)
         for sid in t.settlement_ids
         if sid in gs.settlements and gs.settlements[sid].religious_building != 'Cathedral'],
    )
    added = 0
    for tier, sid in candidates:
        if added >= tier_added:
            break
        build_settlement_building(gs, sid, target='upgrade', reason=reason)
        added += 1

...
Mutation-verified at introduction: editing MB_WHEEL's default '1'→'0' in the
...
_KNOWN_INERT = {
    ...
    # ⚠ The previous rationale was CIRCULAR — "no 'yield' order, MB_YIELD_EMERGENT off" — i.e. the flag
    ...
    # a bat.py digest at the shipped defaults. Note this is a claim about the BATTERY only — MB_YIELD_RALLY
    ...
    'MB_YIELD_POCKET', 'YIELD_POCKET_REACH', 'D_YIELD', 'YIELD_POOL_MULT',
    # no-ops while MB_FACING_MODEL is pinned '0':
    'MB_FACING_ATTENTION', 'MB_FACING_SLEW_BASE', 'MB_FACING_FOV_GATE', 'MB_FACING_ROUT',
    ...
    'MB_FEIGNED_RETREAT', 'FEIGNED_RECOGNIZE_OB', 'FEIGNED_RETREAT_OB', 'OVEREXTEND_PENALTY',
...
def test_no_unclassified_env_reads():
    ...
    # non-literal-default reads (e.g. YIELD_POOL_MULT's str(MB_SHOCK_HOLD_BRACE)) are
...
def test_mode_selectors_cover_every_out_of_budget_golden_mode():
    ...
    for mode, sel in MODES.items():
        ...
        assert sel['MB_NODE_COHESION'] == want_field, mode
        ...
        if morale == 'mor1':
            assert sel.get('MB_CELL_MORALE') == '1', mode
    # the §4a mode is the ONLY one that overrides the MB_CELL_MORALE pin, and it must
    assert MODES['cell_legacy_mor1']['MB_CELL_MORALE'] == '1'
    for mode in ('unit_field_mor0', 'cell_field_mor0'):
        assert 'MB_CELL_MORALE' not in MODES[mode], (
            f"{mode} must inherit the FIELD_PINS MB_CELL_MORALE='0'; overriding it here would "
...
def test_grid_pin_dict_consistency():
    ...
    # FIELD_MOVEMENT/MB_NODE_COHESION legitimately differ (mode selectors, and
...
def test_known_inert_reasons_are_not_self_referential():
    ...
    no 'yield' order, MB_YIELD_EMERGENT off" and "self-gated OFF". Those are CIRCULAR: the flag is
    ...
    about the BATTERY only; MB_YIELD_RALLY is not behaviourless in general — test_dg2_yield_residuals
    ...
    # CMD_COG_WEIGHT, MB_VOLLEY_TARGETING) are deliberately OUT of scope: their reasons cite ANOTHER
    ...
    _FLIPPED_BY_THE_RULING = {'MB_FEIGNED_RETREAT', 'MB_RESERVE_COMMIT',

...
def test_articulation_states():
    assert workbench.articulation(None, 'x', 'y')['state'] == 'silent'                  # no doc
    assert workbench.articulation('talks about Prosperity here', 'Prosperity', 'Zzz')['state'] == 'mentioned'
    assert workbench.articulation('Prosperity drives Local Economy', 'Prosperity',

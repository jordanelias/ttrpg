...
def _rollup_text(rollup: dict, built: dict) -> str:
    return json.dumps({'_generated': 'GENERATED by tools/build_identifier_census.py',
                       'schema_version': SCHEMA_VERSION,
...
def main(argv=None):
    ...
    if args.check:
        # THE ROLL-UP IS CHECKED TOO -- it was not, and that was the whole defect (ED-IN-0162).
        # `--check` used to `return` here, before the write below, so the ONE file ED-IN-0162
        # names first, `references/identifier_census.json`, was the one file --check could not
        # see. Wiring the old --check into a gate would have shipped an assertion incapable of
        # observing the failure it excludes (CLAUDE.md §0.1 point 2) -- and the roll-up is the
        # copy `tools/dashboard_data.py` and any roll-up reader consume, so it drifting silently
        # is worse than a per-subsystem file drifting.
        # SCOPED RUNS CANNOT CHECK IT: `--subsystem X` computes one row, so comparing a
        # 1-row roll-up against the 15-row committed file would report drift that is an
        # artifact of the flag. Full runs only, and the scoped path says so rather than
        # passing silently.
        if not args.subsystem:
            rp = os.path.join(REPO, ROLLUP)
            if not os.path.exists(rp) or open(rp, encoding='utf-8').read() != _rollup_text(rollup, built):
                drift.append(ROLLUP.replace(os.sep, '/'))
        if drift:
            print(f'[identifier-census] DRIFT in {len(drift)} file(s): {drift[:5]}')
            return 1
        scope = f'{len(subs)} subsystem(s)' + ('' if args.subsystem else ' + the roll-up')

...
def main():
    ...
    if warnings:
        ...
        print("    Action: archive WHOLE settled ids — never individual rows. The ledgers")
        print("    are append-only, so an id's effective status is its LAST row; moving only")
        print("    the resolved row silently reverts it (ED-IN-0112 incident, pinned by")
        print("    tests/valoria/test_ledger_hygiene.py).")
        print("    ⚠ TWO CRITERIA, RULED by Jordan 2026-09-17 (ED-IN-0245), and the second is")
        print("    the one that keeps being missed:")
        print("      1. ANYTHING DATED BEFORE THE CURRENT MONTH leaves the live ledger.")
        print("      2. ANYTHING CARRYING A TERMINAL STATUS leaves it — EVEN IF ITS BODY")
        print("         IMPLIES IT IS STILL CURRENT. The status field decides, not the prose.")
        print("    The live lane ledger is an INDEX OF OPEN WORK, not a history. Deep archive")
        print("    is registers/archive/ as dated .yaml fragments — UNCAPPED (atomization_rules")
        print("    on_exceed: skip) and globbed by validate_ed_citations.py, so every archived")
        print("    id still resolves. Archiving into the _archive.jsonl sibling only defers the")
        print("    problem: on 2026-09-17 that file hit 92% of ITS cap the same hour.")
    if violations:

...
def generate_editorial_summary(active_yaml: str, archive_yamls: dict[str, str] = None) -> str:
    """
    ...
    preventing ID collisions when new entries are filed. Architecture V2.4
    M6 requires this; the prior signature accepted only active and produced
    archive-blind next_id values.
    """
    entries = _collect_ed_entries(active_yaml, archive_yamls)
    ...
    for e in p1:
        # [M6 fix 2026-05-20] quote description to escape colons safely
        desc_q = e['desc'].replace('"', "'")
        lines.append(f'  - {e["id"]}: "{desc_q}"')
    if not p1:
        lines.append("  - none")

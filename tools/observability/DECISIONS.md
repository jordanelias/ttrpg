...
**258 open items** across 470 files · P1 79 · P2 27 · P3 152 · 25 already settled (supersessions).
...
| **FA** — Faction actions | 3 | 0 | 22 | 25 |
| **WR** — World | 0 | 0 | 13 | 13 |
| **IN** — Infrastructure / cross-cutting | 6 | 1 | 15 | 22 |
| **GO** — Godot | 9 | 2 | 10 | 21 |
| **SE** — Settlements | 0 | 0 | 3 | 3 |
| _unassigned_ (`lane: null`) | 55 | 24 | 76 | 155 |

## By category

- **Documented gaps** — 76
- **Flagged assumptions** — 62
- **Open rulings** — 45
- **Naming & collisions** — 33
- **Mechanical wiring** — 15
- **Ratifications** — 13
- **TODO / FIXME** — 9
- **Stubs** — 5
...
- `references/module_contracts.yaml` — 59
- `references/CONTRACT_INDEX.md` — 37
...
- `engine/engine_params/params_tables.yaml` — 4
- `systems/social_contest/sim/parliamentary_vote.py` — 4
- `systems/threadwork/threadwork_flow_skeleton_v1.md` — 4

---

## P1 — decide first (the real rulings you owe)  (79)
...
- registry-derived; runtime-system vs QA-tooling classification [OPEN — Jordan]; home doc unlocated [GAP] _(systems: audit)_  
  ↳ `references/CONTRACT_INDEX.md:265` +1 more
- RECLASSIFIED 2026-06-10: doc is the 2026-04-17 victory-revision CONSOLIDATION (Church infra, RM identity, MS/Coherence reform, Thread revelation, IP escalation, Warden, Portrait/Lineage) — a cross-cutting design doc, not a runtime module; its contents distribute across victory/threadwork/settlement_layer/peninsular_… _(systems: campaign_architecture)_  
  ↳ `references/CONTRACT_INDEX.md:295` +1 more
...
- threadwork_v30 header: "design proposal, requires editorial approval" yet carries CANONICAL POOL NOTICE — doc status ambiguity [OPEN — Jordan] _(systems: threadwork)_  
  ↳ `references/CONTRACT_INDEX.md:1411` +1 more

### Naming & collisions (21)
...
- | `systems/ui/_identifier_census.yaml` (generated, `tools/build_identifier_census.py`) reports 28 identifiers as `disposition: BUILT` against the UI docs' vocabulary, but every `built_in:` pointer for those rows resolves into **other** subsystems' code (e.g. `systems/combat/combat_engine_v1/combatant.py`, `systems/s…  
  ↳ `systems/ui/ui_flow_skeleton_v1.md:46`
...
- added: '2026-06-09 D-3 voice-home consolidation — extracted verbatim from articulation_layer §4.2 (omniscient chronicle voice); home pending Jordan ratification'  
  ↳ `references/canonical_sources.yaml:241`
...
- `[OPEN — Jordan]` markers are carried in `references/CONTRACT_INDEX.md`'s own gap notes. None of that is  
  ↳ `systems/_architecture/engine_atlas_v1.md:365`
- | Retire-vs-merge for the phantom settlement-economy module and its edges | SE | `references/CONTRACT_INDEX.md:122 settlement_economy` `[OPEN — Jordan]`. ED-SE-0005 is **resolved** — it settled the settlement-action half only |  
  ↳ `systems/_architecture/engine_atlas_v1.md:375`
- | The threadwork head's doc-status ambiguity: a header reading "design proposal, requires editorial approval" over a CANONICAL POOL NOTICE | WR | `references/CONTRACT_INDEX.md:108 threadwork` `[OPEN — Jordan]` |  
  ↳ `systems/_architecture/engine_atlas_v1.md:376`
- purpose: '[5. Open decisions] Decision | Lane | Tracked at :: Retire-vs-merge for the phantom settlement-economy module and its edges | SE | `references/CONTRACT_INDEX.md:122 settlement_economy` `[OPEN — Jordan]`. ED-SE-0005 is **resolved** — it settled the settlement-action half only'  
  ↳ `systems/_architecture/_identifier_census.yaml:5129`
- "registry system name faction_layer vs module faction_state — vocabulary unification [OPEN — Jordan]"  
  ↳ `references/module_contracts.yaml:118`
- {type: "state.belief_revised", terminal: false}   # Procedure B Resolution (doc-12 §8) — registry attributes this type to fieldwork: attribution conflict [OPEN — Jordan]  
  ↳ `references/module_contracts.yaml:187`
...
- {open: true}   # 2-cycle with social_contest (emits opinion_revised it consumes; consumes its scene.dialogue/contest_resolved) + self-loop via scene.witness; dampers unconfirmed [OPEN — Jordan]  
  ↳ `references/module_contracts.yaml:203`
...
- {type: "scene.thread_operation", terminal: false}   # substrate §8.4 verbatim — NOT in registry (F2 class); registry instead has meta.thread_woven; same event? [OPEN — Jordan]  
  ↳ `references/module_contracts.yaml:354`
...
- ZERO Key integration in a CANONICAL doc — no registry entry names ci_political; CI=100 Theocracy Unification Attempt (§2.2) is unkeyed [registry §10 candidate] _(systems: ci_political)_  
  ↳ `references/CONTRACT_INDEX.md:330` +1 more
- CANONICAL BG-era doc with ZERO Key integration — no emitting/consuming_systems entry in the registry names it; CV drift consumes thread-op effects (§1.3b ED-676) and Calamity drift (§1.3) with no Key types [registry §10 candidate / Keys-migration pending] _(systems: territorial_piety)_  
  ↳ `references/CONTRACT_INDEX.md:1361` +1 more
- world-state era transitions (§5: MS=0 Post-Calamity, IP=100 Phased Occupation, all-dissolved Anarchy) are UNKEYED — no mechanical_event/state_transition type exists; articulation is blind to era changes via the Key stream [registry §10 candidate] _(systems: victory)_  
  ↳ `references/CONTRACT_INDEX.md:1456` +1 more
...
- registry-derived; home doc unlocated [GAP]; see scene_slate attribution conflict on mechanical.scene_entered _(systems: game_director)_  
  ↳ `references/CONTRACT_INDEX.md:668` +1 more
...
- "ZERO Key integration in a CANONICAL doc — no registry entry names ci_political; CI=100 Theocracy Unification Attempt (§2.2) is unkeyed [registry §10 candidate]"  
  ↳ `references/module_contracts.yaml:812`
- "world-state era transitions (§5: MS=0 Post-Calamity, IP=100 Phased Occupation, all-dissolved Anarchy) are UNKEYED — no mechanical_event/state_transition type exists; articulation is blind to era changes via the Key stream [registry §10 candidate]"  
  ↳ `references/module_contracts.yaml:854`
- W-GAP [territorial_piety]: CANONICAL BG-era doc with ZERO Key integration — no emitting/consuming_systems entry in the registry names it; CV drift consumes thread-op effects (§1.3b ED-676) and Calamity drift (§1.3) with no Key types [registry §10 candidate / Keys-migration pending]  
  ↳ `references/CONTRACT_INDEX.md:105`
- W-GAP [ci_political]: ZERO Key integration in a CANONICAL doc — no registry entry names ci_political; CI=100 Theocracy Unification Attempt (§2.2) is unkeyed [registry §10 candidate]  
  ↳ `references/CONTRACT_INDEX.md:125`
- W-GAP [victory]: world-state era transitions (§5: MS=0 Post-Calamity, IP=100 Phased Occupation, all-dissolved Anarchy) are UNKEYED — no mechanical_event/state_transition type exists; articulation is blind to era changes via the Key stream [registry §10 candidate]  
  ↳ `references/CONTRACT_INDEX.md:127`
...
- npc_behavior: feedback loop open — damper/cap unconfirmed _(systems: npc_behavior)_  
  ↳ `references/module_contracts.yaml`
- personal_combat: feedback loop open — damper/cap unconfirmed _(systems: personal_combat)_  
  ↳ `references/module_contracts.yaml`

### Naming & collisions (12)

- "registry-derived; home doc unlocated [GAP]; see scene_slate attribution conflict on mechanical.scene_entered"  
  ↳ `references/module_contracts.yaml:487`
- W-GAP [game_director]: registry-derived; home doc unlocated [GAP]; see scene_slate attribution conflict on mechanical.scene_entered  
  ↳ `references/CONTRACT_INDEX.md:112`
...
## P3 — cleanup backlog (152)  — summarized
...
- **systems/combat/combat_reference_v1.md** — personal-combat RESOLUTION layer (pool/stamina/wound resolution) — NOTE: PARTIAL only. Repointed from combat_v30.md by t → _PARTIALLY superseded (recorded for completeness, banner already in the file since 2026-06-04). RESOLUTION superseded by combat_engine_v1; config.py supersedes e_

...
def main():
    ...
    # build the lexicon too (reads the graph.json we just wrote, for cross-linking)
    try:
        import build_lexicon
        build_lexicon.main()
    except Exception as e:  # lexicon is additive — never block the graph build
        print(f"  (lexicon build skipped: {e})")
    lex_js = ""
    lex_path = OUT_DIR / "lexicon_data.js"
    if lex_path.exists():

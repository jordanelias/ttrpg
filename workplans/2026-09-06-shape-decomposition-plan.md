...
| 12 | `loop.py` | `SeasonDriver`, `as_scenes`, `stratum_of`, `resolvable_verbs`, `names_a_verb`, `SOURCE_353_TEXT`, `_S353_CACHE`, **`sense` (`:4550`)** — added **2026-09-09**, moved off the `decision.py` row on `04:116` (*"`sense()` called by the loop, never by the decision"*) and `04:158` (`loop/deliberate` *"Calls `sense()`"*) |
...
⚠ **STEPS 7-10 LANDED AND THIS TABLE'S PREDICTIONS FOR THEM ARE SUPERSEDED BY `workplans/2026-09-09-shape-decomposition-plan-v2.md`.** Two of the warnings above did not fire, for a reason worth keeping: step 8 built `seam.py` as a FLAT module and did **not** move `combat_seam.py`, so `PATH_SEAM_ALLOWED` was never touched and `test_engine_does_not_import_systems` stayed green — the rename that row demands was avoided rather than performed. ⚠ **AND THIS ROW WAS RIGHT: the avoidance is the defect, not the saving.** `architecture/meta/04_CODE_ARCHITECTURE.md` §A.2 (`04:135`) and its table's `seam/wrappers/*` row make `seam/` a directory and put a wrapper under it, RATIFIED 2026-09-05 (ED-IN-0204); `v2` §3's *"does not move"* premise is struck there as of 2026-09-10, and **the rename this row demands is performed by unit L2 of `workplans/2026-09-09-layer1-conformance-plan.md`.** Filed as ED-IN-0206. The step-8 row above stands as written. What DID need a co-edit was `test_import_cycle_game_state_npe.py`, which this table never names: dissolving the `combat_seam ↔ shape` cycle took its pinned count 4 → 3.

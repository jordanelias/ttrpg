...
**`workplans/2026-09-09-layer1-conformance-plan_part2.md`** carries §9–§15: the agonist→antagonist
method binding every unit, the fix-on-error rule for broken paths, per-unit model tiering, the
standing instrument set with this session's measured values, what may not happen, the entry state,
and the adversarial-pass record. **Split in reading order per `CLAUDE.md` §4** — sequential parts,
not index+infill — when execution facts took this file past
`references/atomization_rules.yaml`'s 15,000-token cap. Part 1 is WHAT the work is; part 2 is HOW it
is run. **Both are required reading before the first edit of any unit.**

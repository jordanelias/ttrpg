...
| **13d** | **10 — OFFICES** | IN | `offices.yaml` — bases as rostered values · both predicates rewritten · four title helpers + `is_title` + the `titles` roster deleted · ✅ **holders seated** · purview corrected. **SPLIT 2026-09-25 into `13d-i` and `13d-ii` below** | **OPEN** ⭐ · **1 of 5 done** | — its two build-order deps are spent (16 DONE, 4 REVERTED). ⚠⚠ **`13b` MADE THIS THE POSITION THAT UNBLOCKS GOVERNANCE, and it was not that before.** The remaining 8 of 9 remit verbs are now blocked by **office DATA, not by any mechanism**: `harness/populated.py:706` — *"EVERY OTHER OFFICE CARRIES `remit_acts: []`, WHICH IS A DECLARED ABSENCE"*, and only 3 of 143 corpus cases grant anything. Seating holders on offices with populated remits now grants them automatically, because the grant rides on the Tenure. **This is the cheapest remaining move on `R-05`**. ⚠ **PARTIAL 2026-09-19 — see `§3.8b`:** *holders seated* landed as the 13-seat generic spine (`engine/season/governance_spine.yaml`). Purview is still behind `Act.via` (position 6, Arc-2) and the `is_title` deletion is behind `ED-IN-0256` ruling (3) being BUILT. **Do not read this position as closed** |
| **13d-i** | **OFFICES AS DATA** | IN | conferral (`appointed · elected · annex`, `ED-IN-0256` ruling (2)) and revocation (*"rung above of same faction"*, ruling (3)) as rostered values · `_req_confer` / `_req_revoke` rewritten on them · the four title helpers + `is_title` deleted · the title-in-a-body refusal RE-HOMED, not deleted · ~~`engine/season/data/offices.yaml`, read by `build_realm`~~ | **DONE — items (1)-(4)** ⭐ | ✅ **`ED-IN-0273`, 2026-09-26** — `rosters.yaml`, `loop/predicates.py`, `data/rosters.py`, `state/carriers.py`; `H-109` closed (`hole_register.yaml`, `source:` appended). Item (5), `offices.yaml` + its `harness/populated.py` wiring, is a SEPARATE deferred unit — `title_domain`/the `titles` roster are KEPT because `populated.py`'s populated-realm seating still calls `title_domain`, and r2's own two documents disagree about whether `titles` is ultimately deleted (`03` §A.12: stays; `05` RULED (c): folds into `offices.yaml`) — left for that unit or Jordan. Verified by an antagonist pass that found the built "immediate parent" reading of ruling (3) had no falsifier distinguishing it from "nearest same-faction ancestor above"; closed with a dedicated divergence test, mutation-tested against the rejected reading. ⚠ **r2 `03:1043-1056`'s roster VALUES are SUPERSEDED** (`conferral_bases: [confer, determine, succeed]`, `revocation_bases: [purview, holdings, none]`, which its 29 seats are authored on). Both lists predate `ED-IN-0256` rulings (2) and (3). r2 supplies the STRUCTURE (closed rosters, a basis per seat); the ED supplies the VALUES; mapping r2's seats onto them is item (5)'s work. Detail `_part2` 13d |
...
| **24d-i** | **DWELLING SUBSTRATE** | SE | `dwelling` added to `site_kinds`, plus the rows the loader then forces (`wear_per_season`, `band_floors`) · `build_realm` mints one `dwelling` Site per `hearth` rung | **DONE, NOT `DONE·INERT`** | ✅ **`ED-IN-0274`, 2026-09-26** (built `ED-IN-0273`'s follow-on, 2026-09-25/26) — `rosters.yaml`, `data/rosters.py`, `harness/populated.py`, `harness/governance_spine.py`; measured, not assumed: Events +253 on one populated season (211 `condition.worn`, one per dwelling; 42 `claim.deposited`, one per living resident, none moved home). Questions and the resolved-act sequence hold identical through season 3; claim displacement at `ledger_cap` starts season 2 and the act sequence itself first diverges mid-season-4 — so the label is **DONE, and explicitly NOT `DONE·INERT`** past one season, per the falsifier's own instruction for exactly this outcome. Verified by an antagonist pass that caught a missed CI-blocking artifact regen, a latent `ValueError` in `queries/world_q.py` (fixed, one token), nine stale line citations from this position's own insertion, and a false "same shape as `body_step=0`" claim in `rosters.yaml` (all corrected). Detail `_part2` 24d |
...
`harness/governance_spine.build` builds `lr_hearth_a` / `lr_hearth_b` (`governance_spine.py:127`;
